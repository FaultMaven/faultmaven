...
- **Redefined the default LLM model per provider to one coherent, valid set** across `settings.py` and `registry.py` (e.g. Anthropic `claude-sonnet-4-6`, Fireworks `accounts/fireworks/models/qwen2p5-coder-32b-instruct`, Groq `llama-3.3-70b-versatile`, Gemini `gemini-2.0-flash`). Previously `settings.py`, `registry.py`, and `.env.example` disagreed and some IDs were invalid (`qwen2.5` vs `qwen2p5`, deprecated `claude-3-sonnet-20240229`). Added a `groq_model` field for parity and fixed the registry to resolve Groq/Cohere via the canonical model field.
