...
- Every migration runs on **SQLite and PostgreSQL**: RLS is PostgreSQL-only DDL and dialect-guarded; triggers and partial indexes exist on both dialects with per-dialect spellings (`sqlite_where=`/`postgresql_where=`, separate trigger syntax); column types stay SQLite-compatible. Conventions: `docs/guides/database-migrations.md`; `/migrate` command.
