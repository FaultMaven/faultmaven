...
| Teams | `DELETE /teams/{team_id}/members/me` | Leave; 409 for the last admin while others remain, and the sole member leaving retires the team and revokes its pending offers. Both decided under a row lock on the team — the rule is a read-then-write over the roster |
...
| Invitations | `POST /invitations/{invitation_id}/accept` | Consent. **The only call that creates a team membership** — written BEFORE the offer is stamped, so a failure leaves the offer retryable rather than spent; 410 once it has expired |
