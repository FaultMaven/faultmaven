...
from sqlalchemy import (
    String,
    Text,
)  # autogenerate emits bare type names inside dialect variants
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "status", sa.String(length=50), server_default="inquiry", nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "tags",
            sa.Text().with_variant(postgresql.ARRAY(String(length=50)), "postgresql"),
            nullable=True,
        ),
        sa.Column(
            "embedding_model",
    ...
    op.create_table(
        ...
        sa.Column(
            "status", sa.String(length=20), server_default="captured", nullable=False
        ),
        sa.Column(
            "likelihood",
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "LENGTH(TRIM(content)) > 0", name="evidence_content_not_empty"
        ),
        sa.CheckConstraint(
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_agent_tool_calls_execution_id"), table_name="agent_tool_calls"
    )
