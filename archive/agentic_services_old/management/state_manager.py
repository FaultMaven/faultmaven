...
class AgentStateManager(IAgentStateManager):
    ...
    async def update_conversation_memory(self, session_id: str, memory: ConversationMemory) -> bool:
        ...
        try:
            ...
            # Convert to dict for storage, handle both ConversationMemory objects and plain dicts
            if isinstance(memory, dict):
                # Handle legacy case where dict is passed
                logger.warning(f"Received dict instead of ConversationMemory for session {session_id}, converting")
                ...
                logger.error(f"Invalid memory object type for session {session_id}: {type(memory)}")
                return False
    ...
    async def get_investigation_state(self, case_id: str):
        """Retrieve investigation state for a case (v3.3.0 - FIXED: use case_id, not session_id)
        ...
        """
        try:
            # FIXED: Changed from investigation_state:{session_id} to investigation:{case_id}
            ...
            if not state_data:
                logger.debug(f"No investigation state found for case {case_id}")
                return None
            ...
            logger.debug(f"Retrieved investigation state for case {case_id}")

            return state

        except Exception as e:
            logger.error(f"Failed to retrieve investigation state for case {case_id}: {e}")
            return None

    @trace("state_manager_update_investigation_state")
    async def update_investigation_state(self, case_id: str, state, ttl: int = None) -> bool:
        """Update investigation state for a case (v3.3.0 - FIXED: use case_id, not session_id)
        ...
        Args:
            ...
            ttl: Optional TTL override (defaults to 30 days to match case TTL)
        ...
        try:
            ...
            # FIXED: Changed TTL from session TTL (hours) to case TTL (30 days)
            CASE_TTL = 30 * 24 * 60 * 60  # 30 days in seconds
            ttl = ttl or CASE_TTL
            ...
            if success:
                logger.debug(f"Updated investigation state for case {case_id}")
            else:
                logger.error(f"Failed to store investigation state for case {case_id}")

            return success

        except Exception as e:
            logger.error(f"Failed to update investigation state for case {case_id}: {e}")
            return False
    ...
    async def initialize_investigation_session(
        ...
        """Initialize a new investigation for a case with OODA framework (v3.3.0 - FIXED)

        Investigation state belongs to CASE (permanent), not SESSION (temporary).
        See case-and-session-concepts.md for architectural principles.

        Args:
            case_id: Case identifier (investigation state keyed by case_id)
            session_id: Session identifier (optional, for audit trail only)
        ...
        try:
            ...
            # Note: session_id tracked in metadata for audit, but state is keyed by case_id
            ...
            # FIXED: Store investigation state by case_id (not session_id!)
            success = await self.update_investigation_state(case_id, initial_state)

            if success:
                logger.info(f"Initialized investigation for case {case_id}")
            else:
                logger.error(f"Failed to initialize investigation for case {case_id}")

            return success

        except Exception as e:
            logger.error(f"Failed to initialize investigation for case {case_id}: {e}")
            return False
