    ...
    async def _create_view_state(
        ...
        try:
            ...
            if diagnostic_state and diagnostic_state.investigation_state_id and self._state_manager:
                # FIXED: Get investigation state from state manager using case_id (not session_id!)
                # Investigation state belongs to CASE (permanent), not SESSION (temporary)
                investigation_state = await self._state_manager.get_investigation_state(case_id)
