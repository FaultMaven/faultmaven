...
"""
    ...
    def _assess_investigation_completeness(
        self,
        investigation_state: InvestigationState,
    ) -> tuple[str, Dict[str, Any]]:
        """Assess investigation completeness level (v3.0)

        Four completeness levels based on phases reached:
        - FULL: Phases 0-6, validated root cause (≥70%)
        - HIGH: Phases 0-4, hypothesis validation complete
        - PARTIAL: Phases 0-2, scope/timeline but no hypotheses
        - MINIMAL: Phases 0-1, basic problem assessment only

        Args:
            investigation_state: Current investigation state

        Returns:
            Tuple of (completeness_level, assessment_details)
        """
        phase_history = [t['to_phase'] for t in investigation_state.lifecycle.phase_history]
        ...
        # Track which phases were completed
        phases_completed = set(phase_history)
        phases_completed.add(current_phase.value)

        # Check validation confidence
        validated_hypothesis = (
            working_conclusion and working_conclusion.confidence >= 0.70
        ) if working_conclusion else False

        # Assess completeness
        if 6 in phases_completed:
            # Reached document phase
            if validated_hypothesis:
                level = "FULL"
                description = "Complete investigation: validated root cause with documentation"
            else:
                level = "HIGH"
                description = "High completeness: investigation completed but root cause not fully validated"
        elif 4 in phases_completed:
            # Reached validation phase
            if validated_hypothesis:
                level = "HIGH"
                description = "High completeness: hypothesis validated (≥70% confidence)"
            else:
                level = "PARTIAL"
                description = "Partial completeness: validation attempted but confidence <70%"
        elif 2 in phases_completed or 3 in phases_completed:
            # Reached timeline or hypothesis
            level = "PARTIAL"
            description = "Partial completeness: scope and timeline established"
        else:
            # Only intake/blast radius
            level = "MINIMAL"
            description = "Minimal completeness: basic problem assessment only"
        ...
        assessment = {
            ...
            "recommended_artifacts": self._get_recommended_artifacts(level),
        }

        return level, assessment

    def _get_recommended_artifacts(self, completeness_level: str) -> List[str]:
        """Get recommended documentation artifacts based on completeness

        Args:
            completeness_level: FULL, HIGH, PARTIAL, or MINIMAL

        Returns:
            List of recommended artifact types
        """
        artifacts_by_level = {
            "FULL": [
                "Case Report (complete)",
                "Runbook (reproduction + solution)",
                "Knowledge Base Entry (problem pattern)",
                "Post-Mortem Document"
            ],
            "HIGH": [
                "Case Report (validated hypothesis)",
                "Runbook (reproduction only)",
                "Knowledge Base Entry (partial)",
            ],
            "PARTIAL": [
            ...
            "MINIMAL": [
                "Problem Report (basic)",
                "Escalation Summary"
            ],
        }
        return artifacts_by_level.get(completeness_level, [])

    async def check_completion(
        self,
        investigation_state: InvestigationState,
    ) -> tuple[bool, List[str], List[str]]:
        """Check if Phase 6 completion criteria met (v3.0)

        Criteria:
        - Investigation completeness assessed
        - Appropriate artifacts offered based on completeness
        ...
        # v3.0: Assess investigation completeness
        completeness_level, assessment = self._assess_investigation_completeness(investigation_state)
        met_criteria.append(f"✓ Completeness assessed: {completeness_level}")
        met_criteria.append(f"  → {assessment['description']}")

        # Store completeness level for analytics (v3.0)
        investigation_state.lifecycle.completeness_level = completeness_level

        # Note: mark_complete workflow progression happens in Phase 5, not Phase 6
        # By the time we reach Phase 6, case status is already RESOLVED/CLOSED
        # Phase 6 is purely for documentation generation

        # Check if artifacts offered
        if investigation_state.lifecycle.artifacts_offered:
            met_criteria.append("✓ Artifacts offered to user")
            met_criteria.append(f"  → Recommended: {', '.join(assessment['recommended_artifacts'][:2])}")
        else:
