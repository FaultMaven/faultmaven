...
- **Owns Data**: the case-domain PostgreSQL tables — `cases`, `evidence`, `hypotheses`, `solutions`, `case_messages`, `uploaded_files`, `case_actions` (audit table; ORM model `CaseActionModel`), `case_checkpoints`, `reports`, and related high-cardinality tables
