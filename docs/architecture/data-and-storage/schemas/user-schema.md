...
CREATE TABLE users (
    ...
    hashed_password VARCHAR(255),  -- NULL for an SSO-only user AND for every service account
    ...
    -- Account Status
    -- The kill switch. Setting this false refuses the account's refresh
    -- credential (`OAuthService.refresh_access_token`) and writes the per-user
    -- revocation watermark, so every outstanding token dies with it. For a
    -- service account it is the complete answer, because the credential is the
    -- only door: there is no password and no SSO identity to fall back to.
    ...
    -- value here, not a third account kind. One service account per Slack
    -- workspace (ADR-012 D10, ADR-017 D6), and its cases auto-share to every
    -- team it belongs to — so it must belong to exactly one.
    ...
    -- Its absence is also what permits a service account, which has neither.
);
...
### What a service account row looks like

A service account (`account_kind = 'service'`) is a real row in this table, and the
columns above carry values a reader would not guess:

| Column | Value | Why |
|---|---|---|
| `username` | `slack-<Slack workspace id>` | Derived, not chosen. The global unique index on `username` is what makes "one service account per workspace" a database guarantee rather than a convention, and it is why taking that username would lock the rightful enterprise out. |
| `email` | `<username>@faultmaven.example` | Auto-generated by the user store when the provisioner supplies none. It satisfies `NOT NULL UNIQUE` and the format CHECK, is **deliberately not routable**, and is never sent to. |
| `hashed_password` | NULL | |
| `sso_provider` / `sso_provider_id` | NULL | |
| `enterprise_id` | Operator-supplied at provisioning | Required under `TENANT_PROVIDER=multi`. A service account has no sign-in from which a domain could be read, so it is **exempt from the domain derivation** that gives a human their enterprise (ADR-017 D3); `fm-provision-service-account --enterprise-id` supplies it instead. This is why the `@faultmaven.example` domain never becomes an enterprise. |

It follows that **a service account cannot sign in by any route**: no password, no
identity-provider record, and an address nobody can receive mail at. It authenticates
only by a single-use rotating refresh credential, and `is_active = false` is the
complete way to stop it.
...
CREATE TABLE organizations (
    ...
    -- Constraints
    -- Tier 1 (live ORM) — mirrored on Pydantic ``Organization.name`` via field validator
...
CREATE TABLE roles (
    ...
    -- Scope
    scope VARCHAR(20) NOT NULL DEFAULT 'organization',  -- 'system', 'enterprise', 'organization', 'team'
    ...
    -- Constraints
    -- Tier 1 (live ORM) — mirrored on Pydantic ``Role.scope`` typed as RoleScope enum
    CONSTRAINT roles_scope_check CHECK (scope IN ('system', 'enterprise', 'organization', 'team'))
);
...
| --- | --- | --- |
| 3.4 | 2026-05-10 | Cat #10/#11 audit follow-up (Item 1): added live ORM CHECKs missing from §2.2 DDL — `organizations_name_not_empty`, `organizations_slug_not_empty`, `teams_name_not_empty` (all Tier 1, mirrored on Pydantic via field validators / `min_length=1`). Corrected `roles_scope_valid` → `roles_scope_check` (live ORM name, was previously misclassified as Tier 2-only); enum values updated to match the live ORM (added `'enterprise'`). Pydantic now mirrors via the `RoleScope` enum on `Role.scope`. The `enterprises` table itself is not yet documented here — separate follow-up. |
