...
| Per-Evidence DA Failure Tracking | **Implemented** | `milestone_engine._tool_augmented_generate()`: simple per-evidence counters (`da_empty_search_counts`, deep-analysis confidence) track empty searches, confidence, and timeouts. **Within a turn only** — there is no cross-turn DA history. The `EvidenceDAState` structure and the `/sessions/execute` path that held it were removed with the agent orchestration service, and the `da_invocation_count` Evidence field that was to carry the count across turns was never added. |
