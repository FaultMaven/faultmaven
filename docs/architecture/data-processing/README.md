...
**Out-of-band lane (#1329).** After the tenant turn cap is charged (every message pays — owner ruling on #1329) and after typed choices and greetings are resolved, a text-only CONVERSATION message on a non-terminal case with no attachment, no structured intent and no pending gate is asked whether it is incident work at all. `agent_meta` is out of band by construction; `knowledge_query` and the low-confidence `directed_analysis` fall-through (≤ 0.6, no entities) go to a one-token classifier call (`modules/agent/domain/services/out_of_band.py`, 8 s timeout) — unless the message has fewer than four words or continuation vocabulary ("check", "next", "still", "restart"…), which is incident work without asking. The classifier sees the assistant's previous *investigation* message, so a short reply reads as a continuation. A lone `2` means off-topic; anything else — including a failed or timed-out call — is incident work. An out-of-band turn is answered from a ~1.5 KB prompt (synthesis role first, chat chain second, canned fallback last; a cut body is never returned as whole), still advances the message clock, is recorded with `TurnOutcome.OUT_OF_BAND` and its own telemetry path, is excluded from every investigative-turn counter, and is rendered as a one-line aside — "(aside — not part of the investigation)" — in every history fidelity (EARLIER TURNS, the verbatim window and the compact `<previous_turn>`). Orientation turns (greeting / help / empty message, #1343) are recorded and hidden the same way.
