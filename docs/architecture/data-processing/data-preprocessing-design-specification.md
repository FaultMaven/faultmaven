...
- **ssh_session_opened event** — counted when `session opened for user` opens the message
  under an sshd tag (sshd-specific; excludes su, klogind, and other PAM session opens; read by
  `extractors/sshd_auth.py` since fm#1657 — the earlier `sshd[^:]*: session opened` search also
  matched inside a client's login name). Counted in `event_counts`
...
> v5.2 column describes the engine path, which is live, with one exception:
> `da_invocation_count` is **not** persisted on the Evidence model. No such field
...
| 5 | `METRICS_AND_PERFORMANCE` | `MetricsAndPerformanceExtractor` | `statistical` | Auto-detect JSON/CSV/Prometheus format, per-metric stats (min/max/mean/std/p50/p95/p99), IQR-proxy anomaly detection (spikes via p95−p50 spread with a p99×3 extreme-outlier fallback, drops >50% below mean) | **METRICS** |
...
**Cross-turn persistence**: none. `da_invocation_count` existed only to let the
secondary `/sessions/execute` path reconstruct per-evidence DA history for its
`EvidenceDAState` tracking. That path, its service and the field were all removed
in #982. DA failure tracking on the engine path is **in-turn only**, keyed by
`evidence_id` in turn-local dicts; persisting it would need a backing column on
Evidence that nothing else wants yet. `Evidence.vectorized` still persists across
turns — that flag is what stops repeat vectorization, not a call counter.
...
Token counts are computed by `_estimate_tokens(text)` in `vector_storage.py`. Primary path: tiktoken's `cl100k_base` encoding (BPE-family, agrees with BGE-M3's XLM-RoBERTa tokenization within ±15% on typical mixed content). Fallback path (only when tiktoken cannot be imported, or its encoding file cannot be loaded; the encoding is loaded on first use through `utils.token_estimation`'s shared loader, not at import): the `len(text) // 4` heuristic — lossy by ±40% on realistic log content (long hex strings, CSV rows, non-ASCII text), kept solely so the preprocessor continues to function with degraded chunking precision rather than refusing to start. tiktoken is a hard project dependency in `pyproject.toml`, and the Docker image bakes the `cl100k_base` file into `TIKTOKEN_CACHE_DIR`, so a container does not reach the fallback even without network access: the image build fails if the baked file does not load offline.
...
Two mechanical safety nets in `MilestoneEngine`: vectorization with proactive + reactive paths (R4), and context budgeting (R5). Coverage gap detection (R3) is described below but no longer exists.
...
> **#982 change**: `EvidenceDAState` and the persisted cross-turn DA count are gone with `AgentOrchestrationService`. Tracking is in-turn only, in the turn-local dicts shown below.
...
| ~~**Classifier cooperative clarification**~~ | **Done.** On `classification_failed=True`, `_build_classification_clarification` in `InvestigationService` injects DECIDE choices built from the classifier's `suggested_types` + a "Something else" fallback. No LLM call. See §2.5. | Done |
...
- **LogsAndErrorsExtractor** — `file_extract` contains FILE SUMMARY (2–4 sentences) followed by the crime scene excerpt. `search_map` contains the ENTITY PROFILE followed by EVENT TEMPLATE COUNTS. This order guarantees the FILE SUMMARY is always the first thing the LLM reads and survives per-item char truncation.
  ...
  **ENTITY PROFILE structure** (in `search_map`):
  ...
  - **IP auth breakdown** (top 5 IPs with auth events): `IP: failed_password=N, pam_auth_failure=N, invalid_user=N → auth total=N`. Use these event-specific counts for auth questions, not the broader line-occurrence counts below. `auth total` is the number of **attempts** from that IP, counted by outcome line (fm#1627): the sshd lines `Failed <method> for …` / `Accepted <method> for …` for any method except `none` (the client's credential-free method query), one per line; or — where the IP has none — its `pam_auth_failure` lines, since Format B logs (loghub Linux) write no outcome line. Outcomes for a method no category names (`Failed publickey`, `Failed keyboard-interactive/pam`) are shown beside the total as `other_method_outcome=N`. sshd logs one password attempt against an invalid user on three lines carrying the IP (`Invalid user`, the `pam_unix` authentication failure, `Failed password`), so three attempts render as `auth total=3` (9 when the total counted auth lines, 12 when it summed categories). It is **not** the sum of the per-event numbers, which are not mutually exclusive (a single `Failed password for invalid user X from IP` line matches both `failed_password` and `invalid_user`; fm#1596). A row is listed for any IP with an auth line, so a pre-auth probe with only `Invalid user` lines shows `auth total=0` — no authentication outcome and no PAM failure logged. The rendered header says all of this. Every sshd category, and the outcome test, is read from the words sshd **opened its message with** and credited to the address in **sshd's own slot** for that message shape — never searched for anywhere in the line, and never credited to an IPv4 that merely appears on it (fm#1657, `extractors/sshd_auth.py`) — **on a line whose header that module positively reads**: one whose header gives sshd's own tag, or whose message start opens with an sshd event. sshd copies the client's login name verbatim, so a search let `Invalid user Failed password for x from 7.7.7.7 from 1.2.3.4` count a password failure, and crediting every IPv4 credited it to 7.7.7.7 as well. **Every other line is read exactly as before fm#1657** — the same patterns searched anywhere, every IPv4 credited (`LogsAndErrorsExtractor._searched_sshd_events`) — so an export format the module does not read (grep output, CSV/TSV/JSON exports, `log show` columns, anything next) never counts less than before, and keeps that reading's exposure. On a read line, a slot that is ambiguous (client text on both sides of it — a certificate key id, a disconnect reason, two `rhost=` fields) credits no IP; IPv4-mapped slots (`::ffff:203.0.113.9`) are credited as their IPv4. `connection_closed` alone is still a search on every line: it is written mid-message by sshd and every other daemon, and feeds no auth total.
  ...
  - **Ports / PIDs / HTTP paths**: each line: `value: N lines`. Every **entity** count in this profile — IPs, usernames, ports, PIDs, HTTP paths, and the `Top source: X (N lines)` sentence in FILE SUMMARY — is the number of LINES the value appears on; a value named twice on one line counts once (fm#1587). The three **non-entity** tallies in the same profile (Windows Update KB packages, Windows CBS HRESULTs, Apache mod_jk worker states) are per MATCH by design and say `occurrences`, not `lines`. See `docs/architecture/data-processing/entity-registry.md` §*The mention unit*.
