...
- **LogsAndErrorsExtractor** — `file_extract` contains FILE SUMMARY (2–4 sentences) followed by the crime scene excerpt. `search_map` contains the ENTITY PROFILE followed by EVENT TEMPLATE COUNTS. This order guarantees the FILE SUMMARY is always the first thing the LLM reads and survives per-item char truncation.
  ...
  **ENTITY PROFILE structure** (in `search_map`):
  ...
  - **IP auth breakdown** (top 5 IPs with auth events): `IP: failed_password=N, pam_auth_failure=N, invalid_user=N → auth total=N`. Use these event-specific counts for auth questions, not the broader line-occurrence counts below. `auth total` is the number of **attempts** from that IP, counted by outcome line (fm#1627): the lines carrying `failed_password` or `accepted_login`, or — where the IP has none — its `pam_auth_failure` lines, since Format B logs (loghub Linux) write no outcome line. sshd logs one password attempt against an invalid user on three lines carrying the IP (`Invalid user`, the `pam_unix` authentication failure, `Failed password`), so three attempts render as `auth total=3` (9 when the total counted auth lines, 12 when it summed categories). It is **not** the sum of the per-event numbers, which are not mutually exclusive (a single `Failed password for invalid user X from IP` line matches both `failed_password` and `invalid_user`; fm#1596). A row is listed for any IP with an auth line, so a pre-auth probe with only `Invalid user` lines shows `auth total=0`. The rendered header says all of this.
