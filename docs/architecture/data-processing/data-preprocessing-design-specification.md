...
- **LogsAndErrorsExtractor** — `file_extract` contains FILE SUMMARY (2–4 sentences) followed by the crime scene excerpt. `search_map` contains the ENTITY PROFILE followed by EVENT TEMPLATE COUNTS. This order guarantees the FILE SUMMARY is always the first thing the LLM reads and survives per-item char truncation.
  ...
  **ENTITY PROFILE structure** (in `search_map`):
  ...
  - **IP auth breakdown** (top 5 IPs with auth events): `IP: failed_password=N, pam_auth_failure=N, invalid_user=N → auth total=N`. Use these event-specific counts for auth questions, not the broader line-occurrence counts below. `auth total` is the number of LINES carrying at least one auth event for that IP — **not** the sum of the per-event numbers, which are not mutually exclusive (a single `Failed password for invalid user X from IP` line matches both `failed_password` and `invalid_user`; fm#1596). It is an **upper bound on attempts, not an attempt count**: sshd logs one password attempt against an invalid user on three lines carrying the IP (`Invalid user`, the `pam_unix` authentication failure, `Failed password`), so three attempts render as `auth total=9`. The rendered header says so.
