...
| `user` | logs | `alice`, `root` | One rule, in `preprocessing/log_usernames.py`, shared with the logs extractor's entity profile (fm#522). `user=<name>` / `user <name>` on any line; `for [invalid user] <name>` only on a line carrying an SSH/PAM auth keyword — so `Failed password for root from …` **is** a hit (this row previously said it was not) while `installed for high-res timesource` is not. Reverse-DNS hostnames, PAM structural words and tokens that are the next field's name are rejected. Counted once per line here; the entity profile counts every match (fm#1574). |
