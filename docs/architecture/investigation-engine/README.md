...
| Post-Terminal Lifecycle | **Implemented** | Interaction modes (ACTIVE → QUERY-ONLY → FROZEN), session cleanup, auto-generated summaries with skip-if-trivial guardrail. See [Lifecycle Logic §1.7](./investigation-lifecycle-logic.md#17-post-terminal-lifecycle). |
| Terminal Summary Auto-Generation | **Implemented** | `RESOLUTION_SUMMARY` and `CLOSURE_SUMMARY` report types, SYNTHESIS LLM, fire-and-forget, skip-if-trivial guardrail. See [Data Models §1.5.1](./investigation-data-models.md#151-terminal-summary-report-types). |
...
- **Post-terminal lifecycle**: 3 interaction modes (ACTIVE → QUERY-ONLY → FROZEN), auto-generated summaries, knowledge flywheel for RESOLVED cases
