...
| Context budget (R5) | Assembled messages exceed the prompt target plus `tool_observation_max_tokens`, or — when the model's context window is known — messages plus the call's `tools=` definitions exceed that window | Elide whole earlier tool-call groups, oldest first, leaving a marker telling the agent to re-run a search | Mechanical: token estimate per message, and for the tools payload against the window only |
