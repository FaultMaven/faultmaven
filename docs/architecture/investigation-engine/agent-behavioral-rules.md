...
| Context budget (R5) | Assembled messages plus the call's `tools=` definitions exceed the resolved tool-loop token budget | Elide whole earlier tool-call groups, oldest first, leaving a marker telling the agent to re-run a search | Mechanical: token estimate per message and for the tools payload, against the prompt target plus `tool_observation_max_tokens`, clamped by the model's context window |
