...
**Out-of-band turns (#1329)**: the same "not about the incident" judgement is generalised in `InvestigationService.process_turn`. A text-only message that is small talk, trivia, a creative request — or an `agent_meta` question — is routed AROUND the engine: the daily tenant turn is not charged, the reply comes from a small prompt on the synthesis role (`modules/agent/domain/services/out_of_band.py`; the `agent_meta` variant carries `ABOUT_FAULTMAVEN_PROFILE`), the message clock still advances (every persisted exchange does — #500/#1264), and the turn is recorded with `TurnOutcome.OUT_OF_BAND`, excluded from every investigative-turn count and rendered in EARLIER TURNS as "(off-topic exchange — not part of the investigation)". Off-topic detection for the open-ended remainder is a one-token classifier call shown the assistant's previous message; every uncertain or failed verdict is incident work. The in-prompt Rule 9 layers above remain the path for a self-referential question that arrives with an attachment or a structured intent.
