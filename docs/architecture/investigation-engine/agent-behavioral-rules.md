...
**Out-of-band turns (#1329)**: the same "not about the incident" judgement is generalised in `InvestigationService.process_turn`. A text-only message that is small talk, trivia, a creative request — or an `agent_meta` question — is routed AROUND the engine: the daily tenant turn is still charged (owner ruling on #1329 — the cap bounds compute), the reply comes from a small prompt on the synthesis role (`modules/agent/domain/services/out_of_band.py`; the `agent_meta` variant carries `ABOUT_FAULTMAVEN_PROFILE`), the message clock still advances (every persisted exchange does — #500/#1264), and the turn is recorded with `TurnOutcome.OUT_OF_BAND`, excluded from every investigative-turn count and rendered as "(aside — not part of the investigation)" in every history fidelity. Off-topic detection for the open-ended remainder is a one-token classifier call (short timeout) shown the assistant's previous investigation message; every uncertain or failed verdict is incident work. The cheap gates before it read CONTINUITY and never subject matter: a short reply, or continuation vocabulary (`tried`, `check`, `next`, `still`), means the user is on what was just being discussed. Incident NOUNS (`error`, `log`, `config`, `latency`) were removed from that list — they guess at a message's subject from one word, which is the classifier's question, and gating on them investigated a retirement-account question as an incident. Because a continuity word presupposes a previous turn, those gates apply **only on a case that already has investigation history** (`has_investigation_history`, derived from the message clock minus out-of-band turns); until an investigation turn has run there is no status quo to preserve. The same predicate picks the prompt framing and the chips, so a case with no history is never told it is mid-investigation, never offered a way back to one, and — like the orientation chips (see above) — never has a placeholder title quoted as its subject. Terminal cases keep their own Q&A path. The in-prompt Rule 9 layers above remain the path for a self-referential question that arrives with an attachment or a structured intent.
...
| Context budget (R5) | Assembled messages exceed the prompt target plus `tool_observation_max_tokens`, or — when the model's context window is known — messages plus the call's `tools=` definitions exceed what that window leaves beside the call's completion | Elide whole earlier tool-call groups, oldest first, leaving a marker telling the agent to re-run a search | Mechanical: token estimate per message, and for the tools payload against the window only |
