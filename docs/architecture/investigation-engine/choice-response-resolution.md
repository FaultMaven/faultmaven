...
- `last_suggestions` is rewritten **only** on `process_turn`'s success path, in two different ways. A **non-turn writer** — the standalone close/transition endpoints, `PATCH /evidence/{id}/classification` — moves the case without touching the list at all; the stamp handles that one, since the clock keeps moving and the entry ages out. A **mid-turn engine save** that is never followed by the final one commits turn *N*'s state beside turn *N-1*'s suggestions; the stamp does **not** handle that one (see below). Either way, "it is in the row" is not evidence that a turn put it there *for now*.
