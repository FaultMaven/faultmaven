...
These were the limitations that motivated consolidating the handling into a single engine-computed status. §3.5 addresses 2 and 3; 1 is narrowed but not fully closed.

1. **Two unreconciled layers (a composition seam)** — *narrowed, not fully closed.* Stall/`EXHAUSTED` detection reads the hypothesis layer; grounding and `cause_state` read the causal-graph layer. The unified read now sources both readers' shared dimensions (stall thresholds; category/evidence breadth) from one neutral `exhaustion_thresholds` module so they cannot drift, and every new consumer reads the single `assess_verification_status` join. The residual seam is that a few older consumers still branch on one axis directly (a bounded consumer-migration task); until they migrate, the *dynamic drift* risk is reduced but not eliminated.
