...
### 3.5 The unified read, the (flag-gated) code-guarded handoff, and the persisted status

Most of the §5 direction is now built. The unified read, its persistence, the
terminal capture-on-close, and the surfacing are always-on; only the *proactive
structured handoff* sits behind a flag:
...
3. **The load-bearing behavior is Prompt-only *when the flag is off*.** With `enable_insufficient_evidence_handoff` off, the boundary statement and handoff are still the Prompt-only §3.1–§3.4 path (limitation 2 above applies); turning the flag on promotes them to the code-guarded handoff of §3.5. The flag stays off until the calibration eval validates firing precision — so on a weak model, the *proactive* boundary statement can still be skipped until then. (The first-class status itself is no longer missing: `verification_status` is now a persisted assessment variable, `closed_insufficient_evidence` is a recorded closure reason, and both are evaluable — the earlier "no persisted/queryable/evaluable status" limitation is closed by §3.5.)
