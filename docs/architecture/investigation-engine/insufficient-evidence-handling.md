...
- [Two-Dimensional Hypothesis Methodology](./two-dimensional-hypothesis-methodology.md) — the hypothesis/causal-graph distinction, the two grades of root-cause validation, and the causal-graph-layer assurance grade (§9.5: `grade_cause_assurance` → `GROUNDED`)
...
- **`grade_cause_assurance` → `GROUNDED` / `FALLBACK_ONLY` / `NO_ROOT`** — the harvest-gate grade (see [Two-Dimensional Hypothesis Methodology §9.5](./two-dimensional-hypothesis-methodology.md)). `NO_ROOT` means "no validated root," but does not distinguish "still working" from "cannot be grounded from available data."
