...
| **Candidate / residual candidate** | A *candidate* (root-cause claim) is a hypothesis (`case.hypotheses`). Runbook-sourced causes enter the **same** hypothesis space — the LLM generates them from retrieved runbook context (RAG) — so "candidate = hypothesis" covers both LLM- and runbook-sourced causes; there is no separate candidate representation to reconcile. A candidate's *discriminators* are the `causal_verification` `EvidenceNeed`s motivated by it. *Residual* candidates are those still in play: hypothesis state **not** `REFUTED` or `RETIRED`. The declared-data-wall stall (§5.3) and the close record (§5.4) range over the residual candidates. |
...
- **One computed read.** `assess_verification_status(case)` (`verification_status.py`) computes the verification-status join of §5.1 — grounding (an authority-grounded root, `grade_cause_assurance == CONFIRMED` — the top M2 grade, the direct successor of the pre-M2-alignment `GROUNDED` as the axis value, **anchored on a verified symptom** — see the grounding-anchor bullet below) × progress (stalled?) — as a single value (`HEALTHY` / `TREATMENT_BLOCKED` / `OPEN` / `NOT_YET_PRODUCTIVE` / `INSUFFICIENT_EVIDENCE`), reading the two existing layers rather than adding a third signal. The work gate (§5.2) and the stall thresholds are the **decomposed** `EXHAUSTED` conditions; both readers source them from **one** neutral, dependency-free module (`exhaustion_thresholds.py`) so the dimensions they measure *identically* (the stall thresholds; category/evidence breadth) cannot drift — the first concrete narrowing of the §4.1 composition seam. The hypothesis dimension is deliberately *not* shared: the work gate counts generation depth (hypotheses that exist) while `EXHAUSTED` counts elimination depth (hypotheses refuted), so those keep separate constants.
...
- **Open calibration question — should the grounding axis read "any validated root" instead of the top grade?** With the M2 grades, `CONFIRMED` (counterfactual) typically arrives only at resolution time, so a case with a *mechanistically identified* cause that stalls on fix execution disposes to `INSUFFICIENT_EVIDENCE` rather than `TREATMENT_BLOCKED` — even though `TREATMENT_BLOCKED`'s own description ("have a cause but can't reach a verified fix") fits it. Mapping the axis to `grade ≥ MECHANISTIC` would route such cases to `TREATMENT_BLOCKED` and keep the insufficient-evidence handoff for genuinely ungrounded cases. Sharper still: because the confirm-stamp fires only at RESOLVED execution (after the last recompute), `CONFIRMED` is effectively never true on a live INVESTIGATING turn — `HEALTHY` and `TREATMENT_BLOCKED` are engine-unreachable in-flight, and a stalled case holding a validated root disposes `INSUFFICIENT_EVIDENCE` (whose consumers — the flag-gated handoff, `closed_insufficient_evidence`, the Data Boundary report block — would then assert no cause could be grounded over a case that holds one). This reachability collapse is the strongest argument for the `≥ MECHANISTIC` axis; it still belongs to the calibration eval, not the grade alignment. This was deliberately **not** changed with the grade alignment (which mapped the axis to the top grade, preserving the prior semantics for the common empirical shape): it changes the join's behavior for the most common case shape, so it belongs to the §5.5 calibration eval that must run before the handoff flag flips on.
...
The two correctives are **mutually exclusive by construction** (0 hypotheses vs the ≥2 work gate) and both sit LAST in `engine_owned_affordances`, below the state-machine gates (disposition, gate1). Firing precision is the same discipline as §5.5: a case one axis off the vacuum (still progressing, or holding a hypothesis, or grounded, or not INVESTIGATING) yields None, not a pull-back. And the same remediation lever applies — if the time-arm proves noisy on real trajectories, raise the shared `exhaustion_thresholds` integers; do not re-gate the corrective off (that restores the silent spin).
