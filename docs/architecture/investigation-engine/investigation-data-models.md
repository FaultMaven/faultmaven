...
If eligible, `ConversionService.convert_from_case()` runs as a fire-and-forget background task. The draft appears in the Dashboard under Knowledge Base > Drafts.
...
| `ACTIVE` → `RETIRED` | likelihood < 0.30 when a likelihood update lands (evidence link or model update) | **System-automated** (`hypothesis_manager.py`, `_check_state_transition`) |
| `ACTIVE` → `RETIRED` | Anchoring prevention — fixation: 4+ hypotheses in one category, 2+ stalled for 3+ iterations, or the top hypothesis stalled for 3+ iterations below 0.70 | **System-automated** (`hypothesis_manager.py`, `force_alternative_generation`: retires every flagged hypothesis stalled for 2+ iterations, except one whose chain root is validated or count-held; stands down while recently requested evidence is outstanding, with a cooldown) |
| `ACTIVE` → `RETIRED` | Age-out — no standing causal support, stagnant for 3+ iterations, likelihood < 0.30 | **System-automated** (`hypothesis_manager.py`, `retire_if_aged_out`, from housekeeping; the same stand-down and root protections as anti-anchoring) |
...
The system-automated `INCONCLUSIVE` and low-confidence `RETIRED` transitions run after each likelihood update (`update_likelihood_from_evidence`, `update_hypothesis_likelihood`). Stagnation decay does not run them: a hypothesis decayed below 0.30 stays ACTIVE until an update lands, and age alone retires only through the guarded paths — anti-anchoring, and the age-out once the full stagnation horizon has passed. Wiring the threshold check into decay would retire a 0.30 prior on its first decay step, bypassing those paths' horizon, stand-down and root-protection guards. They are mechanically applied thresholds, not LLM judgment calls — they prevent stale low-confidence hypotheses from consuming LLM context across turns when no new evidence is advancing them. The user-intent paths apply **before** LLM processing on the same turn so the agent's reply sees the updated hypothesis and can acknowledge it (refute / retire apply the state change directly; validate records a strong prior — `likelihood = 1.0` — since VALIDATED is derived solely from the chain root) — see [Choice-Response Resolution §5.2](./choice-response-resolution.md#52-state-transitions-applied-by-the-engine).
