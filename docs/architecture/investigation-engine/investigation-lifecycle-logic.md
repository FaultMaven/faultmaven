...
- **INV-07** schema permissiveness: `InquiryStateUpdate` uses Pydantic's default `extra='ignore'`. An LLM emitting `evidence_to_add` (or any other invalid field) in an INQUIRY response has the field silently dropped, not rejected. The invariant holds (no Evidence row created — the engine has no creation branch either), but a schema violation by the LLM does not surface as an error. Hardening to `extra='forbid'` would convert LLM training drift into a visible failure rather than a quiet drop — open design question; flagged for awareness.
- **INV-09** terminology drift: this row's *Enforcement* column refers to `require_case_not_terminal()` as "middleware", but it's a plain helper function in `routes.py:519`, used at exactly one call site (`routes.py:987`, the case-update endpoint). Other write endpoints (`routes.py:1173`, `2142`, `2781`) check `case.is_terminal` inline. The protection is consistent in intent across endpoints, but the mechanism varies. Matrix wording should change "middleware" → "helper function (used at one site; other endpoints inline the check)". A future consolidation pass could centralize this via a FastAPI dependency-injection layer.
...
- **INV-15** scan scope is narrower than the prompt rule: the matrix's "light vocabulary check" refers to the `_completion_phrases` scan at `milestone_engine.py:2602`, which only catches transition-completion claims (*"case closed"*, *"marking as resolved"*, etc.). The prompt's `_ADVISOR_ROLE_CONSTRAINT` also bans *"Let me check"*, *"I will run"*, *"Let me look at"*, *"I'll execute"* — none of which the runtime scan covers. The runtime check is a **narrower** backstop than the prompt rule it claims to support. Broadening the scan would catch LLM drift on the advisor-role boundary more comprehensively; open design question.
