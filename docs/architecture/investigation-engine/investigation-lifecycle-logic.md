...
|-------------|-------------|-------------------|
| RESOLVED | `RESOLUTION_SUMMARY` | Problem Statement, Root Cause (from validated hypotheses), Causal Map (gated), Solution Applied (the executed fix — see below), Confirming Evidence, Timeline, Milestones Reached, Mitigation (if any) |
