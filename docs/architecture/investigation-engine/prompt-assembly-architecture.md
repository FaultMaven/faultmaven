...
The **conversation section truncates the other way** (`keep="tail"` — its most-recent turns are at the end), so what it loses is the *opening* delimiter: a close with nothing open before it reads as though every earlier section of the prompt were inside the quoted transcript. `reseal()` handles both directions — prepending the opening delimiter when only the close survived, appending the close (plus a terminator if needed) when only the open did, and dropping the block when neither survived intact (#1256).

**No section may end part-way through a tag.** Forgery is settled by authorship; absorption is settled by the bytes alone, so an *unfenced* section that ends mid-tag swallows whatever follows it — and after #1256 what follows it may be `<conversation_history fence="…">` or `<user_message fence="…">`, handing the half-written tag the live token. `<system_feedback>` is the measured instance: it is the one unfenced section that does not wrap itself in a tag, and it renders immediately before `{user_message}`. `_allocate_sections` therefore runs `fence.terminate_dangling()` over **every** section it emits, not over the ones that are adjacent today — a section can render empty, which promotes the one before it to adjacent. Renderer-owned bytes only; the section's own are untouched, and clean sections pay nothing.
