...
| **4b-2** | **Make matching fire (resolvers) + cost guard** | Wire the matcher's inputs so a match can actually happen: a `case_evidence_qa` callable (T2 — prose `AnswerFromCaseEvidence` + an LLM yes/no classify; expensive, ~2 LLM calls/rung) and, if/when per-runbook-step output tracking exists, the T1 `step_output_resolver`. Add a per-turn skip-guard (cause already IDENTIFIED, or matcher already matched this case) and batch `get_runbook_causes` so an enabled matcher doesn't run KB retrieval + N DB sessions every turn. | Pending |
