...
model as retrieved prose and through the KB QA tools, and only the engine's
unasked assertion of their causes is withheld.
...
than argued. Eight counters in `lifecycle_metrics.py`:

| Counter | Reads as |
|---|---|
| `faultmaven_kb_cause_seed_attempt_total{outcome}` | One increment per seeding **attempt** (retrieval returned hits, flag on), labeled `seeded` / `all_causes_skipped` / `no_cause_chunk_matched` / `no_corroborated_runbook` / `not_lexically_grounded` / `no_seedable_cause` / `crashed`. The labels are mutually exclusive and sum to attempts, so `seeded`/total is the seeding **yield** and its complement the zero-seed rate. |
...
| `faultmaven_kb_cause_seed_ungrounded_total` | Retrieved chunks were excluded because the query did not name their runbook (#1272, #1285). One increment per **affected attempt**, never per chunk. **Not an alarm and no value is healthy** — it is the grounding gate's *cost*, and the basis for re-sizing what grounds a seed. Read it beside `not_lexically_grounded` (every chunk turned away) and beside the counter below, because a gate that has stopped applying reports nothing in either. |
| `faultmaven_kb_cause_seed_grounding_unmeasured_total` | An attempt carried at least one chunk with **no lexical grounding evidence at all**, so the gate did not apply to it (#1285). Reachable when a KB search reaches the seeder without a reranker — the pure-vector path — which `KnowledgeService` warns about from the producing side. **Zero is the expected state**; sustained nonzero means the gate is off for those searches and the two counters above are measuring less than they appear to. |
...
pasted, at the same turn in both arms or one turn earlier ON; seeds never
shortened the path. Seeds are not inert — seeded rung needs surfaced to the user
in 4 of 5 seeded cases — and in the one case where a seeded root validated
(kafka) it validated on a literally-true, non-causal indicator (3 members, 6
partitions) and became the recorded conclusion while the true root (a dropped
database index) went unexplored. That is the fm#1144 failure mode, reproduced
...
the measurement can be repeated. The push-seeding path, its provenance readers,
metrics and the cause-record extraction are now inert in production and are
removed in a follow-on (fm#1295 step 4b); the remediation channel that carries
the corrective pattern to the model (the `root_cause` prefetch and the KB QA
tools) is unaffected and pinned by
