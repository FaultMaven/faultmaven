...
so a retrieved Cause reads as one self-contained cause→fix unit when the model
...
| **Diagnostic Steps** | The agent proposes these commands to the user during DIAGNOSIS; their findings become case evidence the agent reasons over. | **Procedure only — no interpretation.** Command, expected output shape, nothing else. What a finding *means* belongs in the cause `Statement`, with per-`Step` notes optionally mirrored in `Indicators`. Keeping interpretation out of the Steps chunk avoids duplicating it across the Diagnostic-Step and Cause chunks (which dilutes retrieval signal). **Name the locus where it is not derivable** — when a step's target (account, project, cluster, namespace, region, replica) depends on the reader's topology rather than on the object's name, say which one and how to determine it. A step that silently assumes one topology sends every reader with the other one to the wrong place. |
...
| Unstated locus on a topology-dependent step | A step names a resource whose account/project/cluster/namespace/region placement follows from the reader's **layout**, not from the object's name, and the example silently fixes one layout. The reader queries the wrong place, finds nothing, and concludes the object is healthy — a false exoneration, which is worse than no guidance. Placeholders make it invisible: an example ARN reads as illustrative while carrying a load-bearing account id. | State the **deciding rule**, not a constant — "X lives wherever Y is", plus how to read Y off an earlier Step. Where a rule exists, derive the value in the command rather than hard-coding it (`eks-cluster-authentication` Step 8 derives the OIDC issuer from the cluster and matches it against the registered providers, asserting no account). A constant is only safe when the placement is definitional (a role's trust policy is always in that role's account). |
