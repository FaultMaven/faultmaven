...
`_admit_diverse` now takes up to `KB_CONTEXT_MAX_ENTRIES` (5) hits, at most `KB_CONTEXT_MAX_PER_RUNBOOK` (2) from any one runbook, then **backfills** unused slots from the hits the cap skipped, in rank order. Rank is never reordered — hits are only skipped. Coverage measured **17/23 → 21/23, zero regressions**.
...
Applied to the pool already fetched: no extra query, embedding or round trip. What it does *not* settle is whether breadth beats depth for diagnosis — coverage counts correct runbooks reaching the model, not answer quality; that needs an end-to-end investigation eval.
