...
**Which keywords, and in what order — the whole decision.** Only the first three survive into probes. With a corpus term index available the order is: terms the index has SEEN first, rarest of those first, and terms it has not seen last — ranked down, never dropped. The index covers only the global tier, so a document frequency of zero means either "unmatchable anywhere" or "present only in the caller's own personal or team runbooks", and the probes carry the CALLER's scope filter rather than the index's. Dropping them (as an earlier cut did, when the index still spanned every tier) would have made a personal runbook's own identifiers permanently unprobeable. On `qemu cannot write its PID file` the shape-based order spends its slots on `qemu` (0 chunks), `cannot` (241) and `write` (98); this order spends them on `PID`, `write` and `file`, and that is what puts the right runbook back in the candidate set. Without an index it falls back to the shape heuristics (error codes matching `ERR-\d+`, CamelCase names like `CrashLoopBackOff`, dotted names like `java.lang.OutOfMemoryError`).
...
`_admit_diverse` now takes up to `KB_CONTEXT_MAX_ENTRIES` (5) hits, at most `KB_CONTEXT_MAX_PER_RUNBOOK` (2) from any one runbook, then **backfills** unused slots from the hits the cap skipped, and returns the result **in rank order**. Coverage measured **17/23 → 21/23, zero regressions**.

Rank order is load-bearing, not cosmetic: this section is head-truncated under budget pressure ("rank-ordered best-first, so `keep='head'`"), the prompt numbers entries `MATCH 1..n` implying descending relevance, and the pre-fetch log prints `score=` in list order. `KB_CONTEXT_MAX_PER_RUNBOOK` is a **preference, not a bound** — the backfill deliberately exceeds it rather than hand back budget, so it bounds how far one runbook can crowd out *another*, not how much of one runbook can appear.

Each entry now also carries the matched chunk's own heading (`section`), because two chunks of one runbook share its title and would otherwise render as indistinguishable `MATCH n` blocks — readable by the model as two corroborating sources rather than two parts of one document.

Both levers matter: widening alone reaches only 19/23 (the extra slots go to more causes of the same runbook), and capping alone collapses depth on the best-matching runbook. The backfill is what keeps the cap free — without it a query whose answer genuinely lives in one runbook would render fewer entries than the budget allows, which is 5 of the 16 labelled queries. Because a hit is deferred only once its parent has filled its share, backfill can never introduce or remove a runbook, so it cannot affect coverage — it only decides how leftover budget is spent.

Applied to the pool already fetched: no extra query, embedding or round trip. The prompt cost is two extra entries of title + snippet (`KB_MAX_SOLUTION_CHARS` does not apply — a pre-fetch entry carries no `solution` key, so that truncation is dead code for this channel). What it does *not* settle is whether breadth beats depth for diagnosis — coverage counts correct runbooks reaching the model, not answer quality; that needs an end-to-end investigation eval.
