> **ARCHIVED 2026-09-02.** The harness this README describes was deleted with
> the KB cause seeder (fm#1295 step 4b): every command and path below refers
> to the tree at commit `6b3592bf5` and none of it exists on main. The
> recorded runs beside this file are the evidence record; the design doc is
> [`../kb-cause-seeder-design.md`](../kb-cause-seeder-design.md). The two
> labelled corpora the harness measured retrieval with live on at
> `tests/eval/kb_retrieval/`.
...
[`seeder-on-vs-off-ab-local.md`](seeder-on-vs-off-ab-local.md)
measured whether they *help* and turned the default off (fm#1295). Design + gate
definition:
[`../kb-cause-seeder-design.md`](../kb-cause-seeder-design.md).
...
provider) — see
[`enabling-gate-run-fireworks-deepseek-v4-flash.md`](enabling-gate-run-fireworks-deepseek-v4-flash.md):
