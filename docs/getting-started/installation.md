...
| `JWT_ACCESS_TOKEN_EXPIRY_MINUTES` | `15` | Access token lifetime in **minutes** (1–1440) |
| `JWT_REFRESH_TOKEN_EXPIRY_DAYS` | `7` | Refresh token lifetime in **days**, not minutes (1–90) |

> **These two names apply to both auth modes** — Standalone (local, HS256) and
> Cloud (OAuth, RS256) alike. They are the single source for token lifetimes;
> see `.env.example`. Out-of-range values are rejected at startup rather than
> accepted silently. The earlier `JWT_ACCESS_TOKEN_EXPIRE_MINUTES` /
> `JWT_REFRESH_TOKEN_EXPIRE_DAYS` spelling (**EXPIRE**, not **EXPIRY**) is
> retired: setting either now fails startup with an error naming its
> replacement, so an environment carrying the old name cannot quietly run on
> default lifetimes.
