# FaultMaven Agentic Framework - Production Implementation Guide

## Overview

This guide documents the completed integration of the 7-component agentic framework into FaultMaven. The agentic implementation is now production-ready and fully operational, with all previous components replaced.

## API Compatibility and Deprecation

- **Deprecated endpoints**: Previous `/api/v1/agent/*` endpoints have been removed from the live API specification and should no longer be used. The live spec in `FaultMaven/docs/api/openapi.live.json` has been updated and a backup was preserved.
...
- **Canonical usage**: integrations should use the case-scoped endpoints (for example, create a case with `POST /api/v1/cases` and submit queries with `POST /api/v1/cases/{case_id}/queries`) instead of the previous agent endpoints.
