...
- `faultmaven_cause_identification_held_mece_total` — cause identification was HELD at CANDIDATES because ≥2 simultaneously-validated DISTINCT standing roots stood unarbitrated (§7.1.2): the recompute where `cause_state` would otherwise have read IDENTIFIED. One increment per transition INTO the hold (edge-triggered on the persisted `cause_identification_contested` flag), never per turn while the contest stands. Duplicate emissions and same-causal-line roots do not count (they collapse to one cause); a counterfactually confirmed root settles the contest without an event.
