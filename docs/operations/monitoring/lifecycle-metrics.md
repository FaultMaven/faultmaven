...
- `faultmaven_m6_demotion_refused_total{reason}` — an M6 counterfactual demotion was refused because its preconditions could not be established. `reason` is `no_fix_applied` (no accepted actionable `ProposedAction` and no compliance gate), `no_persistence` (a fix was applied but no `SYMPTOM_EVIDENCE` at/after it observes the problem still present), or `resolution_confirmed` (a qualifying gone⇒gone row stands at/after the fix turn — the problem demonstrably did NOT persist).
...
- `faultmaven_hypothesis_dedup_skipped_total` — an LLM-emitted `hypotheses_to_add` item was skipped as a duplicate of a standing (non-terminal) or same-batch hypothesis (`hypothesis_statements_duplicate` — a fail-open 0.8 mutual mirror with a negation-polarity guard, stricter than the §7.1.2 fold, §7.8). Protects the ≥2-active work gate — the axis that separates `INSUFFICIENT_EVIDENCE` from `NOT_YET_PRODUCTIVE` — from duplicate inflation. One increment per skipped item.
