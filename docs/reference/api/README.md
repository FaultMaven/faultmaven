...
- `investigation_turn` (object, optional) — How many of the case's turns so far were investigation work. turn_number is the message clock and advances on every exchange; this excludes out-of-band turns (small talk, trivia, questions about FaultMaven itself), which are answered but neither counted nor charged against the tenant's daily turn cap (#1329). Clients that display a turn counter should prefer this.
- `milestones_completed` (array, required)
