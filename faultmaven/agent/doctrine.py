...
class TroubleshootingDoctrine:
    ...
    def __init__(self):
        ...
        self.phase_guidance = {
            Phase.DEFINE_BLAST_RADIUS: {
                ...
                "outputs": [
                    "affected_systems", "impact_assessment", 
                    "severity_level", "business_impact"
                ],
                "success_criteria": [
                 ...
                 "outputs": [
                     "timeline_of_events", "recent_changes", 
            ...
            Phase.FORMULATE_HYPOTHESIS: {
                ...
                "success_criteria": ["Multiple testable hypotheses", "Evidence-based ranking", "Clear testing approach"]
            },
            Phase.VALIDATE_HYPOTHESIS: {
                ...
                "tools_needed": ["knowledge_base_search"],
                "outputs": ["validated_hypothesis", "test_results", "root_cause_confirmation"],
                "success_criteria": ["Hypothesis validated with evidence", "Root cause identified", "Reproduction steps if applicable"]
            },
            Phase.PROPOSE_SOLUTION: {
                ...
                "tools_needed": ["knowledge_base_search"],
                "outputs": ["solution_options", "implementation_plan", "risk_assessment", "prevention_measures"],
                "success_criteria": ["Comprehensive solution plan", "Risk mitigation strategy", "Prevention measures defined"]
            }
        }
    ...
    def _create_phase_prompt(self, phase: Phase, guidance: Dict[str, Any], agent_state: AgentState, context: Dict[str, Any]) -> str:
        """
        ...
        if context.get('uploaded_data'):
            ...
            for data in context['uploaded_data']:
                prompt_parts.append(f"  - {data.get('data_type', 'unknown')}: {data.get('file_name', 'unnamed')}")
        
        prompt_parts.append("")
        prompt_parts.append("**Instructions:**")
        prompt_parts.append("Follow the 'Single Insight, Single Question' rule - present one key finding and ask one clear question.")
        prompt_parts.append("Keep responses concise and focused for productive turn-based dialogue.")
        prompt_parts.append("Proceed systematically using the available tools and following the guidance above.")
        
        return "\n".join(prompt_parts)
    
    async def _execute_blast_radius_phase(self, prompt: str, agent_state: AgentState, context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            key_insight = parsed_output.get('key_insight', "Blast radius assessment completed")
            follow_up_question = parsed_output.get('follow_up_question', "Should we proceed to establish the timeline?")
            ...
            if parsed_output.get('tool_to_use') == 'knowledge_base_search':
                ...
                if kb_tool:
                    tool_query = parsed_output.get('tool_query', f"blast radius impact assessment {agent_state.get('user_query', '')}")
                    tool_results = kb_tool.search(tool_query, context={'phase': 'define_blast_radius'})
                    findings.append(f"Knowledge base search completed: {len(tool_results.split('**')) // 2} relevant documents found")
                    ...
                    if len(tool_results.split('**')) < 3:  # Minimal useful results
                        ...
                        if web_search_tool and web_search_tool.is_available():
                            ...
                            findings.append("Web search performed for additional blast radius information")
    ...
    async def _execute_timeline_phase(self, prompt: str, agent_state: AgentState, context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # 2. Parse the structured response from the LLM
            parsed_output = self._parse_llm_response(llm_response.content)
            key_insight = parsed_output.get('key_insight', "Timeline analysis completed")
            follow_up_question = parsed_output.get('follow_up_question', "Should we proceed to formulate hypotheses?")
            ...
            if parsed_output.get('tool_to_use') == 'knowledge_base_search':
                kb_tool = context.get('knowledge_base_tool')
                if kb_tool:
                    tool_query = parsed_output.get('tool_query', f"timeline analysis recent changes deployment {agent_state.get('user_query', '')}")
                    tool_results = kb_tool.search(tool_query, context={'phase': 'establish_timeline'})
                    findings.append(f"Knowledge base search for timeline patterns completed")
                    ...
                    if len(tool_results.split('**')) < 3:  # Minimal useful results
                        ...
                        if web_search_tool and web_search_tool.is_available():
                            ...
                            findings.append("Web search performed for timeline analysis information")
            ...
            if context.get('uploaded_data'):
                for data in context['uploaded_data']:
                    if data.get('data_type') == 'log_file':
                        ...
                        findings.append("Metrics data shows performance trends over time")
    ...
    async def _execute_hypothesis_phase(self, prompt: str, agent_state: AgentState, context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # 2. Parse the structured response from the LLM
            parsed_output = self._parse_llm_response(llm_response.content)
            key_insight = parsed_output.get('key_insight', "Hypotheses formulated based on available evidence")
            follow_up_question = parsed_output.get('follow_up_question', "Which hypothesis should we validate first?")
            ...
            if parsed_output.get('tool_to_use') == 'knowledge_base_search':
                kb_tool = context.get('knowledge_base_tool')
                if kb_tool:
                    tool_query = parsed_output.get('tool_query', f"root cause hypothesis similar issues {agent_state.get('user_query', '')}")
                    tool_results = kb_tool.search(tool_query, context={'phase': 'formulate_hypothesis'})
                    findings.append("Knowledge base search for similar issue patterns completed")
                    hypothesis_list.append("Pattern-matched hypothesis from knowledge base")
                    ...
                    if len(tool_results.split('**')) < 3:  # Minimal useful results
                        ...
                        if web_search_tool and web_search_tool.is_available():
                            ...
                            findings.append("Web search performed for hypothesis formulation")
                            hypothesis_list.append("Web-sourced hypothesis from external documentation")
            ...
            if previous_findings:
                ...
                for finding in previous_findings[-3:]:
                    ...
                    if 'error' in finding_text and 'Error-based failure hypothesis' not in hypothesis_list:
                        hypothesis_list.append("Error-based failure hypothesis")
                    elif 'performance' in finding_text and 'Performance degradation hypothesis' not in hypothesis_list:
                        hypothesis_list.append("Performance degradation hypothesis")
                    elif 'connection' in finding_text and 'Connectivity issue hypothesis' not in hypothesis_list:
    ...
    async def _execute_validation_phase(self, prompt: str, agent_state: AgentState, context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # 2. Parse the structured response from the LLM
            parsed_output = self._parse_llm_response(llm_response.content)
            key_insight = parsed_output.get('key_insight', "Hypothesis validation completed")
            follow_up_question = parsed_output.get('follow_up_question', "Does this match your observations?")
            ...
            if parsed_output.get('tool_to_use') == 'knowledge_base_search':
                kb_tool = context.get('knowledge_base_tool')
                if kb_tool:
                    tool_query = parsed_output.get('tool_query', f"validation testing procedures {agent_state.get('user_query', '')}")
                    tool_results = kb_tool.search(tool_query, context={'phase': 'validate_hypothesis'})
                    findings.append("Knowledge base search for validation approaches completed")
                    ...
                    if len(tool_results.split('**')) < 3:  # Minimal useful results
                        ...
                        if web_search_tool and web_search_tool.is_available():
                            ...
                            findings.append("Web search performed for validation approaches")
            
            # Look for hypotheses from previous phase
            investigation_context = agent_state.get('investigation_context', {})
            hypothesis_results = investigation_context.get('formulate_hypothesis_results', {})
            ...
            if hypothesis_list:
                ...
                validated_hypothesis = parsed_output.get('validated_hypothesis', hypothesis_list[0])
    ...
    async def _execute_solution_phase(self, prompt: str, agent_state: AgentState, context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # 2. Parse the structured response from the LLM
            parsed_output = self._parse_llm_response(llm_response.content)
            key_insight = parsed_output.get('key_insight', "Solution proposal completed")
            follow_up_question = parsed_output.get('follow_up_question', "Which solution approach would you like to implement?")
            ...
            if parsed_output.get('tool_to_use') == 'knowledge_base_search':
                kb_tool = context.get('knowledge_base_tool')
                if kb_tool:
                    tool_query = parsed_output.get('tool_query', f"solution fix resolution {agent_state.get('user_query', '')}")
                    tool_results = kb_tool.search(tool_query, context={'phase': 'propose_solution'})
                    findings.append("Knowledge base search for solution patterns completed")
                    solution_options.append("Apply documented solution from knowledge base")
                    ...
                    if len(tool_results.split('**')) < 3:  # Minimal useful results
                        ...
                        if web_search_tool and web_search_tool.is_available():
                            ...
                            findings.append("Web search performed for solution approaches")
                            solution_options.append("Apply web-sourced solution from external documentation")
            ...
            investigation_context = agent_state.get('investigation_context', {})
            validation_results = investigation_context.get('validate_hypothesis_results', {})
            ...
            if validated_hypothesis:
                ...
                if f"Address root cause: {validated_hypothesis}" not in solution_options:
    ...
    def _parse_llm_response(self, content: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            if any(indicator in line.lower() for indicator in ['key insight:', 'insight:', 'finding:']):
                parsed['key_insight'] = line.split(':', 1)[-1].strip()
            
            # Look for follow-up questions
            elif '?' in line and any(indicator in line.lower() for indicator in ['question:', 'next:', 'should']):
            ...
            elif 'hypothesis' in line.lower() and ('1.' in line or '-' in line or 'likely' in line.lower()):
            ...
            elif 'solution' in line.lower() and ('1.' in line or '-' in line or 'option' in line.lower()):
    ...
    def _fallback_blast_radius_analysis(self, agent_state: AgentState, context: Dict[str, Any]) -> Dict[str, Any]:
        """Fallback blast radius analysis without LLM"""
        findings = []
        key_insight = "Need to assess the full scope of impact"
        follow_up_question = "Can you provide more details about which systems or users are affected?"
        ...
        if context.get('uploaded_data'):
            for data in context['uploaded_data']:
                if data.get('data_type') == 'log_file':
                    ...
                    follow_up_question = "Are multiple services showing similar error patterns?"
    ...
    def _fallback_timeline_analysis(self, agent_state: AgentState, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if context.get('uploaded_data'):
            for data in context['uploaded_data']:
                if data.get('data_type') == 'log_file':
                    ...
                    follow_up_question = "Were there any deployments or changes around that time?"
    ...
    def _fallback_hypothesis_analysis(self, agent_state: AgentState, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        key_insight = f"Generated {len(hypothesis_list)} testable hypotheses" if hypothesis_list else "Need more specific symptoms to formulate hypotheses"
        follow_up_question = "Which hypothesis should we test first?" if hypothesis_list else "Can you provide more details about the specific error messages?"
    ...
    def _fallback_validation_analysis(self, agent_state: AgentState, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hypothesis_list:
            ...
            follow_up_question = "Can you provide additional logs or metrics to test our theories?"
    ...
    def _fallback_solution_analysis(self, agent_state: AgentState, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if validated_hypothesis:
            ...
            follow_up_question = "Would you like to proceed with implementing this solution?"
        else:
            key_insight = "Need to develop custom solution based on specific issue characteristics"
            follow_up_question = "What constraints or requirements should I consider for the solution?"
    ...
    def should_request_user_input(self, phase_result: Dict[str, Any]) -> bool:
        ...
        """
        return phase_result.get('requires_user_input', False)
