...
class KnowledgeBaseTool(BaseTool):
    ...
    async def _arun(
        ...
        try:
            ...
            formatted_results = self._format_results(results, expanded_query, metadata_filters)
...
class KnowledgeBaseFilteredTool(BaseTool):
    ...
    def _format_filtered_results(self, results: List[Dict[str, Any]], filters: Dict[str, Any]) -> str:
        """
        ...
        for i, result in enumerate(results, 1):
            ...
            if matched_filters:
                formatted_parts.append(f"**Matched Filters:** {', '.join(matched_filters)}")
