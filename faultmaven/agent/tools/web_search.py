...
class WebSearchTool(BaseTool):
    ...
    async def _arun(self, query: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        try:
            ...
            site_query = " OR ".join([f"site:{domain}" for domain in self._trusted_domains])
    ...
    async def _make_search_request(self, client: httpx.AsyncClient, query: str) -> Optional[httpx.Response]:
        """
        ...
        try:
            ...
            self._logger.error(f"Network error during web search: {e}")
            return None
        except httpx.HTTPStatusError as e:
            self._logger.error(f"HTTP error during web search: {e.response.status_code}")
            return None
        except Exception as e:
            self._logger.error(f"Unexpected error during web search request: {e}")
            return None
    
    def _format_search_results(self, results: Dict[str, Any], original_query: str) -> str:
        """
