...
class ContractProbeMiddleware(BaseHTTPMiddleware):
        ...
        if hasattr(response, "body") and response.body:
            try:
                ...
                if "application/json" in content_type:
                    probe_data["response_shape"] = self._analyze_response_shape(
                        response
                    )
    ...
    def _detect_violations(
        ...
        if status_code in [201, 202]:
            ...
            if not location:
                violations.append(
                    f"MISSING_LOCATION_HEADER: {status_code} response missing Location header"
                )
            elif location in ["null", "", None]:
                violations.append(
                    f"NULL_LOCATION_HEADER: {status_code} response has null/empty Location header"
                )

        # Critical violation: Missing Retry-After on 202
        if status_code == 202 and not headers.get("Retry-After"):
            violations.append(
                "MISSING_RETRY_AFTER: 202 response missing Retry-After header"
            )
    ...
    def _log_contract_probe(
        ...
        if violations:
            ...
            if self._failure_cache[failure_key] == 1:  # Log first occurrence
                ...
                self._failure_cache[failure_key] % 10 == 0
