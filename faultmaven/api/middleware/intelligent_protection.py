...
class IntelligentProtectionMiddleware(BaseHTTPMiddleware):
    ...
    async def _create_protection_response(
        self, decision: ProtectionDecision
    ) -> Response:
    ...
    async def _process_response(
        ...
        try:
            ...
            await self.coordinator.process_response(
    ...
    def _add_protection_headers(self, response: Response, decision: ProtectionDecision):
        ...
        if decision.applied_restrictions:
            response.headers["X-Protection-Restrictions"] = ",".join(
                decision.applied_restrictions
            )
    ...
    async def _update_performance_metrics(self, processing_time: float):
        ...
        # Update average
        self.average_processing_time = sum(self._processing_times) / len(
            self._processing_times
        )
        ...
        if processing_time > self.max_processing_time:
            self.logger.warning(
                f"Phase 2 protection processing took {processing_time:.2f}ms "
