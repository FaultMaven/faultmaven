...
class SystemOptimizationMiddleware(BaseHTTPMiddleware):
    ...
    async def _process_request_optimized(
    ...
    async def _cache_response(self, request: Request, response: Response):
        ...
        if (
            ...
            or len(self._response_cache) >= 1000
        ):  # Cache size limit
            return
    ...
    async def _apply_intelligent_compression(
        ...
        if (
            ...
            or len(response.body) < self.compression_threshold
        ):
            return response
        ...
        if any(
            ...
            for ct in ["image/", "video/", "audio/", "application/zip"]
        ):
            return response
        ...
        try:
            ...
            if compression_ratio < 0.9:  # At least 10% reduction
                ...
                self._optimization_metrics["compression_ratio"] = (
                    self._optimization_metrics["compression_ratio"] * 0.9
    ...
    async def _apply_streaming_optimization(
        self, request: Request, response: Response
    ) -> Response:
        """Apply streaming optimization for large responses"""
        ...
        if (
            ...
            and len(response.body) > 50000  # 50KB threshold
    ...
    async def _optimize_background_tasks(self, background_tasks: BackgroundTasks):
        ...
        self._optimization_metrics["background_tasks_optimized"] += len(
    ...
    async def _process_background_task_batch(self):
        ...
        await asyncio.gather(
            *[execute_task(task) for task in batch], return_exceptions=True
        )
    ...
    async def _optimize_resource_cleanup(self):
        ...
        if (
            self._request_count % 100 == 0 or current_time - self._last_gc_time > 60
        ):  # Every 100 requests or 60 seconds
            ...
            **self._optimization_metrics,
            "cache_status": {
                ...
                "most_accessed_endpoints": sorted(
                    self._cache_hit_counts.items(), key=lambda x: x[1], reverse=True
                )[:5],
            },
            "background_tasks": {
