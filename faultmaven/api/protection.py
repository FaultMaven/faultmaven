...
class ProtectionSystem:
    ...
    def __init__(
        ...
        self.intelligent_protection_enabled = (
            settings.protection.intelligent_protection_enabled
        )
    ...
    async def setup_protection_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.basic_protection_enabled:
                ...
                self.protection_status["basic_protection_active"] = basic_result[
                    "enabled"
                ]
            ...
            if self.intelligent_protection_enabled:
                ...
                self.protection_status["intelligent_protection_active"] = (
                    intelligent_result["enabled"]
                )

            # Calculate total protections
            total_protections = len(
                setup_results["basic_protection"].get("components", [])
    ...
    async def _setup_basic_protection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not validation["valid"]:
                self.logger.error(
                    f"Basic protection settings validation failed: {validation['errors']}"
                )
            ...
            if validation["warnings"]:
                self.logger.warning(
                    f"Basic protection settings warnings: {validation['warnings']}"
                )
            ...
            if not basic_result["components"]:
                basic_result["enabled"] = False
                self.logger.warning(
                    "Basic protection disabled: no components available"
                )
    ...
    async def _setup_intelligent_protection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not intelligent_result["components"]:
                ...
                self.logger.warning(
                    "⚠️ Intelligent protection disabled: no components enabled"
                )
            else:
                self.logger.info(
                    f"✅ Intelligent protection middleware added with {len(intelligent_result['components'])} components"
                )
    ...
    async def get_protection_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "overall_status": (
                "active"
                if (
                    self.protection_status["basic_protection_active"]
                    or self.protection_status["intelligent_protection_active"]
                )
                else "inactive"
            ),
            "basic_protection": {
                "status": (
                    "active"
                    if self.protection_status["basic_protection_active"]
                    else "inactive"
                ),
                "components": [],
            },
            "intelligent_protection": {
                "status": (
                    "active"
                    if self.protection_status["intelligent_protection_active"]
                    else "inactive"
                ),
                "components": [],
            },
        ...
        try:
            ...
            if self.intelligent_middleware:
                try:
                    intelligent_status = (
                        await self.intelligent_middleware.get_middleware_status()
                    )
                    ...
                    self.logger.error(
                        f"Error getting intelligent middleware status: {e}"
                    )
    ...
    async def get_protection_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "combined_metrics": {
                ...
                "total_requests_protected": self.protection_status[
                    "protected_requests"
                ],
                "protection_rate": 0.0,
            },
        ...
        try:
            ...
            if self.intelligent_middleware and self.intelligent_middleware.initialized:
                try:
                    coordinator_status = (
                        await self.intelligent_middleware.coordinator.get_system_status()
                    )
                    metrics["intelligent_metrics"] = coordinator_status.get(
                        "statistics", {}
                    )
                except Exception as e:
                    self.logger.error(
                        f"Error getting intelligent protection metrics: {e}"
                    )
...
async def setup_protection_middleware_async(
    ...
    if session_store is not None:
        ...
        protection_system = ProtectionSystem(
            app, session_store, settings=protection_settings
        )
        ...
        legacy_result = {
            "protection_enabled": setup_results.get("basic_protection", {}).get(
                "enabled", False
            ),
            "middleware_added": setup_results.get("basic_protection", {}).get(
                "middleware_added", []
            ),
            ...
            "advanced_protection": {
                "enabled": setup_results.get("intelligent_protection", {}).get(
                    "enabled", False
                ),
                "components": setup_results.get("intelligent_protection", {}).get(
                    "components", []
                ),
            },
        }
    ...
    try:
        ...
        logger.info(
            f"Protection middleware setup complete: {len(setup_info['middleware_added'])} middleware added"
        )
