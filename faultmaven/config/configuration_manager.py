...
This module implements centralized configuration management to replace scattered
environment variable handling throughout the FaultMaven codebase.
...
class ConfigurationManager(IConfiguration):
    ...
    def __init__(
        ...
        # Load file first, then environment variables (so env vars override file)
        if config_file:
    ...
    def _load_config_file(self, config_file: Path) -> None:
        """Load configuration from file (.env, .json, .yaml)."""
        ...
        try:
            ...
            elif config_file.suffix in ['.env']:
    ...
    def _validate_section(self, section: ConfigSection) -> None:
        ...
        # Check validators for existing keys
        for key, validator in section.validators.items():
            if key in self._config_data:
                try:
                    value = self._config_data[key]
                    if not validator(value):
                        self._validation_errors.append(f"Invalid value for {key}: {value}")
                except Exception as e:
    ...
    def get_bool(self, key: str, default: bool = False) -> bool:
        ...
        if isinstance(value, str):
            ...
            # Handle explicit false values
                ...
                return False
            # For invalid strings, return default
            else:
                return default
        return default
    ...
    def get_security_config(self) -> Dict[str, Any]:
        ...
        return {
            "presidio_analyzer_url": self.get_str("PRESIDIO_ANALYZER_URL", "http://presidio-analyzer.faultmaven.local:30080"),
            "presidio_anonymizer_url": self.get_str("PRESIDIO_ANONYMIZER_URL", "http://presidio-anonymizer.faultmaven.local:30080"),
            "pii_detection_confidence": self.get_int("PII_DETECTION_CONFIDENCE", 0.8),
            "sanitization_enabled": self.get_bool("SANITIZATION_ENABLED", True),
            "sanitization_timeout": self.get_int("SANITIZATION_TIMEOUT", 10)
        }
    
    def get_observability_config(self) -> Dict[str, Any]:
        """Get observability configuration."""
        return {
            "opik_use_local": self.get_bool("OPIK_USE_LOCAL", True),
            "opik_local_url": self.get_str("OPIK_LOCAL_URL", "http://opik.faultmaven.local:30080"),
            "opik_project_name": self.get_str("OPIK_PROJECT_NAME", "FaultMaven Development"),
            "opik_api_key": self.get_str("OPIK_API_KEY", "local-dev-key"),
            "opik_track_disable": self.get_bool("OPIK_TRACK_DISABLE", False),
            "tracing_sample_rate": self.get_int("TRACING_SAMPLE_RATE", 1.0),
    ...
    def get_performance_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "worker_pool_size": self.get_int("WORKER_POOL_SIZE", 4),
            "max_concurrent_requests": self.get_int("MAX_CONCURRENT_REQUESTS", 100),
            "cache_ttl_seconds": self.get_int("CACHE_TTL_SECONDS", 300),
