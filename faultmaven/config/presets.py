...
class PresetDefinition:
    ...
    required_overrides: Dict[str, Set[str]] = field(default_factory=dict)  # provider -> required keys
...
def get_preset(preset_name: str) -> Optional[PresetDefinition]:
    ...
    try:
        ...
        logger.warning(f"Unknown preset: {preset_name}. Available: {[p.value for p in PresetName]}")
        return None
...
def apply_preset_defaults():
    ...
    preset = get_preset(preset_name)
    if not preset:
        logger.warning(f"Preset '{preset_name}' not found, continuing without preset defaults")
        return
...
def validate_preset_requirements() -> List[str]:
    ...
    provider = os.getenv("CHAT_PROVIDER", preset.defaults.get("CHAT_PROVIDER", "")).lower()
