...
async def probe_revocation_storage(store) -> str | None:
...
async def validate_revocation_storage(store) -> None:
    ...
    reason = await probe_revocation_storage(store)
    if reason is None:
    ...
    raise RevocationStorageUnavailableError(
        ...
        f"would be unenforceable: {reason}\n"
        ...
        "already stamped with it. Re-provision on the current baseline "
        "(`fm-wipe-deployment --wipe`), which is the documented upgrade path "
        "for this campaign."
    )
