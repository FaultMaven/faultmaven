...
class PromptBudgetSettings(BaseSettings):
    ...
    turn_token_ceiling: int = Field(
        ...
        description=(
            ...
            "the tool loop is schema-only. The structural bound is per call: "
            "MAX_TOOL_ITERATIONS + 1 calls, each held to PROMPT_TARGET_TOKENS + "
            "PROMPT_TOOL_OBSERVATION_MAX_TOKENS estimated tokens of messages "
            "plus tools payload, output not counted. Because this meters real "
            "tokens and more calls than that, it can remove the last tool round "
            "on an uncached turn with a full-size prompt (the first "
    ...
    tool_observation_max_tokens: int = Field(
        ...
        description=(
            ...
            "min(model_ceiling, prompt_target + this) — messages plus the tools "
            "payload sent on that call — so no continuation call grows "
            "unbounded past the jar. The system instruction and tools payload "
            "come out of this allowance when the base task fills prompt_target "
            "(~13,850 tokens on a diagnosis turn), so set it above that "
            "or full-size turns keep no observations. When the accumulated tool "
            "exchanges would exceed the bound, the OLDEST are elided (with a "
            "marker; the agent can re-search), keeping the base task + newest "
            "observations."
        ),
    )
