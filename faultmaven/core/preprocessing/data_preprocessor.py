    ...
    if top_errors:
        ...
        for i, error_info in enumerate(top_errors[:10], 1):
            if isinstance(error_info, dict):
                ...
                summary_parts.append(
                    f"{i}. [{error_count}x] {_truncate(error_msg, 150)}"
                )
            else:
...
def preprocess_metrics(
    ...
    summary_parts.append(
        "This feature will format time-series metrics, detect anomalies,"
    )
...
def preprocess_config(
    ...
    summary_parts.append(
        "This feature will analyze config files, detect misconfigurations,"
    )
