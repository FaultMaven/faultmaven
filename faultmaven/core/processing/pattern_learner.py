...
class PatternLearner:
            ...
            for pattern in relevant_patterns:
                try:
                    ...
                    regex_matches = re.finditer(
                        pattern.regex_pattern, content, re.IGNORECASE
                    )
    ...
    def get_pattern_statistics(self) -> dict[str, Any]:
        ...
        for pattern_type in PatternType:
            ...
            stats["patterns_by_type"][pattern_type.value] = {
                "count": len(patterns),
                "avg_confidence": (
                    np.mean([p.confidence for p in patterns]) if patterns else 0.0
                ),
                "avg_success_rate": (
                    np.mean([p.success_rate for p in patterns]) if patterns else 0.0
                ),
            }
        ...
        stats["pattern_quality"] = {
            ...
            "recently_used_patterns": len(
                [
                    p
                    for p in self._pattern_index.values()
    ...
    def _analyze_feedback(
        ...
        predicted_type = predicted_result.get("data_type") or predicted_result.get(
            "classification"
        )
        actual_type = actual_result.get("data_type") or actual_result.get(
            "classification"
        )
    ...
    async def _learn_classification_patterns(
        ...
        for pattern_info in extracted_patterns:
            pattern_id = self._generate_pattern_id(
                pattern_info["regex"], PatternType.CLASSIFICATION
            )
            ...
            if pattern_id in self._pattern_index:
                ...
                existing_pattern.confidence = min(
                    1.0, existing_pattern.confidence + self._learning_rate
                )
                updated += 1
            else:
                # Create new pattern
                new_pattern = Pattern(
                    ...
                    context_tags=[
                        actual_type,
                        learning_context.get("user_expertise", "unknown"),
                    ],
    ...
    async def _learn_error_patterns(
        ...
        for error_info in error_patterns:
            ...
            for pattern_info in extracted:
                pattern_id = self._generate_pattern_id(
                    pattern_info["regex"], PatternType.ERROR
                )
    ...
    async def _learn_security_patterns(
        ...
        for issue in security_issues:
            ...
            for pattern_info in extracted:
                pattern_id = self._generate_pattern_id(
                    pattern_info["regex"], PatternType.SECURITY
                )
        ...
        for line in lines:
            ...
            if classification == "log_file":
                ...
                error_words = re.findall(
                    r"\b(error|exception|failed|failure)\b", line, re.IGNORECASE
                )
        ...
        for line in lines:
            if any(
                ...
                if len(words) >= 2:
                    ...
                    regex_pattern = (
                        r"\b"
                        + r"\b.*\b".join(re.escape(w) for w in pattern_words)
                        + r"\b"
                    )
    ...
    def _cleanup_patterns(self) -> dict[str, int]:
        ...
        for pattern in self._pattern_index.values():
            ...
            if (
                pattern.confidence < self._min_confidence_threshold
                and self._pattern_usage.get(pattern.pattern_id, 0) < 2
            ):
    ...
    def _generate_pattern_id(
        self, regex_pattern: str, pattern_type: PatternType
    ) -> str:
