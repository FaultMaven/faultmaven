...
class CacheAnalytics:
    ...
    def record_access(
        ...
        with self._analytics_lock:
            ...
            if user_hash:
                pattern.user_distribution[user_hash] = (
                    pattern.user_distribution.get(user_hash, 0) + 1
                )

            # Update seasonal patterns (hour of day)
            hour = now.hour
            pattern.seasonal_pattern[str(hour)] = (
                pattern.seasonal_pattern.get(str(hour), 0) + 1
            )

            # Calculate effectiveness score
            if hit:
                pattern.effectiveness_score = min(
                    1.0, pattern.effectiveness_score + 0.1
                )
            else:
                pattern.effectiveness_score = max(
                    0.0, pattern.effectiveness_score - 0.05
                )
            ...
            if len(self.temporal_patterns[key_pattern]) > 1000:
                self.temporal_patterns[key_pattern] = self.temporal_patterns[
                    key_pattern
                ][-500:]
                    ...
                    recommendations.append(
                        {
                            ...
                            "suggested_ttl": min(
                                86400, int(pattern.recommended_ttl * 2)
                            ),
                        }
                    )
                    ...
                    recommendations.append(
                        {
                            ...
                            "suggested_tier": (
                                "L2" if len(pattern.user_distribution) < 50 else "L3"
                            ),
                        }
                    )
...
class IntelligentCache(BaseExternalClient):
    ...
    def __init__(
        ...
        self._analytics = (
            CacheAnalytics(metrics_collector) if enable_analytics else None
        )
        ...
        try:
            ...
            if hit:
                ...
                self._record_cache_access(
                    "L1", cache_key, True, access_time, user_id, context
                )
                return value, True
            ...
            if hit:
                ...
                access_time = (time.time() - start_time) * 1000
                self._record_cache_access(
                    "L3", cache_key, True, access_time, user_id, context
                )
                return value, True
            ...
            access_time = (time.time() - start_time) * 1000
            self._record_cache_access(
                "miss", cache_key, False, access_time, user_id, context
            )
            return None, False
    ...
    async def set(
        ...
        try:
            ...
            if cache_tier in ["L1", "auto"]:
                success = (
                    await self._set_to_l1(cache_key, value, ttl, context, tags)
                    or success
                )

            if cache_tier in ["L2", "auto"] and self._redis_client:
                success = (
                    await self._set_to_l2(cache_key, value, ttl, context, tags)
                    or success
                )

            if cache_tier in ["L3", "auto"]:
                success = (
                    await self._set_to_l3(cache_key, value, ttl, context, tags)
                    or success
                )
    ...
    async def get_cache_statistics(self) -> Dict[str, Any]:
        ...
        try:
            stats = {
                ...
                "overall": {
                    "total_hits": self._l1_stats.hits
                    + self._l2_stats.hits
                    + self._l3_stats.hits,
                    "total_misses": self._l1_stats.misses
                    + self._l2_stats.misses
                    + self._l3_stats.misses,
                },
            }
    ...
    async def warm_cache(self, warming_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Cache warming completed: {results['succeeded']}/{results['requested']} successful"
        )
        return results
    ...
    def _calculate_eviction_score(self, entry: CacheEntry) -> float:
        ...
        score *= max(
            0.1, 1.0 - (time_since_access / 3600)
        )  # Access recency decay over 1 hour
                    ...
                    for key_pattern, pattern in self._analytics.access_patterns.items():
                        for rule in self._optimization_rules:
                            try:
                                ...
                                self.logger.error(
                                    f"Error applying optimization rule: {e}"
                                )
        ...
        cache_health = {
            **base_health,
            ...
            "analytics": {
                ...
                "patterns_tracked": (
                    len(self._analytics.access_patterns) if self._analytics else 0
                ),
            },
