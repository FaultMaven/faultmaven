...
class RetryRecoveryStrategy(RecoveryStrategy):
    ...
    async def execute(
        ...
        if retry_count >= self.max_retries:
            self.logger.warning(
                f"Max retries ({self.max_retries}) exceeded for operation: {operation}"
            )
        ...
        self.logger.info(
            f"Retrying operation '{operation}' (attempt {retry_count + 1}/{self.max_retries}) after {delay:.2f}s delay"
        )
        ...
        try:
            ...
            if self._is_transient_error(error):
                ...
                self.logger.warning(
                    f"Retry failed for non-transient error: {type(error).__name__}"
                )
                return RecoveryResult.FAILED
...
class CircuitBreakerRecoveryStrategy(RecoveryStrategy):
    ...
    async def execute(
        ...
        if (
            ...
            circuit["state"] = "open"
            circuit["next_attempt_time"] = current_time + timedelta(
                seconds=self.timeout
            )
            self.logger.warning(
                f"Circuit breaker opened for {circuit_key} after {circuit['failure_count']} failures"
            )
        ...
        # If circuit is already open, check if we can transition to half-open
            ...
            if current_time >= circuit["next_attempt_time"]:
                ...
                self.logger.info(
                    f"Circuit breaker transitioning to half-open for {circuit_key}"
                )
                return RecoveryResult.PARTIAL
            else:
    ...
    def get_circuit_status(self, service: str, layer: str) -> Dict[str, Any]:
        ...
        return self.circuit_states.get(
            circuit_key, {"state": "closed", "failure_count": 0}
        )
    ...
    async def execute(
        ...
        try:
            ...
            if result:
                ...
                self.logger.warning(
                    f"Fallback provided partial recovery for {service}.{operation}"
                )
                return RecoveryResult.PARTIAL
...
class RecoveryManager:
    ...
    async def execute_recovery(
        ...
        if not strategy.is_applicable(error, layer):
            self.logger.debug(
                f"Strategy '{strategy_name}' not applicable for {type(error).__name__} in {layer} layer"
            )
            return RecoveryResult.NOT_ATTEMPTED
        ...
        try:
            ...
            recovery_record = {
                ...
                "context": {
                    ...
                    if k not in ["fallback_response", "operation_func"]
                },
            }
            ...
            self.logger.info(
                f"Recovery strategy '{strategy_name}' completed with result: {result.value}"
            )
            return result

        except Exception as recovery_error:
            self.logger.error(
                f"Error executing recovery strategy '{strategy_name}': {recovery_error}"
            )
            return RecoveryResult.FAILED
    ...
    def get_recovery_analytics(self) -> Dict[str, Any]:
        ...
        successful_attempts = len(
            [r for r in self.recovery_history if r["result"] == "success"]
        )
        ...
        for strategy, stats in strategy_stats.items():
            ...
            strategy_records = [
                r for r in self.recovery_history if r["strategy"] == strategy
            ]
            stats["avg_duration"] = sum(
                r["duration_ms"] for r in strategy_records
