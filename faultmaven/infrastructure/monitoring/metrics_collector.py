...
class MetricsCollector:
    ...
    def _initialize_default_thresholds(self) -> None:
        ...
        self.alert_thresholds = {
            ...
            "memory.usage": {"percentage": 85.0},
            "cpu.usage": {"percentage": 80.0},
        }
    ...
    def record_performance_metric(
        ...
        self.logger.debug(
            f"Recorded metric: {metric_key} = {duration_ms}ms, success={success}"
        )
    ...
    def record_gauge_metric(
        ...
        if len(self.metrics[gauge_key]) >= 100:
            ...
            self.metrics[gauge_key] = deque(
                list(self.metrics[gauge_key])[-50:], maxlen=self.max_samples
            )
    ...
    def set_alert_threshold(
        self, metric_name: str, threshold_type: str, threshold_value: float
    ) -> None:
    ...
    def get_dashboard_data(self, time_window_minutes: int = 60) -> Dict[str, Any]:
        ...
        for metric_name in self.metrics.keys():
            if "." in metric_name and not metric_name.startswith(
                ("duration.", "success.", "gauge.")
            ):
        ...
        for component in components:
            ...
            if f"{component}.request" in self.metrics:
                duration_aggregated = self.get_aggregated_metrics(
                    f"duration.{component}.request", time_window_minutes
                )
                if duration_aggregated:
                    ...
                    total_response_time += (
                        duration_aggregated.avg_value * duration_aggregated.count
                    )
            ...
            if success_key in self.metrics:
                success_aggregated = self.get_aggregated_metrics(
                    success_key, time_window_minutes
                )
                if success_aggregated:
                    ...
                    total_errors += int(
                        success_aggregated.count * (1 - success_aggregated.avg_value)
                    )
        ...
        if total_requests > 0:
            ...
            dashboard_data["summary"]["avg_response_time"] = (
                total_response_time / total_requests
            )
            dashboard_data["summary"]["error_rate"] = (
    ...
    def _get_recent_alerts(self) -> List[Dict[str, Any]]:
        ...
        for metric_name, thresholds in self.alert_thresholds.items():
            ...
            if aggregated:
                if (
                    "p95_ms" in thresholds
                    and aggregated.p95_value > thresholds["p95_ms"]
                ):
                    alerts.append(
                        {
                            "metric": metric_name,
