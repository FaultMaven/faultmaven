...
class MetricsCollector(BaseExternalClient):
    ...
    def __init__(
        ...
        # Caching statistics
        self._cache_stats = defaultdict(
            lambda: {"hits": 0, "misses": 0, "hit_rate": 0.0, "avg_retrieval_time": 0.0}
        )
        ...
        self.logger.info(
            "MetricsCollector initialized with advanced performance monitoring"
        )
    ...
    def measure_operation(
        ...
        try:
            ...
            self.record_metric(
                ...
                value=duration_ms,
                unit="milliseconds",
                metadata={**(metadata or {}), "success": not error_occurred},
                tags=tags,
            )
            ...
            with self._analytics_lock:
                ...
                if retrieval_time_ms is not None:
                    ...
                    stats["avg_retrieval_time"] = (
                        (current_avg * (total_requests - 1) + retrieval_time_ms)
                        / total_requests
                        if total_requests > 1
                        else retrieval_time_ms
                    )
    ...
    async def get_service_performance_summary(
        ...
        try:
            ...
            for operation, values in operations.items():
                ...
                thresholds = self._performance_thresholds.get(service, {}).get(
                    operation
                )
                ...
                if stats["avg"] > thresholds.get("target", 0) * 1.5:
                    recommendations.extend(
                        self._generate_operation_recommendations(
            ...
            performance_summary = {
                ...
                "status": (
                    "healthy"
                    if health_score > 0.8
                    else "degraded" if health_score > 0.6 else "critical"
                ),
    ...
    async def get_system_performance_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for service in services:
                try:
                    service_summary = await self.get_service_performance_summary(
                        service, 60
                    )
                    ...
                    if "alerts" in service_summary:
                        for alert in service_summary["alerts"]:
                            if alert["severity"] == "critical":
                                ...
                                and dashboard_data["system_status"] == "healthy"
                            ):
                                dashboard_data["system_status"] = "degraded"

                except Exception as e:
                    self.logger.error(
                        f"Failed to get summary for service {service}: {e}"
                    )
            ...
            dashboard_data["cross_service_metrics"] = (
                await self._analyze_cross_service_performance()
            )
            ...
            for svc in services_to_analyze:
                if svc in self._service_profiles:
                    ...
                    service_recommendations = (
                        await self._analyze_service_for_optimizations(profile)
                    )
    ...
    def _check_performance_thresholds(self, metric: MetricData) -> None:
        ...
        try:
            thresholds = self._performance_thresholds.get(metric.service, {}).get(
                metric.operation
            )
            if not thresholds:
                return
            ...
            if metric.value > thresholds.get("critical", float("inf")):
                ...
                if (
                    alert_key not in self._active_alerts
                ):  # Don't downgrade existing critical alerts
                    self._alert_history.append(
                        {
                            "timestamp": metric.timestamp.isoformat(),
                            "severity": "warning",
            ...
            try:
                if rule["condition"](stats):
                    recommendations.append(
                        f"{service}.{operation}: {rule['recommendation']}"
                    )
            except Exception as e:
                self.logger.error(
                    f"Failed to evaluate optimization rule {rule['name']}: {e}"
                )
    ...
    async def _update_service_profile(
        ...
        try:
            with self._profile_lock:
                ...
                for operation, stats in performance_summary.get(
                    ...
                    error_rates[operation] = (
                        0.0  # Would be calculated from actual error metrics
                    )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        metrics_health = {
            **base_health,
            ...
            "analytics_data_points": sum(
                len(data) for data in self._analytics_data.values()
            ),
