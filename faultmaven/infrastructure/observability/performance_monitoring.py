...
class PerformanceMonitor:
    ...
    def get_service_config(self, service_name: str) -> Dict[str, Any]:
        ...
        return self._service_configs.get(
            service_name,
            {"operations": {}, "track_patterns": False, "cache_enabled": False},
        )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                if (
                    track_user_patterns
                    and monitor._enable_user_patterns
                    and session_id
                    and user_id
                ):
...
def measure_workflow_step(
    ...
    try:
        yield {
            ...
            "set_knowledge_items": lambda count: locals().update(
                knowledge_items_retrieved=count
            ),
            "set_confidence_score": lambda score: locals().update(
...
def track_cache_performance(
    service: str, operation: str, cache_key_extractor: Callable
):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if (
                not monitor
                or not monitor._metrics_collector
                or not monitor._enable_cache_monitoring
            ):
                return await func(*args, **kwargs)
            ...
            try:
                ...
                monitor._metrics_collector.record_cache_event(
                    ...
                    event_type="error",
                    retrieval_time_ms=retrieval_time,
                    metadata={"operation": operation, "error": str(e)[:200]},
                )
                raise
            ...
            pattern_data.update(
                {
                    ...
                    "has_filters": bool(
                        kwargs.get("tags") or kwargs.get("document_type")
                    ),
                }
            )
