...
class AnomalyExplanation:
    ...
    def __init__(self, features: dict[str, float], thresholds: dict[str, float]):
        ...
        if self.contributing_features:
            ...
            self.explanation_text = (
                "Anomaly detected based on overall pattern deviation"
            )
...
class AnomalyDetectionSystem:
    ...
    async def train_models(self, historical_data: list[dict[str, Any]]):
        ...
        try:
            ...
            if len(historical_data) < self.min_training_samples:
                self.logger.warning(
                    f"Insufficient training data: {len(historical_data)} < {self.min_training_samples}"
                )
                return

            self.logger.info(
                f"Training anomaly detection models on {len(historical_data)} samples"
            )
    ...
    def _generate_explanation(
        ...
        if anomaly_score > 0.7:
            ...
            explanation.explanation_text = (
                "Moderate anomaly detected with some unusual patterns"
            )
    ...
    async def _add_training_sample(
        self, feature_array: np.ndarray, anomaly_score: float
    ):
