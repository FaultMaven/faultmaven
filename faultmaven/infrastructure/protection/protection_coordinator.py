...
class ProtectionCoordinator:
    ...
    async def analyze_request(
        self, session_id: str, request_data: dict
    ) -> ProtectionDecision:
        """
        ...
        try:
            ...
            if self.behavioral_analyzer:
                ...
                if client_profile.behavior_profile:
                    client_profile.behavior_profile.last_updated = datetime.now(UTC)
            ...
            if (
                ...
                and client_profile.behavior_profile
                and client_profile.behavior_profile.behavior_vectors
            ):
            ...
            if self.reputation_engine and client_profile.reputation_score:
                ...
                if reputation_level == ReputationLevel.BLOCKED:
                    return ProtectionDecision(
                        decision_id=f"rep_block_{session_id}_{int(datetime.now(UTC).timestamp())}",
            ...
            circuit_decision = await self._check_circuit_breakers(
                request_data, client_profile
            )
    ...
    async def process_response(
        self, session_id: str, request_data: dict, response_data: dict
    ):
        """
        ...
        try:
            ...
            if self.behavioral_analyzer:
                ...
                await self.behavioral_analyzer.update_behavior_model(
            ...
            if self.circuit_breakers:
                ...
                if endpoint in self.circuit_breakers:
                    client_profile = await self._get_client_profile(session_id)
                    await self.circuit_breakers[endpoint].update_metrics(
                        response, client_profile
                    )
        ...
        try:
            ...
            if client_profile.reputation_score:
                ...
                if rep_level in [ReputationLevel.BLOCKED, ReputationLevel.RESTRICTED]:
                    risk_assessment["risk_factors"].append(
                        f"Low reputation: {rep_level.value}"
                    )
            ...
            # Recent anomalies
            if self.behavioral_analyzer:
                recent_anomalies = await self.behavioral_analyzer.detect_anomalies(
                    session_id
                )
                if len(recent_anomalies) > 2:
                    risk_assessment["risk_factors"].append(
                        f"{len(recent_anomalies)} recent anomalies detected"
                    )
                    risk_assessment["overall_risk"] = max(
            ...
            if risk_assessment["risk_factors"]:
                risk_assessment["recommendations"].extend(
                    ["Enhanced monitoring recommended", "Consider stricter rate limits"]
                )
            ...
            confidence_factors = []
            if (
                client_profile.behavior_profile
                and client_profile.behavior_profile.total_requests > 10
            ):
    ...
    async def get_system_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self.behavioral_analyzer:
                status["components"]["behavioral_analyzer"] = {
                    ...
                    "profiles_tracked": len(
                        self.behavioral_analyzer._behavior_profiles
                    ),
                }
            ...
            if self.anomaly_detector:
                status["components"][
                    "anomaly_detector"
                ] = await self.anomaly_detector.get_model_status()
    ...
    async def _get_client_profile(self, session_id: str) -> ClientProfile:
        ...
        if session_id in self._client_profiles:
            ...
            if timestamp and datetime.now(UTC) - timestamp < self._profile_cache_ttl:
        ...
        if self.behavioral_analyzer:
            behavior_profile = await self.behavioral_analyzer.get_behavior_profile(
                session_id
            )
        ...
        if self.reputation_engine:
            reputation_score = await self.reputation_engine.calculate_reputation(
                session_id
            )
    ...
    async def _check_circuit_breakers(
        self, request_data: dict, client_profile: ClientProfile
    ) -> ProtectionDecision:
        ...
        if endpoint in self.circuit_breakers:
            ...
            if decision.action.value != "allow":
                ...
                return ProtectionDecision(
                    decision_id=f"cb_{endpoint}_{client_profile.client_id}_{int(datetime.now(UTC).timestamp())}",
                    session_id=client_profile.client_id,
        ...
        return ProtectionDecision(
            decision_id=f"cb_allow_{client_profile.client_id}_{int(datetime.now(UTC).timestamp())}",
            session_id=client_profile.client_id,
            allow_request=True,
    ...
    async def _make_final_decision(
        self,
        session_id: str,
        behavior_score: BehaviorScore | None,
        ...
        if behavior_score:
            ...
            if behavior_score.risk_level != RiskLevel.LOW:
                ...
                explanation_parts.append(
                    f"Behavioral risk: {behavior_score.risk_level.value}"
                )

        # Anomaly detection factor
        if anomaly_results:
            max_anomaly_score = max(result.overall_score for result in anomaly_results)
            decision_factors["anomaly"] = (
                1.0 - max_anomaly_score
            )  # Invert for decision factor
            confidence += 0.25
        ...
        if explanation_parts:
            explanation = "Protection decision based on: " + ", ".join(
                explanation_parts
            )
        else:
        ...
        return ProtectionDecision(
            decision_id=f"final_{session_id}_{int(datetime.now(UTC).timestamp())}",
            session_id=session_id,
    ...
    async def _update_client_profile(
        self, client_profile: ClientProfile, decision: ProtectionDecision
    ):
        """Update client profile based on protection decision"""
        client_profile.last_activity = datetime.now(UTC)
    ...
    async def _update_reputation_from_response(
    ...
    async def _provide_ml_feedback(
        self, session_id: str, request_data: dict, response_data: dict
    ):
        ...
        feedback = ModelFeedback(
            prediction_id=f"{session_id}_{int(datetime.now(UTC).timestamp())}",
    ...
    async def _get_system_load_factor(self) -> float:
        ...
        try:
            ...
            load_factor = min(
                active_clients / 100.0, 0.5
            )  # Scale to max 100 clients = 50% load

            # Add circuit breaker load
            open_circuits = sum(
                1 for cb in self.circuit_breakers.values() if cb.state.value == "open"
            )
    ...
    async def _monitoring_loop(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                # Log statistics
                self.logger.debug(
                    f"Intelligent protection statistics: {self.protection_statistics}"
                )
    ...
    async def _cleanup_client_profiles(self):
        ...
        cutoff_time = (
            datetime.now(UTC) - self._profile_cache_ttl * 2
    ...
    async def _update_system_metrics(self):
        ...
        active_requests = sum(
            1 for cb in self.circuit_breakers.values() if cb.metrics.total_requests > 0
        )
        failed_requests = sum(
    ...
    async def _on_circuit_failure(self, request, response):
        ...
        if response and response.is_failure:
            self.logger.warning(
                f"Circuit breaker failure detected: {response.status_code}"
            )
