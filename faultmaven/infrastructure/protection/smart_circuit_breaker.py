...
class SmartCircuitBreaker:
    ...
    async def should_allow_request(
        ...
        try:
            ...
            if client:
                reputation_decision = await self._reputation_based_decision(
                    request, client
                )
            ...
            return Decision(
                CircuitDecision.ALLOW, "Error in circuit breaker, defaulting to allow"
            )
        ...
        try:
            ...
            if response.is_success:
                ...
                if response.error_type:
                    self.error_counts[response.error_type] = (
                        self.error_counts.get(response.error_type, 0) + 1
                    )
        ...
        try:
            ...
            if system_metrics.overall_health_score < 0.5:
                ...
                self.adaptive_failure_threshold = max(
                    2, self.config.failure_threshold // 2
                )
            ...
            self.adaptive_response_time_threshold = (
                self.config.response_time_threshold * (1 + load_factor)
            )
            ...
            if recent_error_rate > self.config.error_rate_threshold * 2:
                ...
                self.adaptive_error_rate_threshold = min(
                    0.8, self.config.error_rate_threshold * 1.5
                )
            else:
                self.adaptive_error_rate_threshold = self.config.error_rate_threshold

            self.logger.debug(
                f"Adjusted thresholds - Failures: {self.adaptive_failure_threshold}, "
    ...
    async def predict_failure_risk(
        ...
        try:
            ...
            if len(recent_responses) >= 5:
                ...
                failure_rate = sum(1 for r in recent_responses if r.is_failure) / len(
                    recent_responses
                )
                ...
                if (
                    ...
                    >= self.adaptive_failure_threshold // 2
                ):
                    risk_score += 0.4

                confidence = min(
                    len(recent_responses) / 20.0, 1.0
                )  # More data = higher confidence
                predicted_failures = int(
                    failure_rate * 10
                )  # Predict failures in next 10 requests
    ...
    async def reset(self):
        ...
        self.logger.info(
            f"Circuit breaker '{self.name}' reset from {old_state.value} to {self.state.value}"
        )

    async def _reputation_based_decision(
        self, request: Request, client: ClientProfile
    ) -> Decision:
    ...
    async def _check_state_transitions(self, response: Response):
        ...
        if self.state == CircuitState.CLOSED:
            ...
            if (
                self.metrics.consecutive_failures >= self.adaptive_failure_threshold
                or (
                    self.metrics.total_requests >= self.config.min_requests
                    and self.metrics.error_rate >= self.adaptive_error_rate_threshold
                )
            ):
    ...
    async def _calculate_load_factor(self) -> float:
        ...
        rate_factor = min(
            request_rate / 10.0, 1.0
        )  # Normalize to max 10 requests/second
