...
class TimeoutHandler:
    ...
    async def timeout_context(
        ...
        try:
            ...
            raise TimeoutError(
                operation=operation_name, timeout_duration=timeout_duration
            )
    ...
    async def with_agent_timeout(
        self, coro: Coroutine[Any, Any, T], operation_name: str = "agent_execution"
    ) -> T:
        """Execute agent operation with standard agent timeout"""
        return await self.with_timeout(coro, operation_name, self.config.agent_total)
    ...
    async def with_phase_timeout(
        ...
        return await self.with_timeout(
            coro, f"agent_phase_{phase_name}", self.config.agent_phase, parent_context
        )
    ...
    async def with_llm_timeout(
        ...
        return await self.with_timeout(
            coro, "llm_call", self.config.llm_call, parent_context
        )

    async def emergency_shutdown(
        self, operation_name: str, reason: str = "Resource exhaustion"
    ) -> None:
        """
    ...
    def _record_operation_stats(
        self, operation_name: str, duration: float, timed_out: bool
    ) -> None:
        ...
        op_stats["avg_duration"] = (
            current_op_avg * (op_count - 1) + duration
        ) / op_count
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if timeout_rate > 0.2:  # More than 20% timeouts
            health_status["healthy"] = False
            health_status["issues"].append(f"High timeout rate: {timeout_rate:.1%}")
        ...
        if self._emergency_shutdowns > 0:
            ...
            if self._last_emergency_shutdown > recent_threshold:
                health_status["healthy"] = False
                health_status["issues"].append("Recent emergency shutdown detected")
