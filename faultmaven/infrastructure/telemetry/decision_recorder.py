...
class DecisionRecorder:
    ...
    async def _emit_structured_log(self, record: DecisionRecord, correlation_id: str):
        ...
        try:
            ...
            self._logger.info(
                ...
                extra={"structured_data": log_entry, "correlation_id": correlation_id},
            )
    ...
    def _calculate_budget_utilization(
        ...
        try:
            return {
                ...
                "token_utilization": (used.tokens_used / max(allocated.token_budget, 1))
                * 100,
                "call_utilization": (used.calls_used / max(allocated.call_budget, 1))
                * 100,
            }
        except Exception:
            return {
    ...
    async def _emit_to_opik(self, record: DecisionRecord, correlation_id: str):
        ...
        try:
            ...
            with self._tracer.trace("decision_record") as span:
                ...
                span.set_attribute(
                    "selected_agents", json.dumps(record.selected_agents)
                )
                ...
                self._logger.debug(
                    f"Emitted decision record to Opik: {record.record_id}"
                )
    ...
    async def _process_batch(self):
        ...
        try:
            ...
            if (
                processing_time_ms
                > self._sla_thresholds["max_batch_processing_time_ms"]
            ):
                self._metrics["performance_violations"] += 1
                self._logger.warning(
            ...
            self._logger.debug(
                f"Processed batch of {len(batch)} records in {processing_time_ms:.1f}ms"
            )
    ...
    async def _emit_analytics_event(self, record: DecisionRecord, correlation_id: str):
        ...
        try:
            ...
            event = {
                ...
                "budget_efficiency": {
                    "time_efficiency": (
                        record.budget_used.time_used
                        / max(record.budget_allocated.time_ms, 1)
                    ),
                    "token_efficiency": (
                        record.budget_used.tokens_used
                        / max(record.budget_allocated.token_budget, 1)
                    ),
                    "call_efficiency": (
                        record.budget_used.calls_used
                        / max(record.budget_allocated.call_budget, 1)
                    ),
                },
            ...
            self._logger.debug(
                f"Analytics event: {event['event_type']} for session {record.session_id}"
            )
                ...
                if session_id:
                    ...
                    for record_id in record_ids[-limit:]:  # Get most recent
                        if record_id in self._decision_records:
                            ...
                            records.append(
                                self._format_record_response(record, include_details)
                            )
            ...
            base_info.update(
                {
                    ...
                    "budget_allocated": (
                        record.budget_allocated.dict()
                        if record.budget_allocated
                        else {}
                    ),
                    "budget_used": (
                        record.budget_used.dict() if record.budget_used else {}
                    ),
                    "agent_latencies": record.agent_latencies,
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._metrics["records_created"] > 0:
                ...
                sla_compliance_rate = (
                    (self._metrics["records_created"] - violations)
                    / self._metrics["records_created"]
                ) * 100

            processing_efficiency = 0.0
            if self._metrics["records_created"] > 0:
                processing_efficiency = (
                    self._metrics["records_processed"]
                    / self._metrics["records_created"]
                ) * 100
    ...
    async def get_session_analytics(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            confidence_scores = [
                r.confidence.get("calibrated_score", 0.0) for r in session_records
            ]
        ...
        try:
            ...
            if record_id and record_id in self._decision_records:
                ...
                return {
                    ...
                    "decision_record": self._format_record_response(
                        record, include_details=True
                    ),
                    "trace_complete": True,
                }
