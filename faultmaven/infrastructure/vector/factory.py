...
def _create_chroma_backend(settings) -> IVectorBackend:
    ...
    try:
        ...
        persist_directory = getattr(
            database_settings, "chromadb_persist_dir", "./data/chroma"
        )
        collection_name = getattr(
            database_settings, "chromadb_collection", "faultmaven_kb"
        )
        ...
        if (
            chroma_host_raw is None
            or chroma_host_raw == ""
            or chroma_host_raw
            in ("localhost", "127.0.0.1", "chromadb.faultmaven.local")
        ):
            chroma_host = None
        else:
