...
class LLMRouter:
    ...
    async def route(self, 
                   ...
                   data_type: Optional[DataType] = None) -> LLMResponse:
        """
        ...
        for provider_key in providers_to_try:
            ...
            if provider_config['name'] in ['fireworks', 'openrouter'] and not provider_config['api_key']:
            ...
            try:
                ...
                if response and response.confidence >= self.confidence_threshold:
                    ...
                    self.cache.store(sanitized_prompt, model or provider_config['models'][0], response)
                    self.logger.info(f"High confidence response from {provider_key} (confidence: {response.confidence})")
                    return response
                else:
    ...
    async def _call_provider(self, 
        ...
        async def _make_call():
            if provider_config['name'] == 'fireworks':
                return await self._call_fireworks(prompt, model, max_tokens, temperature)
    ...
    async def _call_fireworks(self, prompt: str, model: str, max_tokens: int, temperature: float) -> LLMResponse:
    ...
    async def _call_openrouter(self, prompt: str, model: str, max_tokens: int, temperature: float) -> LLMResponse:
        """Call OpenRouter API"""
