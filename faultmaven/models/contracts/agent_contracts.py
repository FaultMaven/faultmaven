...
class ExecutionContext(BaseModel):
    ...
    allow_state_changes: bool = Field(
        default=False, description="Whether state-changing operations are allowed"
    )
    ...
    max_parallel_operations: int = Field(
        default=3, ge=1, le=10, description="Maximum parallel operations allowed"
    )
...
class AgentRequest(BaseModel):
    ...
    # Resource management
    budget: Budget = Field(
        default_factory=Budget, description="Resource budget for this request"
    )
...
class TriageResult(BaseModel):
    ...
    user_assessment_confidence: float = Field(
        ge=0.0, le=1.0, description="Confidence in user skill assessment"
    )
...
class ScopingResult(BaseModel):
    ...
    questions: List[str] = Field(
        max_items=2, description="Clarifying questions (max 2 per turn)"
    )
    ...
    scope_clarity_score: float = Field(
        ge=0.0, le=1.0, description="Current scope clarity assessment"
    )
    actionability_score: float = Field(
        ge=0.0, le=1.0, description="Whether enough info exists to proceed"
    )
    ...
    question_effectiveness_estimate: float = Field(
        ge=0.0, le=1.0, description="Estimated effectiveness of generated questions"
    )
...
class DiagnosticResult(BaseModel):
    ...
    diagnostic_confidence: float = Field(
        ge=0.0, le=1.0, description="Overall diagnostic confidence"
    )
    hypothesis_accuracy_estimate: float = Field(
        ge=0.0, le=1.0, description="Estimated accuracy of top hypothesis"
    )
...
class ValidationResult(BaseModel):
    ...
    best_practice_alignment: float = Field(
        ge=0.0, le=1.0, description="Alignment with best practices score"
    )
    ...
    validation_accuracy: float = Field(
        ge=0.0, le=1.0, description="Estimated validation accuracy"
    )
...
class PatternResult(BaseModel):
    ...
    pattern_match_confidence: float = Field(
        ge=0.0, le=1.0, description="Overall pattern matching confidence"
    )
...
class LearningResult(BaseModel):
    ...
    batch_id: str = Field(
        default_factory=lambda: str(uuid4()), description="Batch processing identifier"
    )
    processed_outcomes: int = Field(
        ge=0, description="Number of outcomes processed in this batch"
    )
    ...
    learning_confidence: float = Field(
        ge=0.0, le=1.0, description="Confidence in learned patterns"
    )
    validation_accuracy: float = Field(
        ge=0.0, le=1.0, description="Accuracy on validation dataset"
    )
