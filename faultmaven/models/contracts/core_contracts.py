...
class Budget(BaseModel):
    ...
    token_budget: int = Field(
        default=1500, ge=100, le=32000, description="Maximum tokens for LLM operations"
    )
    call_budget: int = Field(
        default=6, ge=1, le=50, description="Maximum external service calls"
    )
    ...
    def is_exhausted(self) -> bool:
        ...
        return (
            self.time_used >= self.time_ms
            or self.tokens_used >= self.token_budget
            or self.calls_used >= self.call_budget
        )
...
class TurnContext(BaseModel):
    ...
    turn_id: str = Field(
        default_factory=lambda: str(uuid4()), description="Unique turn identifier"
    )
    query: str = Field(
    ...
    budget: Budget = Field(
        default_factory=Budget, description="Resource budget for this turn"
    )
...
class RetrievalRequest(BaseModel):
    ...
    include_recency_bias: bool = Field(
        default=True, description="Whether to apply recency bias to results"
    )
...
class RetrievalResponse(BaseModel):
    ...
    total_found: int = Field(
        ge=0, description="Total number of matches found (before limiting)"
    )
...
class ConfidenceResponse(BaseModel):
    ...
    raw_score: float = Field(
        ge=0.0, le=1.0, description="Raw confidence score before calibration"
    )
...
class LoopCheckRequest(BaseModel):
    ...
    history: List[str] = Field(
        min_items=1, max_items=10, description="Recent query history for analysis"
    )
...
class LoopCheckResponse(BaseModel):
    ...
    confidence: float = Field(
        ge=0.0, le=1.0, description="Confidence in loop detection"
    )
...
class GatewayResult(BaseModel):
    ...
    clarity_assessment: str = Field(
        description="Clarity level (vague/specific/complex)"
    )
    reality_check: str = Field(
        description="Reality assessment (plausible/improbable/impossible)"
    )
    ...
    processing_priority: str = Field(
        default="normal", description="Suggested processing priority"
    )

    # Metadata
    processing_time_ms: int = Field(ge=0, description="Gateway processing time")
