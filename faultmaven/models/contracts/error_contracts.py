...
class BudgetExceededError(ServiceError):
    ...
    def __init__(self, **data):
        ...
        data.setdefault(
            "user_message",
            "Resource limit exceeded. Please try with a simpler request.",
        )
        super().__init__(**data)
...
class ServiceUnavailableError(ServiceError):
    ...
    fallback_available: bool = Field(
        default=False, description="Whether fallback service is available"
    )
...
class AuthenticationError(ServiceError):
    ...
    invalid_credentials: bool = Field(
        default=False, description="Whether credentials are invalid"
    )
...
def create_budget_exceeded_error(
    budget_type: str, limit: int, consumed: int, **kwargs
) -> BudgetExceededError:
...
def create_timeout_error(
    operation: str, timeout_ms: int, elapsed_ms: int, **kwargs
) -> TimeoutError:
...
def create_service_unavailable_error(
    service_name: str, health_check_details: str = None, **kwargs
    ...
    return ServiceUnavailableError(
        ...
        health_check_failures={
            service_name: health_check_details or "Connection failed"
        },
        **kwargs,
    )
