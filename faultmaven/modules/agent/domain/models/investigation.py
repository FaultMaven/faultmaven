...
class PhaseOODAProfile:
    ...
    **Weight Semantics**:
    ...
    def get_primary_steps(self) -> list[str]:
        """Steps with ≥30% normalized weight (primary focus)
...
class ProblemConfirmation(BaseModel):
    ...
    affected_components: List[str] = Field(
        default_factory=list, description="Approximate components"
    )
    ...
    estimated_evidence_needed: List[str] = Field(
        default_factory=list, description="Expected evidence categories"
    )
...
class AnomalyFrame(BaseModel):
    ...
    revised_at_turns: List[int] = Field(
        default_factory=list, description="Turns when revised"
    )
...
class TemporalFrame(BaseModel):
    ...
    first_noticed_at: Optional[datetime] = Field(
        None, description="When symptoms were first noticed by user"
    )
    actually_started_at: Optional[datetime] = Field(
        None, description="When issue actually started (may differ from first_noticed)"
    )
    ...
    confidence: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Confidence in timeline accuracy"
    )
...
class Hypothesis(BaseModel):
    ...
    promoted_to_active_at_turn: Optional[int] = Field(
        default=None, description="Turn when promoted from CAPTURED to ACTIVE"
    )
    ...
    confidence_trajectory: List[Tuple[int, float]] = Field(
        default_factory=list, description="(turn, confidence) history"
    )
    ...
    refuting_evidence: List[str] = Field(
        default_factory=list, description="Evidence IDs that contradict this hypothesis"
    )

    # Testing metadata
    test_plan: Optional[str] = Field(
        default=None, description="How to test this hypothesis"
    )
    ...
    def apply_confidence_decay(self, current_turn: int) -> float:
        ...
        """
        turns_since_progress = current_turn - (
            self.last_progress_at_turn or self.captured_at_turn
        )
...
class HypothesisTest(BaseModel):
    ...
    evidence_required: List[str] = Field(
        default_factory=list, description="Evidence request IDs"
    )
    evidence_obtained: List[str] = Field(
        default_factory=list, description="Evidence provided IDs"
    )
...
class OODAIteration(BaseModel):
    ...
    confidence_delta: float = Field(
        default=0.0, description="Change in max hypothesis confidence"
    )
...
class PhaseTransition(BaseModel):
    ...
    skipped_phases: List[InvestigationPhase] = Field(
        default_factory=list, description="Phases skipped (e.g., critical incident)"
    )
...
class WorkingConclusion(BaseModel):
    ...
    evidence_completeness: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Per-hypothesis completeness"
    )

    # Transparency (what agent communicates to user)
    caveats: List[str] = Field(
        default_factory=list, description="Limitations and uncertainties to communicate"
    )
    alternative_explanations: List[str] = Field(
        default_factory=list, description="Other hypotheses under consideration"
    )

    # Action guidance
    can_proceed_with_solution: bool = Field(
        default=False, description="Confidence ≥ 70%"
    )
    ...
    generated_at_turn: int = Field(
        ..., description="When this conclusion was generated"
    )
...
class ProgressMetrics(BaseModel):
    ...
    evidence_blocked_count: int = Field(
        default=0, description="Critical evidence requests blocked"
    )
    evidence_pending_count: int = Field(
        default=0, description="Evidence awaiting user response"
    )
    ...
    turns_since_last_progress: int = Field(
        default=0, description="Progress = evidence added OR confidence changed"
    )

    # Hypothesis progress
    active_hypotheses_count: int = Field(default=0)
    hypotheses_with_sufficient_evidence: int = Field(
        default=0, description="Evidence completeness ≥ 70%"
    )
    highest_hypothesis_confidence: float = Field(default=0.0, ge=0.0, le=1.0)

    # Next steps
    next_steps: List[str] = Field(
        default_factory=list, description="What agent plans to do next"
    )
...
class EscalationState(BaseModel):
    ...
    or requires escalation. Replaces mode switching (Consultant ↔ Lead Investigator).
    """

    # Escalation tracking
    escalation_suggested: bool = Field(
        default=False, description="Agent suggested escalation"
    )
    escalation_suggested_at_turn: Optional[int] = None
    escalation_reason: Optional[str] = None
    user_decision: Optional[str] = Field(
        default=None, description="accept_escalation, continue_degraded, ask_questions"
    )
    ...
    last_escalation_prompt_turn: int = Field(
        default=0, description="Last time suggested escalation"
    )
...
class MemorySnapshot(BaseModel):
    ...
    iteration_range: Tuple[int, int] = Field(
        ..., description="(start, end) iteration numbers"
    )
...
class HierarchicalMemory(BaseModel):
    ...
    hot_memory: List[OODAIteration] = Field(
        default_factory=list, description="Last 2 OODA iterations (full fidelity)"
    )

    # Warm memory: Summarized, iterations 3-5
    warm_snapshots: List[MemorySnapshot] = Field(
        default_factory=list, description="Iterations 3-5 (summarized, max 3)"
    )

    # Cold memory: Key facts, older iterations
    cold_snapshots: List[MemorySnapshot] = Field(
        default_factory=list, description="Older iterations (key facts only, max 5)"
    )

    # Persistent: Always accessible insights
    persistent_insights: List[str] = Field(
        default_factory=list, description="Key learnings maintained across all turns"
    )
...
class InvestigationLifecycle(BaseModel):
    ...
    entry_phase: InvestigationPhase = Field(
        default=InvestigationPhase.INTAKE, description="Where investigation started"
    )
    ...
    loop_back_history: List[dict] = Field(
        default_factory=list, description="History of loop-back events with reasons"
    )
    ...
    last_workflow_progression_prompt: Optional[str] = Field(
        default=None, description="Last workflow progression suggestion timestamp"
    )

    # Solution tracking (v3.0 NEW - for workflow progression detection)
    solution_verified: bool = Field(
        default=False, description="Whether solution has been verified as working"
    )
    solution_summary: Optional[str] = Field(
        default=None, description="Summary of solution that was applied"
    )
    verification_details: Optional[str] = Field(
        default=None, description="Details of how solution was verified"
    )
...
class OODAEngineState(BaseModel):
    ...
    ooda_active: bool = Field(
        default=False, description="OODA framework active (Phases 1-6)"
    )
    current_step: Optional[OODAStep] = None
    current_iteration: int = Field(
        default=0, description="OODA iteration count in current phase"
    )
    ...
    hypotheses: List[Hypothesis] = Field(
        default_factory=list
    )  # Generated in Phase 3, tested in Phase 4
    ...
    confidence_trajectory: List[float] = Field(
        default_factory=list, description="Max hypothesis confidence per turn"
    )
...
class EvidenceLayer(BaseModel):
    ...
    evidence_requests: List[str] = Field(
        default_factory=list, description="Evidence request IDs (see evidence.py)"
    )
    evidence_provided: List[str] = Field(
        default_factory=list, description="Evidence provided IDs (see evidence.py)"
    )
    evidence_coverage_score: float = Field(default=0.0, ge=0.0, le=1.0)
    critical_evidence_blocked: List[str] = Field(
        default_factory=list, description="Critical evidence requests blocked by user"
    )
