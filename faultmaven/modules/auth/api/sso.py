...
async def sso_login(
    ...
    authorization_url = await service.begin_login(return_to)
    return RedirectResponse(authorization_url, status_code=302, headers=_NO_STORE)
...
async def sso_callback(
    ...
    """
    redirect_url = await service.complete_callback(code=code, state=state, error=error)
