...
async def remove_team_member(
    ...
    try:
        success = await service.remove_member(
            team_id=team_id, user_id=target_user_id, removed_by=user_id
        )
        ...
        logger.info(
            f"Member removed from team: user {target_user_id} from team {team_id}"
        )
...
async def is_team_member(
    ...
    try:
        ...
        return {"is_member": is_member, "team_id": team_id, "user_id": target_user_id}

    except Exception as e:
        logger.error(
            f"Error checking membership for user {target_user_id} in team {team_id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e)
        )
