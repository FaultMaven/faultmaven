"""Persistence adapter for the IdP-organization → FaultMaven-organization map.

Implements ``ISSOOrgMappingRepository`` (``modules/auth/contracts.py``) over the
``sso_org_mappings`` table (migration 038). The table is deliberately outside
RLS: it is read on the unauthenticated SSO callback, before a tenant is bound.

Two shapes, mirroring the organization repositories: a session-bound repository
...
class SSOOrgMappingRepository(ISSOOrgMappingRepository):
    ...
    async def get_organization_id(
        ...
        stmt = select(SSOOrgMappingModel.organization_id).where(
...
class SessionlessSSOOrgMappingRepository(ISSOOrgMappingRepository):
    ...
    async def get_organization_id(
        ...
        async with get_db_session() as session:
            return await SSOOrgMappingRepository(session).get_organization_id(
