...
class InMemorySuggestionRepository(SuggestionRepository):
    ...
    — the same predicate every other factory keys off (fm#1128). The API, the
    jobs runner and the ``fm-*`` operator CLIs refuse to run without a
    persistent database (fm#1647, #1659), so this arm is a test seam. It
    reports ``is_durable == False`` wherever it is composed.
