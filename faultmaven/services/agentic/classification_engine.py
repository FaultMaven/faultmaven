...
from faultmaven.models.agentic import IQueryClassificationEngine
...
class QueryIntent(str, Enum):
    """Types of user intents we can classify"""
    ...
    async def classify_query(self, query: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "domain": TechnicalDomain.GENERAL.value,
...
1. intent: {list(intent.value for intent in QueryIntent)}
2. complexity: {list(level.value for level in ComplexityLevel)}  
3. domain: {list(domain.value for domain in TechnicalDomain)}
4. urgency: {list(urgency.value for urgency in UrgencyLevel)}
