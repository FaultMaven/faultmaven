...
class BusinessLogicWorkflowEngine(IBusinessLogicWorkflowEngine):
    ...
    async def plan_workflow(self, request: Dict[str, Any]) -> PlanningResult:
        ...
        try:
            ...
            # Convert dictionary steps to PlanNode objects
            plan_nodes = []
            step_dependencies = workflow_plan.get("dependencies", {})
            for step_dict in workflow_plan["steps"]:
                plan_node = PlanNode(
                    name=step_dict["name"],
                    description=f"{step_dict['name'].replace('_', ' ').title()} step",
                    action_type=step_dict.get("operation", "unknown"),
                    parameters=step_dict.get("parameters", {}),
                    dependencies=step_dependencies.get(step_dict["name"], [])
                )
            ...
            planning_result = PlanningResult(
                plan=ExecutionPlan(
                    ...
                    estimated_total_duration=timeline_estimate.get("total_duration"),
                    query=request.get("query", "")
                ),
                confidence=workflow_classification["confidence"] * validation_result.get("quality_score", 1.0),
                reasoning=f"Planned {planning_strategy} workflow with {len(workflow_plan['steps'])} steps"
            )
            ...
            logger.info(f"Workflow planned: {context.workflow_id}, strategy={planning_strategy}, steps={len(workflow_plan['steps'])}, confidence={planning_result.confidence:.3f}")
    ...
    async def _classify_workflow_type(self, request: Dict[str, Any], context: WorkflowContext) -> Dict[str, Any]:
        ...
        if self.classification_engine:
            try:
                ...
                return {
                    "type": classification_result.intent.value if hasattr(classification_result.intent, 'value') else str(classification_result.intent),
    ...
    async def _plan_reactive(
        ...
        return {
            ...
            "dependencies": base_workflow.metadata.get("dependencies", {}),
            "parallel_groups": [],
            "strategy": "reactive"
        }
    ...
    async def _execute_state_update_step(
        ...
        try:
            if self.state_manager:
                ...
                # Create a proper ConversationMemory object
                from faultmaven.models.agentic import ConversationMemory

                # Get existing memory or create new one
                existing_memory = await self.state_manager.get_conversation_memory(context.session_id or "default")
                if existing_memory:
    ...
    async def _mock_step_execution(self, step_id: str, context: WorkflowContext) -> StepExecutionResult:
        ...
        return StepExecutionResult(
            ...
            success=True,
            duration=0.1,
            output_data={"result": f"Mock result for {step_id}"}
        )
    ...
    async def _estimate_execution_timeline(self, plan: Dict[str, Any], context: WorkflowContext) -> Dict[str, Any]:
        ...
        return {
            ...
            "step_breakdown": {(step.node_id if hasattr(step, 'node_id') else step.name if hasattr(step, 'name') else f"step_{i}"): base_time_per_step for i, step in enumerate(steps)},
    ...
    async def _assess_workflow_risks(self, plan: Dict[str, Any], context: WorkflowContext) -> Dict[str, Any]:
        ...
        external_steps = [s for s in plan.get("steps", []) if (s.action_type if hasattr(s, 'action_type') else s.get("type", "") if hasattr(s, 'get') else "") == "tool_execution"]
    ...
    async def _execute_workflow_steps(
        ...
        iteration_count = 0
        previous_ready_count = -1  # Track progress to detect deadlocks

        steps_to_execute = plan.get('steps', []) if isinstance(plan, dict) else (plan.steps if hasattr(plan, 'steps') else [])
        dependencies = plan.get('dependencies', {}) if isinstance(plan, dict) else (plan.dependencies if hasattr(plan, 'dependencies') else {})

        # Create step_name to step_id mapping for dependency resolution
        step_name_to_id = {}
        step_id_to_name = {}
        for step in steps_to_execute:
            step_id = self._get_consistent_step_id(step)
            step_name = step.name if hasattr(step, 'name') else step_id
            step_name_to_id[step_name] = step_id
            step_id_to_name[step_id] = step_name

        logger.info(f"Starting workflow execution with {len(steps_to_execute)} steps")
        logger.debug(f"Step name to ID mapping: {step_name_to_id}")
        logger.debug(f"Dependencies: {dependencies}")

        while len(executed_steps) < len(steps_to_execute) and iteration_count < max_iterations:
            iteration_count += 1
            ready_steps = []
            ...
            for step in steps_to_execute:
                ...
                if step_id not in executed_steps:
                    ...
                    # Convert dependency names to IDs for proper checking
                    dependency_ids = []
                    for dep_name in step_deps:
                        dep_id = step_name_to_id.get(dep_name, dep_name)  # Fallback to name if not found
                        dependency_ids.append(dep_id)

                    # Check if all dependency IDs are in executed_steps
                    if all(dep_id in executed_steps for dep_id in dependency_ids):
                        ready_steps.append((step, step_id))
                        logger.debug(f"Step {step_id} ({step_name}) is ready - dependencies {dependency_ids} satisfied")
                    else:
                        missing_deps = [dep_id for dep_id in dependency_ids if dep_id not in executed_steps]
                        logger.debug(f"Step {step_id} ({step_name}) waiting for dependencies: {missing_deps}")

            # Deadlock detection: if no progress is made, break the loop
            current_ready_count = len(ready_steps)
            if current_ready_count == 0 and len(executed_steps) < len(steps_to_execute):
                logger.error(f"Deadlock detected at iteration {iteration_count}. No ready steps, but {len(steps_to_execute) - len(executed_steps)} steps remain.")
                logger.error(f"Executed steps: {executed_steps}")
                logger.error(f"Remaining steps: {[self._get_consistent_step_id(s) for s in steps_to_execute if self._get_consistent_step_id(s) not in executed_steps]}")

                # Try to identify circular dependencies
                remaining_steps = [s for s in steps_to_execute if self._get_consistent_step_id(s) not in executed_steps]
                for step in remaining_steps:
                    step_id = self._get_consistent_step_id(step)
                    step_name = step.name if hasattr(step, 'name') else step_id
                    step_deps = dependencies.get(step_name, [])
                    logger.error(f"Remaining step {step_id} ({step_name}) depends on: {step_deps}")

                break

            if current_ready_count == previous_ready_count and current_ready_count > 0:
                logger.warning(f"No progress made between iterations {iteration_count-1} and {iteration_count}. Same {current_ready_count} ready steps.")

            previous_ready_count = current_ready_count

            if not ready_steps:
                logger.warning(f"No ready steps found at iteration {iteration_count}. Possible deadlock. Executed: {executed_steps}")
                break

            logger.info(f"Iteration {iteration_count}: Executing {len(ready_steps)} ready steps")

            # Execute ready steps (can be done in parallel)
            step_tasks = []
            step_id_mapping = {}  # Map task index to step ID

            for i, (step, step_id) in enumerate(ready_steps):
                step_id_mapping[i] = step_id
                step_executor = self.step_executors.get(step.action_type if hasattr(step, 'action_type') else 'tool_execution')
                if step_executor:
                    step_tasks.append(step_executor(step, context, execution_data))
                else:
                    # Default mock execution with consistent step ID
                    step_tasks.append(self._mock_step_execution(step_id, context))
            ...
            if step_tasks:
                ...
                for i, result in enumerate(step_results):
                    expected_step_id = step_id_mapping[i]
                    ...
                    if isinstance(result, Exception):
                        ...
                        execution.steps_failed.append(expected_step_id)
                        logger.warning(f"Step {expected_step_id} failed: {str(result)}")
                    else:
                        # Ensure result has consistent step ID
                        if hasattr(result, 'step_id'):
                            result.step_id = expected_step_id
                        execution_results.append(result)

                        if result.success:
                            execution.steps_completed.append(expected_step_id)
                            executed_steps.add(expected_step_id)
                            execution_data.update(result.output_data)
                            logger.info(f"Step {expected_step_id} completed successfully")
                        else:
                            execution.steps_failed.append(expected_step_id)
                            logger.warning(f"Step {expected_step_id} marked as failed")

                execution.current_step = len(executed_steps)

        if iteration_count >= max_iterations:
            logger.error(f"Workflow execution stopped due to max iterations ({max_iterations}) reached. This indicates an infinite loop.")
            raise RuntimeError(f"Workflow execution infinite loop detected after {max_iterations} iterations")

        logger.info(f"Workflow execution completed in {iteration_count} iterations. {len(executed_steps)} steps executed successfully.")
        return execution_results

    def _get_consistent_step_id(self, step) -> str:
        """Get a consistent step ID for dependency tracking."""
        # Priority order: id > node_id > name > generate default
        if hasattr(step, 'id') and step.id:
            return step.id
        elif hasattr(step, 'node_id') and step.node_id:
            return step.node_id
        elif hasattr(step, 'name') and step.name:
            return step.name
        else:
            return f"step_{hash(str(step))}"
