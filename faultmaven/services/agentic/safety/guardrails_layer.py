    ...
    async def validate_input(self, content: str, context: Optional[Dict[str, Any]] = None) -> GuardrailsResult:
        ...
        try:
            ...
            if pii_result["violations"]:
                ...
                result.passed = False
                result.security_level = SecurityLevel.CONFIDENTIAL
                self.metrics["pii_detections"] += len(pii_result["violations"])
            
            # PII sanitization handled by Presidio service
            
            # Layer 3: Content policy enforcement
            # Use sanitized content from PII detection
            sanitized_content = pii_result["sanitized_content"]
            policy_violations = await self._enforce_content_policies(sanitized_content, context)
            ...
            threat_assessment = await self._assess_threats(sanitized_content, context)
            ...
            logger.info(f"Input validation completed: passed={result.passed}, violations={len(result.violations)}, time={processing_time:.3f}s")
    ...
    async def validate_output(self, content: str, context: Optional[Dict[str, Any]] = None) -> GuardrailsResult:
        ...
        try:
            ...
            if leakage_result["has_leakage"]:
                ...
                # Content sanitized by external service
                result.passed = False
                result.security_level = SecurityLevel.CONFIDENTIAL
            ...
            if bias_assessment["has_bias"]:
                ...
                result.security_level = SecurityLevel.INTERNAL
            ...
            logger.info(f"Output validation completed: passed={result.passed}, violations={len(result.violations)}")
                ...
                if rule.get("pattern"):
                    ...
                    for match in matches:
                        ...
                        violations.append(PolicyViolation(
                            ...
                            description=f"PII pattern matched: {rule.get('name', 'pattern')}",
                            location=f"position_{match.start()}_{match.end()}",
                            suggested_action=rule.get("action", "flag")
                        ))
    ...
    async def apply_data_sanitization(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, str):
                ...
                # GuardrailsResult doesn't track sanitized content, use original if validation passed
                sanitized_data[key] = value if result.passed else "[BLOCKED]"
            else:
