...
class GlobalConfidenceService(IGlobalConfidenceService):
    ...
    def __init__(
        ...
        if ML_AVAILABLE:
            ...
            self._logger.warning(
                "ML libraries not available - using fallback confidence scoring"
            )
    ...
    def _initialize_default_model(self):
        ...
        try:
            with self._model_lock:
                ...
                for i, (feature, (min_val, max_val, _)) in enumerate(
    ...
    async def score_confidence(self, request: ConfidenceRequest) -> ConfidenceResponse:
        ...
        try:
            ...
            self._logger.debug(
                f"Computed confidence score: {calibrated_score:.3f} ({confidence_band.value})"
            )
            return response
    ...
    def _fallback_confidence_score(self, features: np.ndarray) -> Tuple[float, float]:
        ...
        if len(features) != len(feature_weights):
            ...
            if len(features) < len(feature_weights):
                ...
                feature_weights = np.pad(
                    feature_weights,
                    (0, len(features) - len(feature_weights)),
                    "constant",
                    constant_values=0.1,
                )
    ...
    def _apply_hysteresis(self, session_id: str, current_score: float) -> float:
        ...
        if current_band != previous_band:
            ...
            if state["band_consistency_count"] < required_consistency:
                ...
                self._logger.debug(
                    f"Hysteresis: keeping band {previous_band.value} (consistency: {state['band_consistency_count']}/{required_consistency})"
                )
            else:
                ...
                self._logger.debug(
                    f"Hysteresis: band change {previous_band.value} -> {current_band.value}"
                )
        else:
    ...
    def _classify_confidence_band(self, score: float) -> ConfidenceBand:
        ...
        for band, (min_val, max_val) in self.BAND_THRESHOLDS.items():
            if min_val <= score < max_val or (
                band == ConfidenceBand.RESOLVED and score >= min_val
            ):
                return band
    ...
    def _update_prediction_metrics(self, processing_time_ms: float):
        ...
        self._metrics["avg_prediction_time_ms"] = (
                ...
                model_info = {
                    ...
                    "feature_definitions": {
                        ...
                        for name, (
                            ...
                            max_val,
                            desc,
                        ) in self.FEATURE_DEFINITIONS.items()
                    },
                    "confidence_bands": {
                        band.value: {
                            "range": [min_val, max_val],
                            "actions": self.BAND_ACTIONS[band],
                        }
                        for band, (min_val, max_val) in self.BAND_THRESHOLDS.items()
                    },
                    "hysteresis_config": {
        ...
        try:
            ...
            with self._model_lock:
                ...
                if not all(key in model_dict for key in required_keys):
                    raise ValidationException(
                        f"Invalid model data structure. Required: {required_keys}"
                    )
                ...
                try:
                    ...
                    self._logger.info(
                        f"✅ Updated confidence model to version {version}"
                    )
                    return True
