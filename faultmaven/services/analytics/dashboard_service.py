...
class AnalyticsDashboardService(BaseService):
    ...
    def __init__(
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=3, thread_name_prefix="analytics"
        )
    ...
    async def _generate_system_overview(
        ...
        if self._metrics_collector:
            system_performance = (
                await self._metrics_collector.get_system_performance_dashboard()
            )
        ...
        dashboard = {
            ...
            "cache_performance": {
                "overall_hit_rate": cache_performance.get("overall", {}).get(
                    "hit_rate", 0.0
                ),
                "l1_hit_rate": cache_performance.get("l1_cache", {}).get(
                    "hit_rate", 0.0
                ),
                "l2_hit_rate": cache_performance.get("l2_cache", {}).get(
                    "hit_rate", 0.0
                ),
        ...
        dashboard["optimization_recommendations"] = (
            await self._get_optimization_recommendations()
        )
    ...
    async def _generate_service_dashboard(
        ...
        service_health = await self._get_service_health_metrics(
            service_name, time_range_hours
        )

        # Get operation-specific analytics
        operation_analytics = await self._get_operation_analytics(
            service_name, time_range_hours
        )
        ...
        dashboard = {
            ...
            "health_summary": {
                ...
                "avg_response_time": self._calculate_service_avg_response_time(
                    service_performance
                ),
            },
        ...
        if include_detailed_metrics:
            dashboard["detailed_metrics"] = {
                "operation_statistics": service_performance.get(
    ...
    async def _generate_user_analytics(
        ...
        dashboard = {
            ...
            "engagement_metrics": {
                "avg_session_duration": user_engagement.get(
        ...
        if include_behavior_patterns:
            dashboard["behavior_patterns"] = await self._get_user_behavior_patterns(
                time_range_hours
            )

        # Add satisfaction metrics if requested
        if include_satisfaction_metrics:
            dashboard["satisfaction_metrics"] = (
                await self._get_user_satisfaction_metrics(time_range_hours)
            )
    ...
    async def _generate_workflow_analytics(
        ...
        dashboard = {
            ...
            "workflow_overview": {
                ...
                "avg_steps_per_workflow": workflow_metrics.get(
            ...
            "performance_metrics": {
                ...
                "knowledge_utilization": workflow_metrics.get(
                    "knowledge_utilization", 0.0
                ),
            },
            "common_patterns": await self._get_common_workflow_patterns(
                time_range_hours
            ),
            "failure_analysis": await self._get_workflow_failure_analysis(
                time_range_hours
            ),
        }
        ...
        if include_phase_analysis:
            dashboard["phase_analysis"] = await self._get_workflow_phase_analysis(
                time_range_hours
            )

        # Add optimization insights if requested
        if include_optimization_insights:
            dashboard["optimization_insights"] = (
                await self._get_workflow_optimization_insights(time_range_hours)
            )
    ...
    async def _execute_custom_query(
        ...
        for service in services:
            if self._metrics_collector:
                ...
                aggregated_data = self._apply_query_aggregation(
                    filtered_data, metrics, aggregation
                )
    ...
    def _calculate_overall_health_score(self, service_health: Dict[str, Any]) -> float:
        ...
        health_scores = [
            metrics.get("health_score", 0.0) for metrics in service_health.values()
        ]
    ...
    async def _update_cached_dashboard_data(self):
        ...
        with self._cache_lock:
            ...
            self._analytics_cache["system_overview"] = (
                await self._generate_system_overview(24, True, True)
            )

            # Update service dashboards cache
            services = ["memory_service", "planning_service", "knowledge_service"]
            for service in services:
                self._analytics_cache[f"service_{service}"] = (
                    await self._generate_service_dashboard(service, 24, False)
                )
