...
class SessionManager:
    ...
    def __init__(
        ...
        self.logger.info(
            f"SessionManager initialized with Redis at {redis_url}"
        )
    ...
    async def add_data_upload(self, session_id: str, data_id: str) -> bool:
        ...
        if not session:
            self.logger.warning(f"Attempted to add data to non-existent session: {session_id}")
            return False
    ...
    async def add_investigation_history(self, session_id: str, investigation_data: Dict) -> bool:
        ...
        if not session:
            self.logger.warning(f"Attempted to add history to non-existent session: {session_id}")
            return False
    ...
    async def update_agent_state(self, session_id: str, agent_state: Dict) -> bool:
        """
        ...
        if not session:
            self.logger.warning(f"Attempted to update agent state for non-existent session: {session_id}")
            return False
        ...
        for session_key in session_keys:
            ...
            if session_data:
                try:
                    ...
                    self.logger.error(f"Failed to deserialize session from {session_key}: {e}")
                    # Clean up corrupted session
                    await self.redis_client.delete(session_key)
