...
def create_response_format_json_schema(model: Type[BaseModel]) -> Dict[str, Any]:
    ...
    # ``strict: true`` is a claim about the schema, not a switch: OpenAI rejects
    # the request outright when the schema is not in its strict subset. This
    # previously asserted it over the raw Pydantic schema — 17 properties absent
    # from ``required`` and no ``additionalProperties`` anywhere (fm#1051). When
    # the schema cannot be expressed strictly, ask for the schema WITHOUT the
    # claim rather than sending one the API refuses.
    try:
        return {
            "type": "json_schema",
            "json_schema": {
        ...
        logger.info(
            "strict_schema_unavailable: %s falls back to unenforced json_schema "
            "(%s)",
            model.__name__,
            exc,
        )
