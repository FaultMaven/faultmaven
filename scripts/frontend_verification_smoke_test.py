...
class FrontendVerificationTest:
    ...
    async def run_smoke_test(self) -> dict[str, Any]:
        ...
        print("🚀 Starting Frontend Verification Smoke Test")
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 201:
                    ...
                    if not location_header or location_header in ["null", ""]:
                        issues.append(
                            f"201 response missing Location header: {location_header}"
                        )
                    ...
                    if not location_header or location_header in ["null", ""]:
                        issues.append(
                            f"202 response missing Location header: {location_header}"
                        )
                    ...
                    print(
                        f"   ✅ Async query (202): Location={location_header}, Retry-After={retry_after}"
                    )
                else:
                ...
                if "cases" not in data:
                    ...
                    if self.case_id:
                        ...
                        if self.case_id not in case_ids:
                            issues.append(
                                f"Created case {self.case_id} not immediately visible in session cases"
                            )
                ...
                if issues:
                    ...
                    test_result["details"] = {
                        ...
                        "immediate_visibility": (
                            self.case_id in [c.get("case_id") for c in data["cases"]]
                            if self.case_id
                            else None
                        ),
                    }
                    print(
                        f"   ✅ Session cases: {len(data['cases'])} cases, X-Total-Count={total_count_header}"
                    )
                    if self.case_id:
        ...
        try:
            async with self.session.get(
                ...
                if not isinstance(data, list):
                    issues.append(
                        f"Response is not array: {type(data)} - Frontend expects direct array parsing"
                    )
                ...
                if issues:
                    ...
                    print(
                        f"   ✅ Cases listing: {len(data)} cases, array response ✅, X-Total-Count={total_count_header}"
                    )
        ...
        try:
            ...
            async with self.session.get(
                ...
                if response.status == 500:
                    issues.append(
                        "Pre-auth probe returned 500 - should be 401/403 for service unavailable"
                    )
                elif response.status not in [401, 403, 404]:
                    issues.append(
                        f"Expected 401/403/404 for pre-auth, got {response.status}"
                    )
                ...
                if response.status == 500:
                    ...
                    print(
                        "   ❌ CRITICAL: Pre-auth probe returned 500 instead of 401/403"
                    )
                else:
                    test_result["status"] = "PASS"
                    test_result["details"] = {
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="FaultMaven Frontend Verification Smoke Test"
    )
    parser.add_argument(
        "--backend-url",
