        ...
        "examples": {
            "new_session": {
                ...
                "value": {
                    ...
                    "metadata": {"environment": "production", "team": "platform-team"},
                },
            },
            "resumed_session": {
            ...
            if "requestBody" in details:
                ...
                for content_type, content_details in content.items():
                    ...
                    if "examples" in content_details:
                        for example_name, example_data in content_details[
                            "examples"
                        ].items():
                    ...
                    for content_type, content_details in content.items():
                        if "schema" in content_details:
                            ...
                            if "$ref" in schema_ref:
                                ...
                                if schema_name in schema.get("components", {}).get(
                                    "schemas", {}
                                ):
                                    schema_def = schema["components"]["schemas"][
                                        schema_name
                                    ]
                                    if "example" in schema_def:
                                        md_content += f"```json\n"
                                        md_content += json.dumps(
                                            schema_def["example"], indent=2
                                        )
                                        md_content += "\n```\n\n"
        ...
        for schema_name, schema_def in schema["components"]["schemas"].items():
            ...
            if "properties" in schema_def:
                ...
                for prop_name, prop_def in schema_def["properties"].items():
                    required = (
                        "✅" if prop_name in schema_def.get("required", []) else "❌"
                    )
                    prop_type = prop_def.get("type", "unknown")
                    prop_desc = prop_def.get("description", "No description")
                    md_content += (
                        f"- `{prop_name}` ({prop_type}) {required} - {prop_desc}\n"
                    )
                md_content += "\n"
...
def generate_documentation() -> None:
    ...
    try:
        ...
        method_count = sum(
            len([m for m in methods.keys() if not m.startswith("x-")])
