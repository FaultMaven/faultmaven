...
class TestCoreAgent:
    ...
    def test_decide_if_user_update_needed_low_confidence(self):
        ...
        decision = self.agent._decide_if_user_update_is_needed(state)
        assert decision == "respond_to_user"  # Should require user input due to low confidence
    ...
    def test_decide_if_user_update_needed_findings(self):
        ...
        decision = self.agent._decide_if_user_update_is_needed(state)
        assert decision == "respond_to_user"  # Should respond to user when findings require response
    @pytest.mark.asyncio
    ...
    async def test_formulate_hypothesis_node_success(self):
        ...
        mock_phase_result = {
            ...
            'hypothesis_list': ['Connection pool exhaustion', 'Network timeout', 'Database lock'],
            'requires_user_input': True,
    ...
    async def test_propose_solution_node_success(self):
        ...
        mock_phase_result = {
            ...
            'solution_options': ['Increase connection pool size', 'Optimize queries', 'Add connection monitoring'],
            'requires_user_input': True,
            'next_phase': None
        }
    ...
    async def test_respond_to_user_node(self):
        ...
        assert 'System-wide authentication issue' in result['investigation_context']['last_agent_response']
        assert 'Are multiple users affected?' in result['investigation_context']['last_agent_response']
    ...
    async def test_node_execution_with_error(self):
        ...
        with patch.object(self.agent, 'doctrine') as mock_doctrine:
            mock_doctrine.execute_phase = AsyncMock(side_effect=Exception("LLM service error"))
            
            result = await self.agent.define_blast_radius(self.sample_agent_state)
            
            # Should handle error gracefully
            assert 'blast_radius_error' in result['investigation_context']
            assert 'LLM service error' in result['investigation_context']['blast_radius_error']
            assert 'agent_response' in result['investigation_context']  # Should set error response
    ...
    async def test_resume_method(self):
        ...
        with patch.object(self.agent.compiled_graph, 'get_state') as mock_get_state:
            with patch.object(self.agent.compiled_graph, 'ainvoke') as mock_ainvoke:
                ...
                result = await self.agent.resume("test-session-123", "Yes, multiple users are affected")
    ...
    def test_dangerous_operation_detection(self):
        ...
        for phrase in dangerous_phrases:
            ...
            assert decision == "respond_to_user", f"Should detect '{phrase}' as dangerous"
    ...
    def test_uncertainty_detection(self):
        ...
        for phrase in uncertain_phrases:
            ...
            decision = self.agent._decide_if_user_update_is_needed(phase_result)
            assert decision == "respond_to_user", f"Should detect '{phrase}' as uncertain"
    ...
    async def test_state_management_across_phases(self):
        ...
        with patch.object(self.agent, 'doctrine') as mock_doctrine:
            ...
            assert result['current_phase'] == 'define_blast_radius_completed'
            assert 'define_blast_radius_results' in result['investigation_context']
            assert result['investigation_context']['define_blast_radius_results'] == blast_radius_result
    ...
    async def test_full_workflow_simulation(self):
        ...
        with patch.object(self.agent, 'doctrine') as mock_doctrine:
            ...
            updated_state = AgentState(
                ...
                user_query="Database connection issues",
                findings=[],
                investigation_context={'define_blast_radius_results': result1['investigation_context']['define_blast_radius_results']},
                awaiting_user_input=False,
            ...
            blast_results = result1['investigation_context']['define_blast_radius_results']
            timeline_results = result2['investigation_context']['establish_timeline_results']
