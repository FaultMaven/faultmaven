...
from faultmaven.agent.doctrine import TroubleshootingDoctrine, Phase
...
class TestTroubleshootingDoctrine:
    ...
    def setup_method(self):
        ...
        self.sample_agent_state = AgentState(
            ...
            findings=[
                {"finding": "Connection pool exhausted", "timestamp": "2024-01-01T10:00:00Z"},
                {"finding": "High memory usage detected", "timestamp": "2024-01-01T10:01:00Z"}
            ],
    ...
    async def test_execute_phase_timeline_with_llm(self):
        ...
        assert result['status'] == 'completed'
        assert 'timeline' in result['key_insight'].lower() or 'deployment' in result['key_insight'].lower()
    ...
    async def test_execute_phase_with_knowledge_base_tool(self):
        ...
        mock_kb_tool.search.return_value = "**Result 1**\nRelevant information\n**Result 2**\nMore info"
        ...
        assert any('Knowledge base search completed' in finding for finding in result['findings'])
    
    def test_get_next_phase(self):
        """Test get_next_phase method."""
        assert self.doctrine.get_next_phase(Phase.DEFINE_BLAST_RADIUS) == Phase.ESTABLISH_TIMELINE
        assert self.doctrine.get_next_phase(Phase.ESTABLISH_TIMELINE) == Phase.FORMULATE_HYPOTHESIS
        assert self.doctrine.get_next_phase(Phase.FORMULATE_HYPOTHESIS) == Phase.VALIDATE_HYPOTHESIS
        assert self.doctrine.get_next_phase(Phase.VALIDATE_HYPOTHESIS) == Phase.PROPOSE_SOLUTION
        assert self.doctrine.get_next_phase(Phase.PROPOSE_SOLUTION) is None  # Final phase
    
    def test_validate_phase_transition(self):
        """Test validate_phase_transition method."""
        # Valid transitions
        assert self.doctrine.validate_phase_transition(
            Phase.DEFINE_BLAST_RADIUS, 
            Phase.ESTABLISH_TIMELINE
        ) is True
    ...
    def test_fallback_methods_with_previous_findings(self):
        ...
        agent_state_with_findings = AgentState(
            ...
            findings=[
                {"finding": "Connection error detected in logs", "timestamp": "2024-01-01T10:00:00Z"},
                {"finding": "Performance degradation observed", "timestamp": "2024-01-01T10:01:00Z"}
            ],
            investigation_context={}
        )
        ...
        assert result['phase'] == Phase.FORMULATE_HYPOTHESIS.value
        assert len(result['hypothesis_list']) > 0
        assert any('Error-based failure hypothesis' in h for h in result['hypothesis_list'])
        assert any('Performance degradation hypothesis' in h for h in result['hypothesis_list'])
    ...
    def test_fallback_validation_with_hypothesis(self):
        ...
        agent_state_with_hypothesis = AgentState(
            ...
            investigation_context={
                'formulate_hypothesis_results': {
                    'hypothesis_list': ['Database connection failure', 'Network timeout']
                }
            }
        )
    ...
    def test_fallback_solution_with_validated_hypothesis(self):
        ...
        assert result['phase'] == Phase.PROPOSE_SOLUTION.value
        assert len(result['solution_options']) > 0
        assert any('Connection pool exhaustion' in solution for solution in result['solution_options'])
