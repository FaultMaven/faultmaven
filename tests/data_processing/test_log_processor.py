...
from faultmaven.models import DataType, DataInsightsResponse, AgentState
...
"""
        ...
        assert contextual['contextual_entries'] > 0  # Should find context-relevant entries
    ...
    def test_generate_recommendations(self, processor, sample_agent_state):
        ...
        insights = {
            ...
            'contextual_analysis': {
                'contextual_entries': 2,
                'contextual_errors': 1
            }
        }
        anomalies = []
        
        recommendations = processor._generate_recommendations(insights, anomalies, sample_agent_state)
        
        assert isinstance(recommendations, list)
        # Should generate some recommendations based on insights
        assert len(recommendations) > 0

    def test_generate_phase_specific_recommendations(self, processor, sample_agent_state):
    ...
    def test_calculate_confidence(self, processor, sample_unstructured_logs, sample_agent_state):
    ...
    @pytest.mark.asyncio
    async def test_process_success(self, processor, sample_unstructured_logs, sample_agent_state):
        """Test successful log processing."""
        data_id = "test-data-123"
        result = await processor.process(sample_unstructured_logs, data_id, sample_agent_state)
    ...
    async def test_process_invalid_content(self, processor, sample_agent_state):
        ...
        result = await processor.process("Invalid log format", data_id, sample_agent_state)
...
"""
    ...
    def test_detect_anomalies_no_anomalies(self, processor):
        ...
        # Create normal data
        normal_logs = "\n".join([
            f"2024-01-01 12:00:{i:02d} INFO Normal message {i}"
            for i in range(10)
        ])
    ...
    def test_suggest_next_action(self, processor, sample_agent_state):
        ...
        recommendations = processor._generate_recommendations(high_error_insights, [], sample_agent_state)
    ...
    @pytest.mark.asyncio
    async def test_process_complete_flow(self, processor, sample_unstructured_logs, sample_agent_state):
        """Test complete processing flow."""
        result = await processor.process(sample_unstructured_logs, "test_data_id", sample_agent_state)
...
"""
        ...
        result = await processor.process(anomaly_logs, "test_data_id", sample_agent_state)
    ...
    async def test_process_large_dataset(self, processor, sample_agent_state):
        ...
        large_logs = "\n".join([
            f"2024-01-01 12:00:{i:02d} INFO Message {i}"
            for i in range(100)
        ])
...
"""
        ...
        result = await processor.process(special_logs, "test_data_id", sample_agent_state)
        
        assert isinstance(result, DataInsightsResponse)
        assert result.data_id == "test_data_id"
        assert result.confidence_score >= 0.0

    def test_anomaly_detection_with_insufficient_data(self, processor, sample_agent_state):
