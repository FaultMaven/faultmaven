...
class TestOperationDeduplication:
    ...
    async def test_duplicate_operation_prevention(self):
        ...
        with patch(
            "faultmaven.infrastructure.logging.unified.get_logger"
        ) as mock_get_logger:
            mock_logger = Mock()
            ...
            start_logs = [
                call for call in info_calls if "Operation started" in str(call)
            ]
            end_logs = [
                call for call in info_calls if "Operation completed" in str(call)
            ]
            ...
            assert (
                total_logs < 6
            )  # Without deduplication would be 6 (3 starts + 3 ends)
    ...
    def test_boundary_deduplication(self):
        ...
        with patch(
            ...
            unified_logger.log_boundary(
                "test_operation", "inbound", data={"attempt": 1}
            )
            unified_logger.log_boundary(
                "test_operation", "inbound", data={"attempt": 2}
            )
            unified_logger.log_boundary(
                "test_operation", "inbound", data={"attempt": 3}
            )
            ...
            unified_logger.log_boundary(
                "test_operation", "outbound", data={"attempt": 4}
            )
...
class TestLoggingCoordinatorDeduplication:
    ...
    def test_log_once_basic_deduplication(self):
        ...
        LoggingCoordinator.log_once(
            "test_operation", mock_logger, "info", "Test message"
        )
        LoggingCoordinator.log_once(
            "test_operation", mock_logger, "info", "Test message 2"
        )
        LoggingCoordinator.log_once(
            "test_operation", mock_logger, "info", "Test message 3"
        )
    ...
    def test_log_once_different_levels_same_key(self):
        ...
        LoggingCoordinator.log_once(
            "same_key", mock_logger, "warning", "Warning message"
        )
    ...
    def test_log_once_across_request_boundaries(self):
        ...
        LoggingCoordinator.log_once(
            "shared_operation", mock_logger, "info", "Request 1"
        )
        LoggingCoordinator.log_once(
            "shared_operation", mock_logger, "info", "Request 1 duplicate"
        )
        ...
        LoggingCoordinator.log_once(
            "shared_operation", mock_logger, "info", "Request 2"
        )
        LoggingCoordinator.log_once(
            "shared_operation", mock_logger, "info", "Request 2 duplicate"
        )
...
class TestCrossLayerDeduplication:
    ...
    async def test_same_operation_different_layers(self):
        ...
        with patch(
            ...
            api_operations = [
                call
                for call in start_logs
                if "api.operation.process_request" in str(call)
            ]
            service_operations = [
                call
                for call in start_logs
                if "service.operation.process_request" in str(call)
            ]
            core_operations = [
                call
                for call in start_logs
                if "core.operation.process_request" in str(call)
            ]
    ...
    def test_boundary_logging_different_layers(self):
        ...
        with patch(
            ...
            infra_calls = [
                call for call in calls if "infrastructure.boundary" in str(call)
            ]
...
class TestMetricAndEventDeduplication:
    ...
    def test_event_deduplication_allows_multiple_occurrences(self):
        ...
        with patch(
            ...
            unified_logger.log_event(
                "business", "user_login", "info", {"user": "user1"}
            )
            time.sleep(0.001)  # Ensure different timestamps
            unified_logger.log_event(
                "business", "user_login", "info", {"user": "user2"}
            )
            time.sleep(0.001)
            unified_logger.log_event(
                "business", "user_login", "info", {"user": "user3"}
            )
    ...
    def test_mixed_logging_deduplication(self):
        ...
        with patch(
            ...
            unified_logger.log_event(
                "technical", "processing_complete", "info"
...
class TestConcurrentDeduplication:
    ...
    async def test_concurrent_operations_same_request(self):
        ...
        with patch(
            ...
            info_calls = mock_logger.info.call_args_list
            boundary_calls = [
                call for call in info_calls if "shared_boundary" in str(call)
            ]
    ...
    async def test_concurrent_requests_isolation(self):
        ...
        with patch(
            ...
            info_calls = mock_logger.info.call_args_list
            shared_op_calls = [
                call for call in info_calls if "shared_operation" in str(call)
            ]
