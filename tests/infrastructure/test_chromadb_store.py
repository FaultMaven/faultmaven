...
class TestChromaDBVectorStore:
    ...
    def mock_chromadb_client(self):
        ...
        with patch(
            "faultmaven.infrastructure.persistence.chromadb_store.chromadb"
        ) as mock_chromadb:
    ...
    def test_implements_ivectorstore_interface(
        self, mock_chromadb_client, mock_env_vars
    ):
    ...
    def test_base_external_client_integration(
        self, mock_chromadb_client, mock_env_vars
    ):
        """Test integration with BaseExternalClient"""
        with patch(
            "faultmaven.infrastructure.persistence.chromadb_store.BaseExternalClient.__init__"
        ) as mock_init:
            with patch(
                "faultmaven.infrastructure.persistence.chromadb_store.BaseExternalClient.call_external"
            ):
    ...
    async def test_call_external_integration(self, vector_store):
        ...
        with patch.object(
            store, "call_external", new_callable=AsyncMock
        ) as mock_call_external:
    ...
    async def test_logging_integration(self, vector_store):
        ...
        with (
            patch.object(store.logger, "info") as mock_info,
