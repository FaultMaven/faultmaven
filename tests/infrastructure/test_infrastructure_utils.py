                ...
                if failure_config.get("count", 0) > 0:
                    ...
                    if failure_config["type"] == "timeout":
                        ...
                        raise web.HTTPInternalServerError(
                            text=failure_config.get("message", "Server error")
                        )
...
@pytest.fixture
async def integrated_test_infrastructure(
    http_test_server, redis_test_cluster, vector_test_database
):
...
class ConcurrencyTester:
    ...
    async def measure_concurrent_performance(
        operation_factory, count: int, max_concurrency: int = None
    ):
...
def assert_performance_within_bounds(
    duration: float, min_time: float, max_time: float, operation: str = "operation"
):
    """Assert that operation performance is within expected bounds."""
    assert (
        min_time <= duration <= max_time
    ), f"{operation} took {duration:.3f}s, expected between {min_time:.3f}s and {max_time:.3f}s"
...
def assert_concurrent_performance(
    ...
    ), f"{operation} achieved {ops_per_second:.1f} ops/sec, expected at least {min_ops_per_second} ops/sec"
