...
from faultmaven.infrastructure.llm.providers.base import ProviderConfig, LLMResponse, BaseLLMProvider
...
def clean_env():
    ...
    for key in list(os.environ.keys()):
        if any(prefix in key for prefix in ['CHAT_', 'FIREWORKS_', 'OPENAI_', 'ANTHROPIC_', 
                                            'GEMINI_', 'HUGGINGFACE_', 'OPENROUTER_', 'LOCAL_']):
            del os.environ[key]
...
class TestProviderRegistryInitialization:
    ...
    def test_initialization_without_settings(self, mock_get_settings):
        ...
        assert "LLM provider registry requires unified settings system" in str(exc_info.value)
...
class TestProviderConfiguration:
    ...
    def test_provider_schema_completeness(self):
        ...
        required_fields = [
            'api_key_var', 'model_var', 'base_url_var', 'default_base_url',
            'default_model', 'provider_class', 'max_retries', 'timeout', 'confidence_score'
        ]
        
        for provider_name, schema in PROVIDER_SCHEMA.items():
            for field in required_fields:
                assert field in schema, f"Provider {provider_name} missing required field: {field}"
    ...
    def test_provider_initialization_success(self, clean_env, mock_provider_classes):
        ...
        with patch.dict('faultmaven.infrastructure.llm.providers.registry.PROVIDER_SCHEMA', {
            'test_provider': {
    ...
    def test_provider_initialization_failure(self, clean_env):
        ...
        with patch.dict('faultmaven.infrastructure.llm.providers.registry.PROVIDER_SCHEMA', {
            'failing_provider': {
                'provider_class': Mock(side_effect=Exception("Provider init failed"))
            }
        }):
            registry = ProviderRegistry()
            config = ProviderConfig(
...
class TestFallbackChain:
    ...
    def test_fallback_chain_setup_primary_available(self, clean_env, sample_env_vars, mock_provider_classes):
    ...
    def test_fallback_chain_setup_primary_unavailable(self, clean_env, mock_provider_classes):
    ...
    def test_invalid_primary_provider_fallback(self, clean_env, sample_env_vars, mock_provider_classes):
    ...
    @pytest.mark.asyncio
    async def test_route_request_success_first_provider(self, clean_env, sample_env_vars, mock_provider_classes):
    ...
    @pytest.mark.asyncio
    async def test_route_request_fallback_on_failure(self, clean_env, sample_env_vars, mock_provider_classes):
        """Test request routing falls back on provider failure."""
        os.environ.update(sample_env_vars)
        
        # Make first provider fail
        mock_provider_classes['fireworks'].generate.side_effect = Exception("Provider failed")
    ...
    @pytest.mark.asyncio
    async def test_route_request_all_providers_fail(self, clean_env, sample_env_vars, mock_provider_classes):
    ...
    @pytest.mark.asyncio
    async def test_route_request_confidence_threshold(self, clean_env, sample_env_vars, mock_provider_classes):
...
class TestProviderHealthAndStatus:
    ...
    def test_get_available_providers(self, clean_env, sample_env_vars, mock_provider_classes):
    ...
    def test_get_specific_provider(self, clean_env, sample_env_vars, mock_provider_classes):
    ...
    def test_get_provider_status(self, clean_env, sample_env_vars, mock_provider_classes):
...
class TestApiKeySecurity:
    ...
    def test_api_key_masking_in_logs(self, clean_env, caplog):
        ...
        with patch('faultmaven.infrastructure.llm.providers.registry.FireworksProvider'):
            registry = ProviderRegistry()
            registry._ensure_initialized()
    ...
    def test_secure_provider_config_storage(self, clean_env, sample_env_vars, mock_provider_classes):
...
class TestRegistryStateManagement:
    ...
    def test_registry_reset_clears_state(self, clean_env, sample_env_vars, mock_provider_classes):
