...
class TestRealTracingBehavior:
    ...
    async def test_real_error_span_handling(self, mock_opik_tracer):
        ...
        success_span = next(
            s for s in spans_collected if s.name == "successful_operation"
        )
        handled_error_span = next(
            s for s in spans_collected if s.name == "handled_error_operation"
        )
        unhandled_error_span = next(
            s for s in spans_collected if s.name == "unhandled_error_operation"
        )
...
class TestRealMetricsCollection:
    ...
    def mock_metrics_collector(self):
        ...
        metrics_collected = {"counters": {}, "histograms": {}, "gauges": {}}

        class MockMetricsCollector:
            def increment_counter(self, name, value=1, tags=None):
                key = f"{name}:{json.dumps(tags or {}, sort_keys=True)}"
                metrics_collected["counters"][key] = (
                    metrics_collected["counters"].get(key, 0) + value
                )
        ...
        # Check if Prometheus metrics are available before patching
        from faultmaven.infrastructure.observability.tracing import PROMETHEUS_AVAILABLE

        # Create a context manager that conditionally patches metrics
        if PROMETHEUS_AVAILABLE:
            # Import metrics to ensure they exist
            ...
            with (
                ...
                ) as mock_request_counter,
                patch(
                    "faultmaven.infrastructure.observability.tracing.LLM_REQUEST_COUNTER"
                ) as mock_llm_counter,
                patch(
                    "faultmaven.infrastructure.observability.tracing.REQUEST_DURATION"
                ) as mock_request_duration,
                patch(
                    "faultmaven.infrastructure.observability.tracing.LLM_REQUEST_DURATION"
                ) as mock_llm_duration,
                patch(
                    "faultmaven.infrastructure.observability.tracing.GENERIC_FUNCTION_DURATION"
                ) as mock_generic_duration,
                patch(
                    "faultmaven.infrastructure.observability.tracing.ACTIVE_SESSIONS"
                ) as mock_active_sessions,
            ):

                # Configure mock counters
                mock_request_counter.labels.return_value.inc.side_effect = (
                    lambda value=1: collector.increment_counter(
                        "api.requests",
                        value,
                        {"endpoint": "mock", "method": "GET", "status": "200"},
                    )
                )
                mock_llm_counter.labels.return_value.inc.side_effect = (
                    lambda value=1: collector.increment_counter(
                        "llm.requests",
                        value,
                        {"provider": "mock", "model": "mock", "status": "200"},
                    )
                )

                # Configure mock histograms
                mock_request_duration.labels.return_value.observe.side_effect = (
                    lambda value: collector.record_histogram(
                        "api.response_time_ms",
                        value,
                        {"endpoint": "mock", "method": "GET"},
                    )
                )
                mock_llm_duration.labels.return_value.observe.side_effect = (
                    lambda value: collector.record_histogram(
                ...
                mock_generic_duration.labels.return_value.observe.side_effect = (
                    lambda value: collector.record_histogram(
                        "function.duration_ms",
                        value,
                        {"function_name": "mock", "status": "success"},
                    )
                )

                # Configure mock gauges
                mock_active_sessions.set.side_effect = (
                    lambda value: collector.set_gauge(
            ...
            # Prometheus not available - skip patching and yield collector without mocks
            yield collector, metrics_collected
    ...
    def test_real_counter_metrics_behavior(self, mock_metrics_collector):
        ...
        # Import metrics only if Prometheus is available
        from faultmaven.infrastructure.observability.tracing import (
            LLM_REQUEST_COUNTER,
            REQUEST_COUNTER,
        )
        ...
        api_key = (
            'api.requests:{"endpoint": "/users", "method": "GET", "status": "200"}'
        )
        llm_key = (
            'llm.requests:{"provider": "openai", "model": "gpt-4", "status": "success"}'
        )
    ...
    def test_real_histogram_metrics_behavior(self, mock_metrics_collector):
        ...
        with patch.object(REQUEST_DURATION, "labels") as mock_req_duration_labels:
            ...
            mock_histogram.observe.side_effect = (
                lambda value: collector.record_histogram(
            ...
            for response_time in api_response_times:
                REQUEST_DURATION.labels(endpoint="/users", method="GET").observe(
                    response_time
                )
        ...
        with patch.object(LLM_REQUEST_DURATION, "labels") as mock_llm_duration_labels:
            ...
            mock_llm_histogram.observe.side_effect = (
                lambda value: collector.record_histogram(
                    "llm.response_time_seconds",
                    value,
            ...
            for response_time in llm_response_times:
                LLM_REQUEST_DURATION.labels(provider="openai", model="gpt-4").observe(
                    response_time
                )
        ...
        with patch.object(GENERIC_FUNCTION_DURATION, "labels") as mock_generic_labels:
            ...
            mock_generic_histogram.observe.side_effect = (
                lambda value: collector.record_histogram(
                    "function.duration_seconds",
                    value,
                    {"function_name": "process_data", "status": "success"},
                )
            )
    ...
    def test_real_gauge_metrics_behavior(self, mock_metrics_collector):
        ...
        # Import metrics only if Prometheus is available
        from faultmaven.infrastructure.observability.tracing import ACTIVE_SESSIONS
        ...
        collector.set_gauge(
            "system.cpu_usage_percent", 45.7, tags={"host": "api-server-1"}
        )
        collector.set_gauge(
            "system.memory_usage_percent", 67.2, tags={"host": "api-server-1"}
        )
        collector.set_gauge(
            "database.active_connections", 15, tags={"database": "redis"}
        )

        # Update some gauges to test overwriting behavior
        collector.set_gauge(
            "system.cpu_usage_percent", 52.3, tags={"host": "api-server-1"}
        )
    ...
    async def test_real_metrics_performance_under_load(self, mock_metrics_collector):
        ...
        async def generate_metrics_load(operation_id):
            ...
            with (
                ...
                mock_counter_labels.return_value.inc.side_effect = (
                    lambda value=1: collector.increment_counter(
                        "load_test.operations", value, {"operation_id": operation_id}
                    )
                )
                mock_histogram_labels.return_value.observe.side_effect = (
                    lambda value: collector.record_histogram(
                        "load_test.processing_time_ms",
                        value,
                ...
                for i in range(50):  # Reduced iterations for faster test execution
                    ...
                    REQUEST_COUNTER.labels(
                        endpoint=f"/test_{operation_id}", method="GET", status="200"
                    ).inc()

                    # Test histogram observations
                    REQUEST_DURATION.labels(
                        endpoint=f"/test_{operation_id}", method="GET"
                    ).observe(i * 0.001)
        ...
        histogram_keys = [
            k for k in histograms.keys() if "load_test.processing_time_ms" in k
        ]
...
class TestRealLogCorrelationIntegration:
    ...
    async def test_real_integrated_observability_workflow(
        self, integrated_observability
    ):
        ...
        with tracer.trace("troubleshooting_session") as session_span:
            ...
            metrics.increment_counter(
                "sessions.started", tags={"type": "troubleshooting"}
            )
            ...
            for step_data in investigation_steps:
                with tracer.trace(f"step_{step_data['name']}") as step_span:
                    ...
                    # Log step start
                    logger.info(
                        f"Executing step {step_data['name']} for session {session_id}"
                    )
                    ...
                    if step_data["name"] == "upload_logs":
                        ...
                        step_span.set_attribute(
                            "patterns.found", step_data["patterns_found"]
                        )
                        metrics.set_gauge(
                            "analysis.patterns_found",
                            step_data["patterns_found"],
                            tags={"session_id": session_id},
                        )
                        ...
                        step_span.set_attribute(
                            "solution.confidence", step_data["confidence"]
                        )
                        metrics.set_gauge(
                            "solution.confidence",
                    ...
                    # Log step completion
                    logger.info(
                        f"Completed step {step_data['name']} in {actual_duration:.1f}ms"
                    )
                    step_span.add_event(
                        "step_completed", {"duration_ms": actual_duration}
                    )
            ...
            metrics.increment_counter(
                "sessions.completed",
                tags={"type": "troubleshooting", "status": "success"},
            )
        ...
        sessions_completed_key = (
            'sessions.completed:{"status": "success", "type": "troubleshooting"}'
        )
        ...
        duration_histogram_keys = [
            k for k in histograms.keys() if "step_duration_ms" in k
        ]
        ...
        assert (
            len(session_logs) >= 6
        )  # Actual captured logs: start + 4 steps + completion

        # Check log sequencing and content
        start_log = next(
            log
            for log in session_logs
            if "Starting troubleshooting" in log.getMessage()
        )
        completion_log = next(
            log for log in session_logs if "completed successfully" in log.getMessage()
        )
    ...
    async def test_real_error_correlation_across_observability(
        self, integrated_observability
    ):
        ...
        try:
            with tracer.trace("error_prone_operation") as error_span:
                ...
                logger.info(
                    f"Starting error-prone operation for session {error_session_id}"
                )
                metrics.increment_counter(
                    "operations.started", tags={"type": "data_processing"}
                )

                # Simulate error occurrence
                error_span.add_event(
                    "error_detected", {"error_type": "validation_error"}
                )
                logger.error(
                    f"Validation error in session {error_session_id}: Invalid data format"
                )
                ...
                metrics.increment_counter(
                    "operations.retries", tags={"session_id": error_session_id}
                )
            ...
            metrics.increment_counter(
                "operations.failed",
                tags={"type": "data_processing", "session_id": error_session_id},
            )
        ...
        validation_error_log = next(
            log for log in error_logs if "Validation error" in log.getMessage()
        )
        retry_log = next(
            log for log in error_logs if "Retrying operation" in log.getMessage()
        )
        final_failure_log = next(
            log for log in error_logs if "Operation failed" in log.getMessage()
        )
        ...
        assert (
            validation_error_log.created < retry_log.created < final_failure_log.created
        )
