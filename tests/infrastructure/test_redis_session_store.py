...
class TestRedisSessionStore:
    ...
    def mock_redis_client(self):
        ...
        with patch(
            "faultmaven.modules.auth.infrastructure.stores.redis_session_store.create_redis_client"
    ...
    async def test_extend_ttl_default(self, session_store):
        ...
        mock_client.expire.assert_called_once_with(
            "session:test-123", store.default_ttl
        )
        assert result is True
    ...
    async def test_extend_ttl_failure(self, session_store):
        ...
        mock_client.expire.assert_called_once_with(
            "session:nonexistent", store.default_ttl
        )
        assert result is False
