...
class TestProtectionIntegration:
    ...
    async def test_protection_endpoints_integration_with_real_app(self):
        ...
        mock_protection_system.get_protection_metrics = AsyncMock(
            return_value={
                "timestamp": "2025-01-15T10:30:00Z",
                "basic_metrics": {
        ...
        async with AsyncClient(
            ...
            with patch.object(
                app, "extra", {"protection_system": mock_protection_system}
            ):
                ...
                for endpoint, expected_key in endpoints_and_expected:
                    ...
                    assert isinstance(
                        data, dict
                    ), f"Endpoint {endpoint} should return dict"
                    assert (
                        expected_key in data
                    ), f"Endpoint {endpoint} missing expected key {expected_key}"
    ...
    async def test_protection_endpoints_openapi_compliance(self):
        ...
        async with AsyncClient(
            ...
            for endpoint in protection_endpoints:
                assert (
                    endpoint in paths
                ), f"Protection endpoint {endpoint} not documented in OpenAPI spec"

                # Verify GET method is documented
                endpoint_spec = paths[endpoint]
                assert (
                    "get" in endpoint_spec
                ), f"GET method not documented for {endpoint}"
                ...
                assert (
                    "200" in get_spec["responses"]
                ), f"No 200 response defined for {endpoint}"
        ...
        async with AsyncClient(
            ...
            with patch.object(app, "extra", {}):
                ...
                for endpoint in endpoints:
                    ...
                    ), f"Endpoint {endpoint} should return 503 when protection system unavailable"
                    assert response.headers["content-type"] == "application/json"
    ...
    async def test_protection_endpoints_contract_validation(self):
        ...
        mock_protection_system.get_protection_status = AsyncMock(
            return_value={
                ...
                "statistics": {"total_requests": 5000, "protected_requests": 150},
            }
        )
