    ...
    async def simple_app():
        ...
        try:
            ...
            async def mock_agent_service():
                ...
                async def mock_process_query(*args, **kwargs):
                    ...
                    try:
                        ...
                        if args:
                            ...
                            session_id = getattr(
                                query_request, "session_id", "test_session_12345"
                            )
                    ...
                    if "database" in query_lower or "connection" in query_lower:
                        ...
                        findings.extend(
                            [
                                ...
                                {
                                    "type": "info",
                                    "message": "CPU utilization within normal range",
                                    "severity": "low",
                ...
                async def mock_get_investigation_results(
                    investigation_id, session_id=None, *args, **kwargs
                ):
                ...
                async def mock_list_session_investigations(*args, **kwargs):
                    return [
                        ...
                        {
                            "investigation_id": "inv_3",
                            "query": "Network issue",
                            "status": "completed",
                            "created_at": "2024-01-01T12:02:00Z",
                        },
                        {
                            "investigation_id": "inv_4",
                            "query": "CPU issue",
                            "status": "completed",
                ...
                mock_service.list_session_investigations = (
                    mock_list_session_investigations
                )

                # Add get_case_results method to match actual AgentService
                async def mock_get_case_results(
                    case_id, session_id=None, *args, **kwargs
                ):
                    return await mock_get_investigation_results(
                        case_id, session_id, *args, **kwargs
                    )

                mock_service.get_case_results = mock_get_case_results

                # Add list_session_cases method to match actual AgentService
                async def mock_list_session_cases(
                    session_id, limit=10, offset=0, *args, **kwargs
                ):
                    return await mock_list_session_investigations(
                        session_id, limit, offset, *args, **kwargs
                    )
            ...
            async def mock_data_service():
                ...
                async def mock_ingest_data(*args, **kwargs):
                    ...
                    uploaded_data = {
                        ...
                        "content": content
                        or "2024-01-01 12:00:01 INFO Default test content",
                        "file_name": filename,
            ...
            async def mock_knowledge_service():
                    ...
                    **kwargs,
                    ...
                    document = {
                        ...
                        "tags": (
                            ...
                            else (tags.split(",") if isinstance(tags, str) else [])
                        ),
                    ...
                    test_stats["document_types"][doc_type] = (
                        test_stats["document_types"].get(doc_type, 0) + 1
                    )
                    cat = category or "general"
                    test_stats["categories"][cat] = (
                        test_stats["categories"].get(cat, 0) + 1
                    )
                ...
                async def mock_search_documents(
                    query, document_type=None, tags=None, limit=10, *args, **kwargs
                ):
                ...
                async def mock_update_document_metadata(document_id, *args, **kwargs):
                    ...
                    updates = {
                        k: v for k, v in kwargs.items() if k not in ["document_id"]
                    }
                    doc = test_documents.get(document_id)
                    if doc:
                        doc.update(updates)
                ...
                async def mock_delete_document(document_id, *args, **kwargs):
                    if document_id in test_documents:
                        ...
                        test_stats["total_documents"] = max(
                            0, test_stats["total_documents"] - 1
                        )
                        return {"success": True, "document_id": document_id}
                    return {"success": False, "error": "Document not found"}

                async def mock_bulk_update_documents(
                    document_ids, updates, *args, **kwargs
                ):
                    updated_count = 0
                ...
                async def mock_bulk_delete_documents(document_ids, *args, **kwargs):
                    ...
                    for doc_id in document_ids:
                        if doc_id in test_documents:
                            ...
                            deleted_count += 1
                    test_stats["total_documents"] = max(
                        0, test_stats["total_documents"] - deleted_count
                    )
            ...
            async def mock_session_service():
                ...
                async def mock_list_sessions(
                    user_id=None, status=None, limit=None, offset=None, *args, **kwargs
                ):
                    sessions = []
                ...
                async def mock_record_data_upload_operation(
                    session_id,
                    data_id,
                    filename,
                    file_size=0,
                ...
                async def mock_get_session_stats(session_id, *args, **kwargs):
                    ...
                    query_operations = len(
                        [
                            h
                            for h in session.case_history
            ...
            async def content_type_validation_middleware(request: Request, call_next):
                ...
                if request.method in ["POST", "PUT"] and request.url.path.startswith(
                    ...
                    if (
                        "/agent/query" in request.url.path
                        or "/agent/troubleshoot" in request.url.path
                        or "/knowledge/search" in request.url.path
                    ):
                        if (
                            content_type
    ...
    def performance_tracker():
        ...
        class PerformanceTracker:
            ...
            def assert_performance_target(
                self, operation_name: str, target_seconds: float
            ):
                duration = self.timings.get(operation_name, 0)
                assert (
                    duration <= target_seconds
                ), f"Operation {operation_name} took {duration}s, expected <= {target_seconds}s"
