...
class TestNoServiceLocatorInApiLayer:
    ...
    def test_module_routes_no_container_imports(self):
        ...
        module_routes = glob.glob(
            'faultmaven/modules/*/api/routes.py'
        ) + glob.glob(
            ...
            if os.path.exists(route_file):
                ...
                for i, line in enumerate(lines):
                    ...
                    if 'container.get_' in line:
                        ...
                        code_part = line.split('#')[0] if '#' in line else line
                        if 'container.get_' in code_part and '"' not in code_part and "'" not in code_part:
                            pytest.fail(
...
class TestArchitecturalCompliance:
    ...
    def test_main_py_wires_services_to_app_state(self):
        ...
        assert 'app.state.' in content, \
            "main.py should wire services to app.state"

        # Should have Composition Root comment or pattern
        assert 'Composition Root' in content or 'app.state' in content, \
