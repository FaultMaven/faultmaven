...
def test_refused_root_adoption_records_the_claim_on_the_node():
    """fm#1137: the refusal denies the ATTACHMENT, not the claim. The claim is
    kept on the claimed node so the §7.1 restatement guard does not later read
    the refused hypothesis as an unrelated standing cause and put it into the
    frame of the very node it claimed."""
    from faultmaven.core.investigation.causal_graph import contested_root_claimants

    eng = _engine()
    case = _case()
    owner, claimant = _hyp(), _second_hyp()
    case.hypotheses = {owner.hypothesis_id: owner, claimant.hypothesis_id: claimant}
    owned_root = _root_owned_by(eng, case, owner)
    ...
    eng._apply_chain_emission(
        ...
        {
            "hypotheses_generated": [],
            "hyp_root_refs": {claimant.hypothesis_id: owned_root},
        },
    )

    assert claimant.root_node_id != owned_root  # refused, as designed
    assert contested_root_claimants(case.causal_nodes[owned_root]) == frozenset(
        {claimant.hypothesis_id}
    )
    # Idempotent across turns — a re-emitted ref must not grow the list.
    eng._apply_chain_emission(
        case,
        _updates(),
        {
    ...
    assert case.causal_nodes[owned_root].metadata["contested_root_claims"] == [
        claimant.hypothesis_id
    ]
