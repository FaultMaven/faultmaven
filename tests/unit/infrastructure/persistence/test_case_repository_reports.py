...
@pytest.mark.asyncio
@pytest.mark.unit
async def test_inmemory_add_report(
    inmemory_repository: InMemoryCaseRepository, sample_report: CaseReport
):
...
@pytest.mark.asyncio
@pytest.mark.unit
async def test_inmemory_get_report(
    inmemory_repository: InMemoryCaseRepository, sample_report: CaseReport
):
...
@pytest.mark.asyncio
@pytest.mark.unit
async def test_inmemory_get_report_not_found(
...
async def test_inmemory_get_reports_current_only(
    ...
    old_report = sample_report.model_copy(
        update={
            "report_id": str(uuid4()),
            "is_current": False,
            "version": 2,
...
async def test_inmemory_update_report(
    ...
    # Update report
    updated_report = sample_report.model_copy(
        update={
    ...
    assert (
        result.updated_at != original_generated_at
        or result.updated_at == result.generated_at
    )
    ...
    assert (
        retrieved.updated_at == result.updated_at
    )  # Should match the updated timestamp
...
async def test_inmemory_update_report_marks_previous_not_current(
    ...
    second_report = sample_report.model_copy(
        update={
            "report_id": str(uuid4()),
            "version": 1,
            "is_current": False,
    ...
    updated_second = second_report.model_copy(
        update={
            "is_current": True,
...
@pytest.mark.asyncio
@pytest.mark.unit
async def test_inmemory_delete_report_not_found(
    inmemory_repository: InMemoryCaseRepository,
):
...
@pytest.mark.asyncio
@pytest.mark.skip(
    reason="PostgreSQLHybridCaseRepository requires PostgreSQL-specific SQL - tested via integration tests"
)
...
async def test_report_versioning_flow(
    ...
    report_v2 = sample_report.model_copy(
        update={
            "report_id": str(uuid4()),
            "version": 2,
            "is_current": True,
