...
Migration 044 installs the constraint trigger (fm#1161); ``organization_repository``
carries the constant and the recogniser that let a caller turn the trigger's
refusal back into its own friendly error. The trigger is exercised for real
...
copy of ``SYSTEM_ROLE_IDS[Role.ADMIN]`` — the same frozen-snapshot convention
migration 029 used when it seeded the roles, and for the same reason: a
migration states the value it was written against instead of importing runtime
code that can move underneath it.
...
MIGRATION = (
    ...
    / "alembic"
    / "versions"
    / "20260826_1200_c7d8e9f0a1b2_044_last_admin_constraint_trigger.py"
)


@pytest.mark.unit
def test_migration_044_admin_role_id_matches_the_seeded_role():
    assert MIGRATION.exists(), f"migration 044 is missing from {MIGRATION}"
    source = MIGRATION.read_text()

    match = re.search(r'^_ADMIN_ROLE_ID = "([0-9a-f-]+)"$', source, re.MULTILINE)
    assert match, "migration 044 no longer declares _ADMIN_ROLE_ID as a literal"

    assert match.group(1) == SYSTEM_ROLE_IDS[Role.ADMIN], (
        "migration 044's frozen admin role id has drifted from "
...
@pytest.mark.unit
def test_migration_044_trigger_name_matches_the_shipped_constant():
    ...
    """
    source = MIGRATION.read_text()
    match = re.search(r'^_TRIGGER = "([a-z_]+)"$', source, re.MULTILINE)
    assert match, "migration 044 no longer declares _TRIGGER as a literal"
    assert match.group(1) == LAST_ADMIN_CONSTRAINT


@pytest.mark.unit
def test_migration_044_trigger_is_deferred_and_security_definer():
    ...
    ``organization_members`` (migration 018, fail-closed) from making the guard
    count zero admins whenever no tenant is bound. Losing either is a silent
