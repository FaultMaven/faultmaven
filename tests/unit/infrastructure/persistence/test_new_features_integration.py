...
async def test_standalone_evidence_workflow():
    ...
    case_uuid = uuid4()
    linked = await repo.link_standalone_evidence_to_case(evidence.evidence_id, str(case_uuid))
...
async def test_agent_execution_workflow():
    ...
    assert created.execution_id == execution.execution_id, "Should return same execution"
    ...
    assert retrieved.execution_id == execution.execution_id, "Should return same execution"
    ...
    assert created_tc.tool_call_id == tool_call.tool_call_id, "Should return same tool call"
    ...
    assert retrieved_with_tc.tool_calls[0].tool_call_id == tool_call.tool_call_id, "Should have correct tool call"
    ...
    assert tool_calls[0].tool_call_id == tool_call.tool_call_id, "Should return correct tool call"
    ...
    assert latest.execution_id == execution.execution_id, "Should return correct execution"
    ...
    tool_calls_after = await repo.get_agent_tool_calls_for_execution(execution.execution_id)
...
async def test_enum_filtering():
    ...
    assert completed[0].execution_id == exec1.execution_id, "Should return correct execution"
    ...
    assert investigator[0].execution_id == exec1.execution_id, "Should return correct execution"
    ...
    assert latest_investigator.agent_type == AgentType.INVESTIGATOR, "Should filter correctly"
