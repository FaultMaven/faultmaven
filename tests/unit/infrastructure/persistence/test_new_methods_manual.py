...
async def test_standalone_evidence():
    ...
    assert evidence.evidence_id in evidence_ids, f"Expected {evidence.evidence_id} in results"
...
async def test_agent_executions():
    ...
    print(f"✅ Retrieved execution with tool calls: {len(retrieved_with_tc.tool_calls)} tool calls")
    ...
    tool_calls_after_delete = await repo.get_agent_tool_calls_for_execution(created.execution_id)
