...
def create_sample_case(
    ...
    # Create case with default progress (InvestigationProgress has default_factory)
    case = Case(
        case_id=case_id,
        user_id=user_id,
    ...
    # Progress is created by default_factory, but ensure it has required attributes
    if not hasattr(case, 'progress') or case.progress is None:
