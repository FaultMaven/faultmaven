...
def _make_case_service(factory, *, owner=CALLER, shared_to=(), resume=True):
    ...
    """
    resume_mock = (
        AsyncMock(side_effect=resume)
        if isinstance(resume, Exception)
        else AsyncMock(return_value=resume)
    )
    service = factory(
        owner=owner,
        shared_to=shared_to,
    ...
    # visible. Losing this assertion in a rewrite is how `get_case(case_id)`
    # with no caller survived the whole module.
    service.calls = []
...
async def test_forwards_the_caller_to_the_gate(
    ...
    """
    service = _make_case_service(owner_or_shared_case_service)
...
async def test_a_case_the_caller_cannot_reach_is_a_404(
    build_app, call_api, owner_or_shared_case_service
):
    ...
    service = _make_case_service(
        owner_or_shared_case_service, owner="user-someone-else"
    )
    app = build_app(session=_session(), case_service=service)
...
async def test_the_services_access_verdict_is_a_404_not_a_500(
    ...
    """
    service = _make_case_service(
        owner_or_shared_case_service, resume=NotFoundError("Case", CASE_ID)
    )
    app = build_app(session=_session(), case_service=service)
...
async def test_an_unevaluable_session_gate_is_a_503(
    build_app, call_api, owner_or_shared_case_service
):
...
async def test_an_unreachable_session_store_is_also_a_503(
    build_app, call_api, owner_or_shared_case_service
):
    """The second spelling of "the gate could not be evaluated".

    A store that is CONFIGURED but unreachable raises `SessionStoreException`,
    which descends from `SessionException` and NOT from `ServiceException`.
    Catching only the latter made the two spellings answer 503 and 500.
    """
    service = _make_case_service(owner_or_shared_case_service)
    app = build_app(
...
async def test_refuses_a_session_that_is_not_the_callers(
    build_app, call_api, owner_or_shared_case_service
):
...
async def test_refuses_an_unknown_session_the_same_way(
    build_app, call_api, owner_or_shared_case_service
):
...
async def test_a_failed_link_is_a_server_error_not_an_absence(
    build_app, call_api, owner_or_shared_case_service
):
    ...
    service = _make_case_service(owner_or_shared_case_service, resume=False)
    app = build_app(session=_session(), case_service=service)
...
async def test_resumes_a_case_the_caller_can_reach(
    build_app, call_api, owner_or_shared_case_service
):
