...
class TestStandaloneEvidence:
    @pytest.mark.asyncio
    async def test_create_and_get_standalone_evidence_log(self, repository):
        # Pre-seed a case since evidence_artifacts has an FK (not enforced in SQLite,
        # but keeping the test closer to real usage)
        case = _make_case()
        await repository.save(case)

        artifact = await repository.create_standalone_evidence(
            filename="server.log",
            content_type="text/plain",
            size_bytes=5000,
            storage_path="/tmp/server.log",
            uploaded_by="user_alpha",
            description="Recent server log",
            tags=["incident", "prod"],
        )

        assert artifact.evidence_id.startswith("ev_")
        assert artifact.evidence_type.value == "log_file"
        assert artifact.tags == ["incident", "prod"]

        fetched = await repository.get_standalone_evidence(artifact.evidence_id)
        assert fetched is not None
        assert fetched.evidence_id == artifact.evidence_id
        assert fetched.tags == ["incident", "prod"]
        assert fetched.description == "Recent server log"

    @pytest.mark.asyncio
    async def test_create_standalone_evidence_infers_type_from_content_type(
        self, repository
    ):
        case = _make_case()
        await repository.save(case)

        img = await repository.create_standalone_evidence(
            filename="shot.png",
        ...
        vid = await repository.create_standalone_evidence(
            filename="clip.mp4",
        ...
        har = await repository.create_standalone_evidence(
            filename="capture.har",
            content_type="application/x-har",
            size_bytes=2000,
            storage_path="/tmp/capture.har",
            uploaded_by="user_alpha",
        )
        other = await repository.create_standalone_evidence(
            filename="thing.bin",
            content_type="application/octet-stream",
            size_bytes=10,
            storage_path="/tmp/thing.bin",
            uploaded_by="user_alpha",
        )

        assert img.evidence_type.value == "screenshot"
        assert vid.evidence_type.value == "video_recording"
        assert har.evidence_type.value == "har_file"
        assert other.evidence_type.value == "other"

    @pytest.mark.asyncio
    async def test_get_standalone_evidence_missing_returns_none(self, repository):
        assert await repository.get_standalone_evidence("ev_missing") is None

    @pytest.mark.asyncio
    async def test_list_standalone_evidence_applies_filters(self, repository):
        case = _make_case()
        await repository.save(case)

        await repository.create_standalone_evidence(
            filename="a.log",
            content_type="text/plain",
            size_bytes=1,
            storage_path="/a",
        ...
        results, total = await repository.list_standalone_evidence(_Filters())

        assert total == 2
        assert {r.user_id for r in results} == {"user_1"}

    @pytest.mark.asyncio
    async def test_list_standalone_evidence_filename_contains(self, repository):
        case = _make_case()
        await repository.save(case)
        await repository.create_standalone_evidence(
            filename="app.log",
        ...
        assert total == 1
        assert results[0].original_filename == "app.log"

    @pytest.mark.asyncio
    async def test_list_standalone_evidence_tag_filter(self, repository):
        case = _make_case()
        await repository.save(case)
        await repository.create_standalone_evidence(
            filename="a.log",
        ...
        results, _ = await repository.list_standalone_evidence(_Filters())

        assert len(results) == 1
        assert "red" in results[0].tags

    @pytest.mark.asyncio
    async def test_delete_standalone_evidence(self, repository):
        case = _make_case()
        await repository.save(case)
        a = await repository.create_standalone_evidence(
            filename="x.log",
            content_type="text/plain",
            size_bytes=1,
            storage_path="/x",
        ...
        result = await repository.delete_standalone_evidence(a.evidence_id)

        assert result is True
        assert await repository.get_standalone_evidence(a.evidence_id) is None

    @pytest.mark.asyncio
    async def test_delete_standalone_evidence_missing(self, repository):
        assert await repository.delete_standalone_evidence("ev_missing") is False

    @pytest.mark.asyncio
    async def test_link_standalone_evidence_to_case(self, repository):
        case = _make_case()
        await repository.save(case)
        a = await repository.create_standalone_evidence(
            filename="x.log",
        ...
        linked = await repository.link_standalone_evidence_to_case(
            a.evidence_id, case.case_id
        )

        assert linked is not None
        assert linked.case_id == case.case_id
        assert case.case_id in linked.linked_case_ids

    @pytest.mark.asyncio
    async def test_link_standalone_evidence_missing_returns_none(self, repository):
        case = _make_case()
        await repository.save(case)

        result = await repository.link_standalone_evidence_to_case(
            "ev_missing", case.case_id
        )
