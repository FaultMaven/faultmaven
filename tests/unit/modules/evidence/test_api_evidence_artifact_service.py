...
class TestUploadEvidence:
    ...
    async def test_upload_evidence_with_is_primary_sets_primary(
        ...
        # Mock get_standalone_evidence to return primary evidence after set_primary_evidence
...
class TestGetEvidence:
    ...
    async def test_get_evidence_success(
        ...
        mock_case_repo.get_standalone_evidence.assert_called_once_with(
    ...
    @pytest.mark.asyncio
    async def test_get_evidence_empty_id_returns_none(self, evidence_service):
    ...
    @pytest.mark.asyncio
    async def test_get_evidence_whitespace_id_returns_none(self, evidence_service):
...
class TestDownloadEvidence:
    ...
    async def test_download_evidence_success(
        ...
        mock_file_storage.retrieve_file.assert_called_once_with(
...
class TestUpdateEvidence:
    ...
    async def test_update_evidence_is_primary_true(
        ...
        mock_case_repo.get_standalone_evidence.side_effect = [
            sample_evidence,
            primary_evidence,
        ]
...
class TestDeleteEvidence:
    ...
    async def test_delete_evidence_success(
        ...
        mock_case_repo.delete_standalone_evidence.assert_called_once_with(
            sample_evidence.evidence_id
        )
    ...
    @pytest.mark.asyncio
    async def test_delete_evidence_empty_id_returns_false(self, evidence_service):
...
class TestHealthCheck:
    ...
    @pytest.mark.asyncio
    async def test_health_check_healthy(self, evidence_service, mock_file_storage):
...
class TestEdgeCases:
    ...
    async def test_upload_evidence_all_evidence_types(
        ...
        for evidence_type in EvidenceArtifactType:
            ...
            mock_case_repo.link_standalone_evidence_to_case.return_value = (
                linked_evidence
            )
    ...
    async def test_concurrent_uploads_generate_unique_ids(
        ...
        def create_evidence_side_effect(*args, **kwargs):
            nonlocal call_count
            call_count += 1
            evidence_id = f"evd_concurrent{call_count}"
            ...
            created_evidence = EvidenceArtifact(
                ...
                user_id="user_123",
                organization_id=sample_case.organization_id,
                original_filename=f"test{call_count-1}.png",
                stored_filename="abc_test.png",
                file_path="org/case/date/abc_test.png",
                evidence_type=EvidenceArtifactType.SCREENSHOT,
                mime_type="image/png",
        ...
        mock_case_repo.create_standalone_evidence.side_effect = (
            create_evidence_side_effect
        )

        def link_evidence_side_effect(evidence_id, case_id):
            # Find the evidence that was created
            evidence_id_str = (
                str(evidence_id) if not isinstance(evidence_id, str) else evidence_id
            )
            linked_evidence = EvidenceArtifact(
                ...
                user_id="user_123",
                organization_id=sample_case.organization_id,
                original_filename="abc_test.png",
                stored_filename="abc_test.png",
                file_path="org/case/date/abc_test.png",
                evidence_type=EvidenceArtifactType.SCREENSHOT,
                mime_type="image/png",
        ...
        mock_case_repo.link_standalone_evidence_to_case.side_effect = (
            link_evidence_side_effect
        )
