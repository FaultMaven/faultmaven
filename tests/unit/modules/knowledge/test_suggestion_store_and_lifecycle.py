...
1. **The store is unbounded.** Making the service a singleton gave its in-memory
   ...
   LLM-generated article. The store is capped and evicts, and it must never
   evict work a reviewer has not seen.

Since #1227 the store is a repository rather than a dict, and the cap is scoped
per organization (the durable store is one table shared by every tenant, so a
deployment-wide count would let one tenant's undrained inbox refuse another
tenant's extraction). These tests drive it through
``InMemorySuggestionRepository``, which — like the database one — hands back a
DETACHED COPY on every read: a suggestion mutated in the service is not stored
until it is saved, so an assertion about state has to re-read the store rather
than inspect the object it was handed.
...
class TestTheStoreIsBounded:
    async def test_it_stops_growing_at_the_cap(self):
        ...
        assert await _stored_count(svc) == 3
    ...
    async def test_the_oldest_decision_is_evicted_first(self):
        ...
        assert await _stored(svc, "oldest") is None
    ...
    async def test_rejected_suggestions_are_evictable_too(self):
        ...
        assert await _stored(svc, "rejected") is None

    async def test_eviction_is_logged(self, caplog):
        svc = _service(capacity=1)
        await _seed(svc, _suggestion("done", SuggestionStatus.APPROVED, age_seconds=50))
...
class TestPendingWorkIsNeverEvicted:
    ...
    async def test_one_terminal_entry_is_enough_to_make_room(self):
        ...
        assert await _stored(svc, "done") is None
