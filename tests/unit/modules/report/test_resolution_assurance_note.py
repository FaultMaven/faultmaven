...
@pytest.mark.asyncio
async def test_cited_branch_filtered_empty_falls_back_to_category_scan():
    """Reviewed: an LLM-authored ``evidence_basis`` can cite ONLY disqualified
    absence rows (e.g. the engine's M6 disconfirmation). The polarity filter
    then empties the cited list, and the section must FALL BACK to the tagged
    causal rows rather than silently vanish from the report."""
    case = _resolved_case(with_root=True, confirmed=False)
    engine_row = Evidence(
        evidence_id="ev_dddddddddddd",
        summary="Counterfactual disconfirmation (M6): fix failed",
        primary_purpose="failed-treatment disconfirmation",
        category=EvidenceCategory.CAUSAL_ABSENCE_EVIDENCE,
        source_type=EvidenceSourceType.USER_DESCRIPTION,
    ...
    # object.__setattr__ — the terminal fixture bypasses cross-field
    # validators (the established pattern in _resolved_case).
    object.__setattr__(
        case,
        ...
        RootCauseConclusion(
            ...
            confidence_level=ConfidenceLevel.CONFIDENT,
            likelihood=0.8,
            evidence_basis=["ev_dddddddddddd"],  # cites ONLY the disqualified row
        ),
    )
    service = ReportGenerationService()
    summary = await service._generate_resolution_summary(case, {"duration": "2h"})
    assert "Counterfactual disconfirmation" not in summary
    assert "## Confirming Evidence" in summary  # fallback rendered causal rows
