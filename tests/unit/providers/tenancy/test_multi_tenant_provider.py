...
@pytest.mark.asyncio
async def test_get_default_organization_raises_not_implemented(
    multi_tenant_provider
):
    ...
    assert "Multi-tenant mode does not have a default organization" in str(exc_info.value)
...
async def test_different_users_can_access_different_organizations(
    ...
    user1 = User(user_id="user_1", email="user1@org1.com", hashed_password="hashed", full_name="User One")
    user2 = User(user_id="user_2", email="user2@org2.com", hashed_password="hashed", full_name="User Two")
...
async def test_users_with_different_roles_can_access_same_org(
    ...
    admin_user = User(user_id="admin_1", email="admin@example.com", hashed_password="hashed", full_name="Admin User")
    member_user = User(user_id="member_1", email="member@example.com", hashed_password="hashed", full_name="Member User")
...
async def test_validation_error_includes_hint(
    ...
    assert "JWT" in error_message or "header" in error_message or "query" in error_message
...
async def test_concurrent_access_different_organizations(
    ...
    user_a = User(user_id="user_a", email="usera@example.com", hashed_password="hashed", full_name="User A")
    user_b = User(user_id="user_b", email="userb@example.com", hashed_password="hashed", full_name="User B")
    ...
    async def get_role_side_effect(org_id, user_id):
        await asyncio.sleep(0.01)  # Simulate async delay
        if (org_id == "org_a" and user_id == "user_a") or (org_id == "org_b" and user_id == "user_b"):
            return "member"
        return None
