...
``IShareRepository.list_resource_ids`` now takes a REQUIRED ``enterprise_id``
...
@pytest.mark.parametrize(
    "org", [ENTERPRISE_A, ENTERPRISE_B, STANDALONE_ENTERPRISE_ID, "ent-3"]
)
async def test_resolver_passes_the_callers_enterprise_through_unchanged(org):
    """Sweep several tenants: whatever enterprise comes in is the enterprise the query carries.
...
async def test_resolver_refuses_the_sentinel_under_multi():
    ...
    ``case.enterprise_id``, which ``create_case`` stamps from the *total*
    ...
    with _MULTI:
        assert (
            await resolve_shared_kb_ids(repo, ["team-1"], STANDALONE_ENTERPRISE_ID)
            == []
        )
...
@pytest.mark.security
@pytest.mark.asyncio
@pytest.mark.parametrize("org", [ENTERPRISE_A, ENTERPRISE_B])
...
@pytest.mark.parametrize(
    "team_ids,org",
    [(None, ENTERPRISE_A), ([], ENTERPRISE_A), (["team-1"], ""), ([], "")],
)
...
@pytest.mark.security
@pytest.mark.asyncio
@pytest.mark.parametrize("org", [ENTERPRISE_A, ENTERPRISE_B, "ent-3"])
async def test_search_documents_scopes_the_share_lookup_to_the_requesting_user(org):
    ...
    user = MagicMock(user_id="u1", enterprise_id=org)
...
async def test_search_documents_still_scopes_by_owner_when_the_team_arm_collapses():
    ...
    user = MagicMock(user_id="u1", enterprise_id=None)
...
@pytest.mark.security
@pytest.mark.asyncio
@pytest.mark.parametrize("org", [ENTERPRISE_A, ENTERPRISE_B, STANDALONE_ENTERPRISE_ID])
...
# =============================================================================
#
# ``get_current_enterprise_id`` is TOTAL: the contextvar's default is the Standalone
...
def test_an_unbound_execution_context_reads_as_the_standalone_sentinel():
    ...
    assert (
        contextvars.Context().run(get_current_enterprise_id) == STANDALONE_ENTERPRISE_ID
    )
...
def test_the_sentinel_is_a_tenant_under_single_and_not_under_multi():
    ...
    ``require_actor_enterprise`` refuses on, shared so the two cannot drift."""
    with _SINGLE:
        assert usable_tenant_id(STANDALONE_ENTERPRISE_ID) == STANDALONE_ENTERPRISE_ID
    with _MULTI:
        assert usable_tenant_id(STANDALONE_ENTERPRISE_ID) is None
...
def test_the_context_read_applies_the_same_rule(restore_org_context):
    ...
    with _SINGLE:
        assert get_current_tenant_id() == STANDALONE_ENTERPRISE_ID
    with _MULTI:
...
async def test_an_unbound_context_under_multi_collapses_the_case_allowlist(
    ...
    )  # == the unbound default, pinned above
    repo = _share_repo(["case-shared"])
    service = _case_service(repo)
