...
class TestEdgeCases:
    ...
    async def test_concurrent_uploads_generate_unique_ids(
        ...
        mock_case_repo.create_standalone_evidence.side_effect = create_evidence_side_effect
        
        def link_evidence_side_effect(evidence_id, case_id):
            # Find the evidence that was created
            evidence_id_str = str(evidence_id) if not isinstance(evidence_id, str) else evidence_id
            linked_evidence = EvidenceArtifact(
        ...
        mock_case_repo.link_standalone_evidence_to_case.side_effect = link_evidence_side_effect
