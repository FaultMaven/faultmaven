...
class TestSourceModel:
    ...
    def test_source_type_validation(self):
        ...
        with pytest.raises(ValidationError):
            Source(type="invalid_type", content="test content")
...
class TestViewStateModel:
    ...
    def test_view_state_creation_valid(self):
        ...
        user = User(user_id="user_123", email="test@example.com", name="Test User")

        case = Case(
            case_id="case_456", title="Database Connection Issue", owner_id="user_123"
        )
    ...
    def test_view_state_empty_uploaded_data(self):
        ...
        view_state = ViewState(session_id="session_123", user=user, uploaded_data=[])
    ...
    def test_view_state_required_fields(self):
        ...
        # Missing user
        with pytest.raises(ValidationError):
            ViewState(session_id="session_123")
    ...
    def test_view_state_new_fields_defaults(self):
        ...
        view_state = ViewState(session_id="session_123", user=user)
...
class TestQueryRequestModel:
    ...
    def test_query_request_creation_valid(self):
        ...
        request = QueryRequest(
            session_id="session_123", query="What is causing the database errors?"
        )
...
class TestAgentResponseModel:
    ...
    def test_agent_response_creation_valid_answer(self):
        ...
        view_state = ViewState(
            session_id="session_123", user=user, active_case=case, uploaded_data=[]
        )
    ...
    @pytest.mark.parametrize(
        "response_type,plan,should_pass",
        [
    ...
    def test_agent_response_plan_consistency_validator(
        self, response_type, plan, should_pass
    ):
    ...
    def test_agent_response_required_fields(self):
        ...
        # Missing content
        with pytest.raises(ValidationError):
            AgentResponse(response_type=ResponseType.ANSWER, view_state=view_state)

        # Missing response_type
        with pytest.raises(ValidationError):
            AgentResponse(content="Test content", view_state=view_state)

        # Missing view_state
        with pytest.raises(ValidationError):
            AgentResponse(content="Test content", response_type=ResponseType.ANSWER)
...
class TestErrorModels:
    ...
    def test_error_detail_creation_valid(self):
        ...
        error_detail = ErrorDetail(
            code="SESSION_NOT_FOUND", message="The specified session could not be found"
        )
...
class TestModelSerialization:
    ...
    def test_agent_response_serialization(self):
        ...
        case = Case(case_id="case_456", title="Serialization Test", owner_id="user_123")
        ...
        assert (
            reconstructed.view_state.active_case.case_id
            == response.view_state.active_case.case_id
        )
    ...
    def test_query_request_serialization(self):
        ...
        request = QueryRequest(session_id="session_123", query="Test query")
...
class TestEdgeCases:
    ...
    def test_large_data_validation(self):
        ...
        # Large plan
        plan = [
            PlanStep(description=f"Step {i}: Execute action {i}") for i in range(50)
        ]
